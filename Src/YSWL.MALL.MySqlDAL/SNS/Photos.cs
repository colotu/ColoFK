...
namespace YSWL.MALL.MySqlDAL.SNS
{
    ...
    public partial class Photos : IPhotos
    {
        ...
        public int GetMaxId()
        {
            return DbHelperMySQL.GetMaxID("PhotoID", "SNS_Photos");
        }
        ...
        public int Add(YSWL.MALL.Model.SNS.Photos model)
        {
            ...
            MySqlParameter[] parameters = {
					...
					new MySqlParameter("?OwnerPhotoId", MySqlDbType.Int32,4),
        ...
        public YSWL.MALL.Model.SNS.Photos GetModel(int PhotoID)
        {
            ...
            YSWL.MALL.Model.SNS.Photos model = new YSWL.MALL.Model.SNS.Photos();
        ...
        /// 得到一个对象实体
        /// </summary>
        public YSWL.MALL.Model.SNS.Photos DataRowToModel(DataRow row)
        {
            YSWL.MALL.Model.SNS.Photos model = new YSWL.MALL.Model.SNS.Photos();
            if (row != null)
            {
        ...
        public DataSet GetZuiInList(int CategoryId, int Top)
        {
            ...
            if (Top > 0)
            {
                strSql.Append("  FROM ( SELECT * FROM SNS_Photos ");
            }
            strSql.Append(" WHERE CategoryId=" + CategoryId + " AND IsRecomend=2 LIMIT " + Top + ") AS p INNER JOIN Accounts_UsersExp ue ON ue.UserID=p.CreatedUserID");
            return DbHelperMySQL.Query(strSql.ToString());
        }
        ...
        public DataSet DeleteListEx(string Ids, out int Result)
        {
            MySqlParameter[] parameters = {
					...
					new MySqlParameter("_TargetIds ", MySqlDbType.Int32, 4)};
        ...
        public int GetPrevID(int photoID, int albumId)
        {
            ...
            if (albumId == -1)
            {
                strSql.Append(" and AlbumID=(select AlbumID from SNS_UserAlbumDetail where  Type=0 and TargetID=?TargetID and AlbumUserId=(select CreatedUserID from SNS_Photos where PhotoID=?TargetID) order by ID LIMIT 1) ");
            }
            else
            {
        ...
        public int GetCountEx(int type, int categoryId, string address)
        {
            ...
            {
                strSql.Append(" and SP.PhotoAddress like '%" + Common.InjectionFilter.SqlFilter(address) + "%'");
            }
            object obj = DbHelperMySQL.GetSingle(strSql.ToString());
            if (obj == null)
            {
        ...
        public DataSet GetListByPageEx(int type, int categoryId, string address, string orderby, int startIndex, int endIndex)
        {
            ...
            if (!String.IsNullOrWhiteSpace(address))
            {
                strSql.Append(" and T.PhotoAddress like '%" + Common.InjectionFilter.SqlFilter(address) + "%'");
            }
            strSql.Append(" ) TT");
            strSql.AppendFormat(" WHERE TT.Row between {0} and {1}", startIndex, endIndex);
            return DbHelperMySQL.Query(strSql.ToString());
