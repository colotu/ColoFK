...
namespace YSWL.MALL.MySqlDAL.CMS
{
    ...
    public partial class Content : IContent
    {
        ...
        public int GetMaxId()
        {
            return DbHelperMySQL.GetMaxID("ContentID", "CMS_Content");
        }
        ...
        public int Add(YSWL.MALL.Model.CMS.Content model)
        {
            ...
            MySqlParameter[] parameters = {
					new MySqlParameter("?Title", MySqlDbType.VarString,200),
					new MySqlParameter("?SubTitle", MySqlDbType.VarString,255),
					new MySqlParameter("?Summary", MySqlDbType.LongText),
					...
					new MySqlParameter("?Remary", MySqlDbType.VarChar,200),
					new MySqlParameter("?TotalComment",  MySqlDbType.Int32,4),
					new MySqlParameter("?TotalSupport",  MySqlDbType.Int32,4),
					new MySqlParameter("?TotalFav",  MySqlDbType.Int32,4),
					new MySqlParameter("?TotalShare",  MySqlDbType.Int32,4),
					new MySqlParameter("?BeFrom", MySqlDbType.VarString,50),
					new MySqlParameter("?FileName", MySqlDbType.VarString,200),
					...
					new MySqlParameter("?StaticUrl", MySqlDbType.VarString,500)};
        ...
        public YSWL.MALL.Model.CMS.Content GetModel(int ContentID)
        {
            ...
            strSql.Append(" where ContentID=?ContentID LIMIT 1");
            MySqlParameter[] parameters = {
					new MySqlParameter("?ContentID",  MySqlDbType.Int32,4)
			};
            parameters[0].Value = ContentID;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT * FROM CMS_Content ");
        ...
        public DataSet GetListByPageEx(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            //strSql.Append(" ) TT");
            //strSql.AppendFormat(" WHERE TT.Row between {0} and {1}", startIndex, endIndex);
            strSql.Append("SELECT T.*,UserName as CreatedUserName,ClassName from CMS_Content T  ");
        ...
        public int UpdatePV(int ContentID)
        {
            ...
            if (rows > 0)
            {
                ...
                return Convert.ToInt32(DbHelperMySQL.GetSingle(strSql2.ToString(), parameters2));
            }
            else
            {
                return 0;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT * FROM View_Content ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" WHERE " + strWhere);
            }
        ...
        public DataSet GetListByView(int Top, string strWhere, string filedOrder)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT ");
            strSql.Append(" * FROM View_Content ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" WHERE " + strWhere);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select   CMSS.*,UserName as CreatedUserName from CMS_Content CMSS LEFT JOIN Accounts_Users AS AU ON AU.UserID = CMSS.CreatedUserID ");
            strSql.Append(" where State=0  AND ContentID=?ContentID LIMIT 1");
            MySqlParameter[] parameters = {
					new MySqlParameter("?ContentID",  MySqlDbType.Int32,4)
			};
            parameters[0].Value = ContentID;
        ...
        /// 得到上一个ContentID
        /// </summary>
        public int GetPrevID(int ContentID,int type)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select MAX(ContentID) from CMS_Content ");
            strSql.Append(" where  State=0 AND ContentID<?ContentID and ClassID in (select ClassID from CMS_ContentClass where ClassTypeID=" + type + " )");
            MySqlParameter[] parameters = {
					new MySqlParameter("?ContentID",  MySqlDbType.Int32,4)
			};
            parameters[0].Value = ContentID;
        ...
        /// 得到下一个ContentID
        /// </summary>
        public int GetNextID(int ContentID,int type)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// <summary>
        /// 分页获取数据列表Count(去除固定页面栏目)
        /// </summary>
        public int GetRecordCountEx(int cid,int type)
        {
            ...
            if (cid > 0)
            {
                strSql.Append(" where  State=0 and ClassID=" + cid);
            }
            else
            {
                strSql.Append(" where  State=0 and ClassID in ( select ClassID from CMS_ContentClass where ClassTypeID=" + type + ")");
            }
            object obj = DbHelperMySQL.GetSingle(strSql.ToString());
            if (obj == null)
            {
        ...
        /// <returns></returns>
        public DataSet GetListByPageEx(int cid, string orderby, int startIndex, int endIndex)
        {
            StringBuilder strSql = new StringBuilder();
            //strSql.Append("SELECT * FROM ( ");
            //strSql.Append(" SELECT ROW_NUMBER() OVER (");
            ...
            //else
            //{
            //    strSql.Append(" where  State=0 and ClassID<>1");
            //}
       
            //strSql.Append(" ) TT");
            //strSql.AppendFormat(" WHERE TT.Row between {0} and {1}", startIndex, endIndex);
            strSql.Append("SELECT T.*  from CMS_Content T  ");
            if (cid > 0)
            {
                strSql.Append(" where  State=0 and ClassID=" + cid);
            }
        ...
        public DataSet GetListByPageEx(int cid, string orderby, int startIndex, int endIndex,int type)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT T.*  from CMS_Content T  ");
            if (cid > 0)
        ...
        /// <returns></returns>
        public bool UpdateStaticUrl(int contentId, string staticUrl)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update CMS_Content set ");
            strSql.Append("StaticUrl=?StaticUrl");
            ...
            MySqlParameter[] parameters = {
					new MySqlParameter("?StaticUrl", MySqlDbType.VarString,500),
					new MySqlParameter("?ContentId", MySqlDbType.Int64,8)};
        ...
        public DataSet GetWeChatList(int ClassID, string keyword, int Top)
        {
            ...
            if (ClassID > 0)
            {
                ...
                strSql.AppendFormat(
                   "  AND ( ClassID = {0}  OR Path LIKE CONCAT(( SELECT Path  FROM   CMS_ContentClass  WHERE  ClassID = {0} ),'|%') ) ) ", ClassID);
            }
        ...
        public DataSet GetHotCom(int diffDate, int top)
        {
            ...
            strSql.AppendFormat(" WHERE    DATEDIFF(day, CreatedDate, now()) < {0} ", diffDate);
            strSql.Append("  GROUP BY ContentId  ) TT ON TT.ContentId=C.ContentID ");
            strSql.Append(" WHERE  State=0 and   EXISTS ( SELECT  ContentId , COUNT(1) AS ComCount ");
            strSql.Append(" FROM   CMS_Comment  WHERE  ContentId = C.ContentID ");
            strSql.AppendFormat(" AND DATEDIFF(day, CreatedDate, now()) < {0} ", diffDate);
            strSql.AppendFormat("  GROUP BY ContentId  ORDER BY ComCount DESC LIMIT {0})", top);
            return DbHelperMySQL.Query(strSql.ToString());
        }
