...
namespace YSWL.MALL.MySqlDAL.Ms.Agent
{
	...
	public partial class AgentMenus:IAgentMenus
	{
		...
		public int GetMaxId()
		{
		return DbHelperMySQL.GetMaxID("MenuId", "Ms_AgentMenus"); 
		}
		...
		public YSWL.MALL.Model.Ms.Agent.AgentMenus GetModel(int MenuId)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  MenuId,MenuName,NavURL,MenuTitle,MenuType,Target,IsUsed,Sequence,Visible,URLType,NavTheme,AgentId from Ms_AgentMenus ");
			strSql.Append(" where MenuId=?MenuId");
			MySqlParameter[] parameters = {
					new MySqlParameter("?MenuId", MySqlDbType.Int32,4)
			};
			...
			YSWL.MALL.Model.Ms.Agent.AgentMenus model=new YSWL.MALL.Model.Ms.Agent.AgentMenus();
		...
		/// 得到一个对象实体
		/// </summary>
		public YSWL.MALL.Model.Ms.Agent.AgentMenus DataRowToModel(DataRow row)
		{
			YSWL.MALL.Model.Ms.Agent.AgentMenus model=new YSWL.MALL.Model.Ms.Agent.AgentMenus();
			if (row != null)
			{
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT T.* from Ms_AgentMenus T ");
