...
namespace YSWL.MALL.MySqlDAL.Ms.Agent
{
	...
	public partial class AgentRank:IAgentRank
	{
		...
		public int GetMaxId()
		{
		return DbHelperMySQL.GetMaxID("RankId", "Ms_AgentRank"); 
		}
		...
		public YSWL.MALL.Model.Ms.Agent.AgentRank GetModel(int RankId)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  RankId,Name,ProductCount,ImageCount,Price,IsDefault,IsApproval,Description,RankMin,RankMax from Ms_AgentRank ");
			strSql.Append(" where RankId=?RankId");
			MySqlParameter[] parameters = {
					new MySqlParameter("?RankId", MySqlDbType.Int32,4)
			};
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT T.* from Ms_AgentRank T ");
