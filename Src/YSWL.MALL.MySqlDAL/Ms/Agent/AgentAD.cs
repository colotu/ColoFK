...
namespace YSWL.MALL.MySqlDAL.Ms.Agent
{
	...
	public partial class AgentAD:IAgentAD
	{
		...
		public int GetMaxId()
		{
		return DbHelperMySQL.GetMaxID("AdvertisementId", "Ms_AgentAD"); 
		}
		...
		public YSWL.MALL.Model.Ms.Agent.AgentAD GetModel(int AdvertisementId)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  AdvertisementId,Name,PositionId,ContentType,FileUrl,AlternateText,NavigateUrl,AdvHtml,CreatedDate,CreatedUserID,Status,Sequence,AgentId from Ms_AgentAD ");
			strSql.Append(" where AdvertisementId=?AdvertisementId");
			MySqlParameter[] parameters = {
					new MySqlParameter("?AdvertisementId", MySqlDbType.Int32,4)
			};
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT T.* from Ms_AgentAD T ");
