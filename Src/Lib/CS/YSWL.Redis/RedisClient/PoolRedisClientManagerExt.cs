    ...
    public class PoolRedisClientManagerExt : IDisposable
    {
            ...
            foreach (var item in this.writeReadClients)
            {
                ...
                if (clientTemps == null || !clientTemps.Any())
                {
                    if (count < this.Config.MaxWritePoolSize)
                    {
                        if (ips[0].Contains("@"))
                        {
                            string[] host = ips[0].Split(new char[] { '@' }, StringSplitOptions.RemoveEmptyEntries);
                            client = this.RedisClientFactory.CreateRedisClient(host[1], int.Parse(ips[1]));
                            client.Password = host[0];
                        }
                        else
                        {
                ...
                if (client.HadExceptions)
                {
                    ...
                    RedisClientExt client2;
                    if (ips[0].Contains("@"))
                    {
                        string[] host = ips[0].Split(new char[] { '@' }, StringSplitOptions.RemoveEmptyEntries);
                        client2 = this.RedisClientFactory.CreateRedisClient(host[1], int.Parse(ips[1]));
                        client2.Password = host[0];
                    }
                    else
                    {
                        client2 = this.RedisClientFactory.CreateRedisClient(ips[0], int.Parse(ips[1]));
                    }
            ...
            lock (this.writeReadClients)
            {
                ...
                if (this.writeReadClients == null || !this.writeReadClients.Any())
                {
                    foreach (var point in this.ReadOnlyHosts)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(point.Password)) client.Password = point.Password;
                        client.Id = this.RedisClientCounter++;
                        client.ClientManager = this;
                        bool isAdd = false;
                        try
                        ...
                        if (isAdd)
                        {
                            ...
                            if (this.writeReadClients.All(k => k.Key != (point.Host + ":" + point.Port)))
                            {
        ...
        public void Start()
        {
            ...
            foreach (var point in this.ReadOnlyHosts)
            {
                ...
                if (this.readClients.All(k => k.Key != (point.Host + ":" + point.Port)))
                {
