...
namespace YSWL.WeChat.BLL.Core
{
    ...
    public partial class PostMsg
    {
        ...
        public static string GetPostMsgXML(YSWL.WeChat.Model.Core.PostMsg msgModel)
        {
            ...
            string bodyXml = "";
            if (!YSWL.Components.MvcApplication.IsAuthorize)
            {
                msgModel.Description = String.IsNullOrWhiteSpace(msgModel.Description) ? "[技术支持: 云商未来]" : (msgModel.Description.Contains("[技术支持: 云商未来]") ? msgModel.Description : msgModel.Description + "\n[技术支持: 云商未来]");
            }
                ...
                case "news":
                    ...
                    if (msgModel.MsgItems.Count > 0)
                    {
                        foreach (var item in msgModel.MsgItems)
                        {
                            if (!YSWL.Components.MvcApplication.IsAuthorize)
                            {
                                item.Title = String.IsNullOrWhiteSpace(item.Title) ? "[技术支持: 云商未来]" : (item.Title.Contains("[技术支持: 云商未来]") ? item.Title : item.Title + "\n[技术支持: 云商未来]");
                            }
                            bodyXml = bodyXml +
