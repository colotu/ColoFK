	...
	public partial class Menu
	{
	    ...
	    public static YSWL.WeChat.Model.Core.JsonMenu  GetJsonMenu(string openId,bool IsAuto)
	    {
           ...
           //顶级菜单
	        ...
	        string enterpiserStr = "";
            if (MvcApplication.IsAutoConn)
	        {
	            long enterpiserId =Common.Globals.SafeLong(YSWL.Common.CallContextHelper.GetAutoTag(),0) ;
	            if (enterpiserId > 0)
	            {
                    returnUrl = "http://" + Common.Globals.DomainFullName + "/wcreturn.aspx?tag="+ enterpiserId;
	                enterpiserStr = Common.DEncrypt.DEncrypt.GetEncryptionStr(enterpiserId);
	            }
            }
            #endregion
            ...
            foreach (var item in MainMenu)
            {
                ...
                if (item.HasChildren)
	            {
                    ...
                    if (childList != null && childList.Count > 0)
	                {
	                    ...
	                    {
	                        if (!IsAuto && MvcApplication.IsAutoConn)
	                        {
	                            child.MenuUrl = child.MenuUrl + "?tag=" + enterpiserStr; 
	                        }
	                        switch (child.Type)
	                        {
                                case "click":
                ...
                else
	            {
                    if (!IsAuto && MvcApplication.IsAutoConn)
                    {
                        item.MenuUrl = item.MenuUrl + "?tag=" + enterpiserStr;
                    }
                    switch (item.Type)
                        {
                            case "click":
