...
namespace YSWL.MALL.BLL.SysManage
{
    ...
    public class ConfigSystem
    {
        ...
        #region  ERP 和WMS 数据切换
        /// <summary>
        /// 获取数据流向
        /// </summary>
        /// <returns></returns>
        public static string GetDataConn()
        {
            string BaseKey = "OMS_ConnectionData";
            string objModel = dataCache.GetCache<String>(BaseKey);
            if (String.IsNullOrWhiteSpace(objModel))
            {
                try
                {
                    int enterpriseId = Common.Globals.SafeInt(CallContextHelper.GetEnterpriseId(), 0);

                    bool isAuto = Common.ConfigHelper.GetConfigBool("AutoConnection");
                    if (isAuto)
                    {
                        #region SAAS平台
                        objModel = GetValue(BaseKey);
                        #region 兼容老用户开通的ERP 或者WMS
                        if (String.IsNullOrWhiteSpace(objModel))
                        {
                            if (YSWL.SAAS.BLL.SAASInfo.AppIsOpen("ERP",
                              enterpriseId))
                            {
                                SetDataConn("ERP");
                                objModel = "ERP";
                            ...
                            if (YSWL.SAAS.BLL.SAASInfo.AppIsOpen("WMS",
                               Common.Globals.SafeInt(CallContextHelper.GetEnterpriseId(), 0)))
                            {
                                SetDataConn("WMS");
                                objModel = "WMS";
                            }
                    ...
                    {
                        #region  私有云部署
                        objModel = Common.ConfigHelper.GetConfigString(BaseKey);
                        if (String.IsNullOrWhiteSpace(objModel))
                        {
                            objModel = "WMS";
                        }
                    ...
                    if (!String.IsNullOrWhiteSpace(objModel))
                    {
                        dataCache.SetCache(BaseKey, objModel, DateTime.MaxValue, TimeSpan.Zero);
                    }
                }
                catch (Exception ex)
                {
        ...
        /// <returns></returns>
        public static bool SetDataConn(string tag)
        {
            string BaseKey = "OMS_ConnectionData";
            bool isSucccess = Modify(BaseKey, tag, "订单处理流向，是ERP系统还是WMS系统处理订单");
            if (isSucccess)//清空缓存
            {
                dataCache.DeleteCache(BaseKey);
            }
            return isSucccess;
        }
