...
namespace YSWL.MALL.BLL.SysManage
{
    ...
    public class SysTree
    {
        ...
        public List<YSWL.MALL.Model.SysManage.SysNode> GetTreeListByTypeCache(int treeType, bool Enabled, bool IsAutoConn)
        {
            //根据配置加载对应的菜单
            bool IsXMLTree = YSWL.MALL.BLL.SysManage.ConfigSystem.GetBoolValueByCache("Shop_Menu_IsXML");
            if (IsXMLTree)
            {
                List<YSWL.MALL.Model.SysManage.SysNode> allSysNodes = GetAllTreeListXmlCache(treeType);
                ...
                {
                    //获取所有的开通应用列表
                    List<long> applist = YSWL.SAAS.BLL.SAASInfo.GetOpenApps(Common.CallContextHelper.GetEnterpriseId());
                    foreach (var item in allSysNodes.ToArray())
                    {
                        if (!IsAppRelation(applist, item.AppStr))
                        {
                            allSysNodes.Remove(item);
                        }
                    }
                }
