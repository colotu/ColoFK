...
namespace YSWL.MALL.BLL.CMS
{
    ...
    public partial class ContentClass
    {
        ...
        public List<YSWL.MALL.Model.CMS.ContentClass> GetModelList(int classid,out Model.CMS.ContentClass classmodel)
        {
            ...
            List<YSWL.MALL.Model.CMS.ContentClass> list = GetModelList(0,string.Format(" ParentId={0}", AClassId), " Sequence ");//子栏目列表
            return list;
        }
      ...
      public List<Model.CMS.ContentClass> GetModelList(int Top,int? classid,out string classname)
      {
        ...
        /// 获得数据列表     
        /// </summary>
        public List<YSWL.MALL.Model.CMS.ContentClass> GetModelList(int Top,int classid, out Model.CMS.ContentClass classmodel)
        {
            int AClassId = GetClassIdById(classid);//根栏目ID
            classmodel = GetModelByCache(AClassId);//根栏目Model
            List<YSWL.MALL.Model.CMS.ContentClass> list = GetModelList(Top,string.Format(" ParentId={0}", AClassId)," Sequence ");//子栏目列表
            return list;
        }
