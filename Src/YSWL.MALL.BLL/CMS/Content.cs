...
namespace YSWL.MALL.BLL.CMS
{
    ...
    public partial class Content
    {
                                                            ...
                                                            int Top = -1,bool hasImage=false)
        {
            ...
            if (ClassId > 0)
            {
                BLL.CMS.ContentClass classBll = new BLL.CMS.ContentClass();
                if (classBll.GetRecordCount(string.Format(" State=0  and  ParentId={0}  ", ClassId)) > 0) //存在子栏目
                {
                    strWhere.AppendFormat(
                  " and EXISTS ( SELECT ClassID   FROM   CMS_ContentClass AS contclass WHERE  State=0  and  ParentId={0} AND contclass.ClassID=cont.ClassID )",
                ClassId);
                }
                else//不存在子栏目
                {
            ...
            DataSet ds = dal.GetListEx(Top, strWhere.ToString(), " Sequence");
