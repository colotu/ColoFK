...
namespace YSWL.MALL.BLL.SNS
{
    ...
    /// 参数配置类
    /// </summary>
    public class ConfigSystem : YSWL.MALL.BLL.SysManage.ConfigSystem
    {
        public static YSWL.MALL.Model.SNS.PostsSet GetPostSetByCache()
        {
            ...
            if (objModel == null)
            {
                try
                {
                    YSWL.MALL.Model.SNS.PostsSet model = new Model.SNS.PostsSet();
                    ...
                    model.TaoProduct = PostSetHelper("SNS_TaoProduct_IsShow");
                    model._Blog = PostSetHelper("SNS_Blog_IsShow");
                    objModel = model;
                    if (objModel != null)
                    {
            ...
            return (YSWL.MALL.Model.SNS.PostsSet)objModel;
        }

        public static bool PostSetHelper(string name)
        {
            string Temp = GetValue(name);
            return (Temp == "1" || Common.Globals.SafeBool(Temp, false));
        }

        public static void UpdatePostSet(YSWL.MALL.Model.SNS.PostsSet model)
        {
            ...
            if (objModel != null)
            {
                YSWL.Common.DataCache.DeleteCache("ConfigSystemPostSetting");
            }
            objModel = model;
            int CacheTime = Common.Globals.SafeInt(GetValue("CacheTime"), 30);
            YSWL.Common.DataCache.SetCache(CacheKey, objModel, DateTime.Now.AddMinutes(CacheTime), TimeSpan.Zero);
