...
namespace YSWL.MALL.BLL.SNS
{
    ...
    public partial class GroupTopics
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(YSWL.MALL.Model.SNS.GroupTopics model)
        {
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(YSWL.MALL.Model.SNS.GroupTopics model)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public YSWL.MALL.Model.SNS.GroupTopics GetModel(int TopicID)
        {
        ...
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public YSWL.MALL.Model.SNS.GroupTopics GetModelByCache(int TopicID)
        {
            ...
            return (YSWL.MALL.Model.SNS.GroupTopics)objModel;
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<YSWL.MALL.Model.SNS.GroupTopics> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<YSWL.MALL.Model.SNS.GroupTopics> DataTableToList(DataTable dt)
        {
            List<YSWL.MALL.Model.SNS.GroupTopics> modelList = new List<YSWL.MALL.Model.SNS.GroupTopics>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                YSWL.MALL.Model.SNS.GroupTopics model;
        ...
        /// 获得前几行数据
        /// </summary>
        public List<YSWL.MALL.Model.SNS.GroupTopics> GetList4Model(int top, string strWhere, string filedOrder)
        {
        ...
        {
            string strWhere = string.Format(" Title like '%{0}%'", Common.InjectionFilter.SqlFilter(Keywords));
            string filedOrder = " TopicID DESC";
            return dal.GetListEx(0, strWhere, filedOrder);
        }
        #region 增加一条话题
        public int AddEx(YSWL.MALL.Model.SNS.GroupTopics TModel, long Pid)
        {
            ...
            {
                TModel.Description = YSWL.MALL.BLL.Settings.FilterWords.ReplaceWords(TModel.Description, out isFilter);
            }
            if (isFilter)
            {
                //达到上限禁用用户
            ...
            #region 获取小组的名称
            YSWL.MALL.BLL.SNS.Groups Gbll = new YSWL.MALL.BLL.SNS.Groups();
            YSWL.MALL.Model.SNS.Groups Gmodel = new YSWL.MALL.Model.SNS.Groups();
        ...
        /// 获得数据列表
        /// </summary>
        public List<YSWL.MALL.Model.SNS.GroupTopics> GetTopicByUserId(int UserId, int top)
        {
            List<YSWL.MALL.Model.SNS.GroupTopics> list = new List<Model.SNS.GroupTopics>();
            return DataTableToList(GetListByPage("CreatedUserID=" + UserId + "", "ReplyCount desc", 0, top).Tables[0]);
        }
        #endregion

        #region 分页得到数据
        public List<YSWL.MALL.Model.SNS.GroupTopics> GetTopicListPageByGroup(int GroupId, int StartIndex, int EndIndex, bool IsRecommand)
        {
            List<YSWL.MALL.Model.SNS.GroupTopics> list = new List<Model.SNS.GroupTopics>();
            ...
            list = DataTableToList(GetListByPage(sql, "IsTop Desc,CreatedDate Desc", StartIndex, EndIndex).Tables[0]);
            return list;
        }
        #endregion
        ...
        #region 得到最新的帖子
        public List<YSWL.MALL.Model.SNS.GroupTopics> GetNewTopListByGroup(int GroupId, int top)
        {
        ...
        #region 得到最热帖子
        public List<YSWL.MALL.Model.SNS.GroupTopics> GetHotListByGroup(int GroupId, int top)
        {
        ...
        public List<YSWL.MALL.Model.SNS.GroupTopics> GetUserRelativeTopicByType(int UserId, YSWL.MALL.Model.SNS.EnumHelper.UserGroupType Type, int StartIndex, int EndIndex)
        {
            List<YSWL.MALL.Model.SNS.GroupTopics> List = new List<Model.SNS.GroupTopics>();
        ...
        public string GetWhereByType(int UserId, YSWL.MALL.Model.SNS.EnumHelper.UserGroupType Type)
        {
            string Sql = "GroupID in(select GroupID from SNS_GroupUsers where UserID=" + UserId + ")";
            switch (Type)
            {
                case YSWL.MALL.Model.SNS.EnumHelper.UserGroupType.UserGroup:
                    Sql = "GroupID in(select GroupID from SNS_GroupUsers where UserID=" + UserId + ")";
                    break;
                case YSWL.MALL.Model.SNS.EnumHelper.UserGroupType.UserFav:
                    Sql = "TopicID in(select TopicID from SNS_GroupTopicFav where CreatedUserID=" + UserId + ")";
                    break;
                case YSWL.MALL.Model.SNS.EnumHelper.UserGroupType.UserPostTopic:
                    Sql = "CreatedUserID=" + UserId + "";
                    break;
                case YSWL.MALL.Model.SNS.EnumHelper.UserGroupType.UserReply:
        ...
        #region 相应类型的数量
        public int GetCountByType(int UserId, YSWL.MALL.Model.SNS.EnumHelper.UserGroupType Type)
        {
            string Sql = GetWhereByType(UserId, Type);
        ...
        public List<YSWL.MALL.Model.SNS.GroupTopics> SearchTopicByKeyWord(int StartIndex, int EndIndex, string q, int GroupId, string orderby)
        {
            switch (orderby)
            {
                case "newreply":
            ...
            List<YSWL.MALL.Model.SNS.GroupTopics> list = new List<Model.SNS.GroupTopics>();
            string sql = " Status=1 and (Title like '%" + Common.InjectionFilter.SqlFilter(q) + "%' or Description like '%" + Common.InjectionFilter.SqlFilter(q) + "%') ";
            if (GroupId > 0)
            {
        ...
        public List<YSWL.MALL.Model.SNS.GroupTopics> GetRecTopics(int Top = -1)
        {
        ...
        public bool DeleteTopic(int topicId)
        {
            ...
            return dal.DeleteGroupTopic(topicId);
        }
        ...
        public static bool BanUserCheck(int userId)
        {
            ...
            {
                YSWL.Accounts.Bus.User userManage = new YSWL.Accounts.Bus.User();
...
/// <returns></returns>
        public List<YSWL.MALL.Model.SNS.GroupTopics> GetPageList(string strWhere, string orderby, int startIndex, int endIndex)
        {
