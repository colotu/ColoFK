...
namespace YSWL.MALL.BLL.SNS
{
    ...
    public partial class UserAlbumDetail
    {
        ...
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public YSWL.MALL.Model.SNS.UserAlbumDetail GetModelByCache(int ID)
        {
            ...
            return (YSWL.MALL.Model.SNS.UserAlbumDetail)objModel;
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<YSWL.MALL.Model.SNS.UserAlbumDetail> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<YSWL.MALL.Model.SNS.UserAlbumDetail> DataTableToList(DataTable dt)
        {
            List<YSWL.MALL.Model.SNS.UserAlbumDetail> modelList = new List<YSWL.MALL.Model.SNS.UserAlbumDetail>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                YSWL.MALL.Model.SNS.UserAlbumDetail model;
        ...
        public List<PostContent> GetAlbumImgListByPage(int albumID, int startIndex, int endIndex,int type=-1)
        {
            ...
            if (ds != null && ds.Tables.Count > 0)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    model.ThumbImageUrl = dr["ThumbImageUrl"].ToString();
                    model.Price = YSWL.Common.Globals.SafeDecimal(dr["Price"].ToString(),-1);
                ...
                if (commentList != null)
                {
                    ...
                    imgList.ForEach(img =>
                    {
                        List<Model.SNS.Comments> List = commentList.FindAll(xx => (xx.TargetId == img.TargetId && xx.Type == (img.Type == (int)YSWL.MALL.Model.SNS.EnumHelper.ImageType.Photo ? (int)YSWL.MALL.Model.SNS.EnumHelper.CommentType.Photo : (int)YSWL.MALL.Model.SNS.EnumHelper.CommentType.Product)));
                        if (List == null) return;
