...
namespace YSWL.MALL.BLL.SNS
{
    ...
    public partial class GradeConfig
    {
        ...
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public YSWL.MALL.Model.SNS.GradeConfig GetModelByCache(int GradeID)
        {
            ...
            return (YSWL.MALL.Model.SNS.GradeConfig)objModel;
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<YSWL.MALL.Model.SNS.GradeConfig> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<YSWL.MALL.Model.SNS.GradeConfig> DataTableToList(DataTable dt)
        {
            List<YSWL.MALL.Model.SNS.GradeConfig> modelList = new List<YSWL.MALL.Model.SNS.GradeConfig>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                YSWL.MALL.Model.SNS.GradeConfig model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new YSWL.MALL.Model.SNS.GradeConfig();
        ...
        public string GetUserLevel(int? grades)
        {
            if (grades.HasValue)
            {
                YSWL.MALL.Model.SNS.GradeConfig model = dal.GetUserLevel(grades);
                if (model != null)
                {
