...
namespace YSWL.MALL.BLL.SNS
{
	...
	public partial class UserFavourite
	{
		...
		/// 增加一条数据
		/// </summary>
		public int  Add(YSWL.MALL.Model.SNS.UserFavourite model)
		{
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(YSWL.MALL.Model.SNS.UserFavourite model)
		{
		...
		/// 得到一个对象实体
		/// </summary>
		public YSWL.MALL.Model.SNS.UserFavourite GetModel(int FavouriteID)
		{
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public YSWL.MALL.Model.SNS.UserFavourite GetModelByCache(int FavouriteID)
		{
			...
			return (YSWL.MALL.Model.SNS.UserFavourite)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<YSWL.MALL.Model.SNS.UserFavourite> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<YSWL.MALL.Model.SNS.UserFavourite> DataTableToList(DataTable dt)
		{
			List<YSWL.MALL.Model.SNS.UserFavourite> modelList = new List<YSWL.MALL.Model.SNS.UserFavourite>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				YSWL.MALL.Model.SNS.UserFavourite model;
        ...
        public bool AddEx(YSWL.MALL.Model.SNS.UserFavourite FavModel,int TopicId, int ReplyId)
        {
        ...
        public List<PostContent> GetFavListByPage(int UserId, string orderby, int startIndex, int endIndex)
        {
            ...
            List<YSWL.MALL.Model.SNS.Comments> comList = new List<Model.SNS.Comments>();
            ...
            if (ds != null && ds.Tables.Count > 0)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    model.Type =((int)dr["Type"]==0?(int)YSWL.MALL.Model.SNS.EnumHelper.FavoriteType.Photo:(int)YSWL.MALL.Model.SNS.EnumHelper.FavoriteType.Product);
                ...
                if (commentList != null)
                {
                    ...
                    {
                        List<Model.SNS.Comments> List = commentList.FindAll(xx => (xx.TargetId == img.TargetId && xx.Type ==(img.Type==(int)YSWL.MALL.Model.SNS.EnumHelper.FavoriteType.Photo ? (int)YSWL.MALL.Model.SNS.EnumHelper.CommentType.Photo : (int)YSWL.MALL.Model.SNS.EnumHelper.CommentType.Product)));
                        if (List == null) return;
        ...
        /// <param name="Type"></param>
        /// <returns></returns>
        public List<YSWL.MALL.Model.SNS.UserFavourite> GetFavUserByTargetId(int TargetId, int Type,int Top)
        {
