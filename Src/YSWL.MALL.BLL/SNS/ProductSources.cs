...
namespace YSWL.MALL.BLL.SNS
{
	...
	public partial class ProductSources
	{
		...
		{
			return dal.Exists(WebSiteName, WebSiteUrl);
		}
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public YSWL.MALL.Model.SNS.ProductSources GetModelByCache(int ID)
		{
			...
			return (YSWL.MALL.Model.SNS.ProductSources)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<YSWL.MALL.Model.SNS.ProductSources> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<YSWL.MALL.Model.SNS.ProductSources> DataTableToList(DataTable dt)
		{
			List<YSWL.MALL.Model.SNS.ProductSources> modelList = new List<YSWL.MALL.Model.SNS.ProductSources>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				YSWL.MALL.Model.SNS.ProductSources model;
        ...
        public  int GetIdByWebSiteName(string WebSiteName)
        {
          ...
          List<YSWL.MALL.Model.SNS.ProductSources> list=GetModelList("WebSiteName='" + Common.InjectionFilter.SqlFilter(WebSiteName) + "'");
          if (list.Count > 0)
          {
              return list[0].ID;
