...
namespace YSWL.MALL.BLL.SNS
{
    ...
    public partial class Posts
    {
        ...
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public YSWL.MALL.Model.SNS.Posts GetModelByCache(int PostID)
        {
            ...
            return (YSWL.MALL.Model.SNS.Posts)objModel;
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<YSWL.MALL.Model.SNS.Posts> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<YSWL.MALL.Model.SNS.Posts> DataTableToList(DataTable dt)
        {
            List<YSWL.MALL.Model.SNS.Posts> modelList = new List<YSWL.MALL.Model.SNS.Posts>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                YSWL.MALL.Model.SNS.Posts model;
        ...
        /// <returns></returns>
        public List<ViewModel.SNS.Posts> GetPostByType(int UserId, int StartIndex, int EndIndex, YSWL.MALL.Model.SNS.EnumHelper.PostType Type, int PostId, string basePath, bool includeProduct = true)
        {
            List<YSWL.MALL.Model.SNS.Posts> Posts = new List<Model.SNS.Posts>();
            List<YSWL.MALL.Model.SNS.Posts> OrigPosts = new List<Model.SNS.Posts>();
            Dictionary<int, YSWL.MALL.Model.SNS.Posts> OrigPostsDic = new Dictionary<int, Model.SNS.Posts>();
            ...
            switch (Type)
            {
                case EnumHelper.PostType.All:
                    QueryCondition = " Status=" + (int)YSWL.MALL.Model.SNS.EnumHelper.PostStatus.AlreadyChecked + "";
                    break;
                case EnumHelper.PostType.Fellow:
                    QueryCondition = " Status=" + (int)YSWL.MALL.Model.SNS.EnumHelper.PostStatus.AlreadyChecked + " and CreatedUserId in (SELECT  PassiveUserID  FROM  SNS_UserShip WHERE  ActiveUserID=" + UserId + " UNION SELECT " + UserId + ")";
                    break;
                case EnumHelper.PostType.User:
                    QueryCondition = " Status=" + (int)YSWL.MALL.Model.SNS.EnumHelper.PostStatus.AlreadyChecked + " and CreatedUserId=" + UserId + "";
                    break;
                case EnumHelper.PostType.OnePost:
                    QueryCondition = " PostID=" + PostId + "";
                    break;
                case EnumHelper.PostType.ReferMe:
                    QueryCondition = " Status=" + (int)YSWL.MALL.Model.SNS.EnumHelper.PostStatus.AlreadyChecked + " and PostID in (SELECT  TagetID FROM SNS_ReferUsers  WHERE  ReferUserID=" + UserId + " and Type=0)";
                    break;
                case EnumHelper.PostType.EachOther:
                    QueryCondition = " Status=" + (int)YSWL.MALL.Model.SNS.EnumHelper.PostStatus.AlreadyChecked + " and CreatedUserId in ( SELECT PassiveUserID  FROM  SNS_UserShip WHERE  ActiveUserID=" + UserId + " and Type=1)";
                    break;
                case EnumHelper.PostType.Photo:
                    QueryCondition = " Status=" + (int)YSWL.MALL.Model.SNS.EnumHelper.PostStatus.AlreadyChecked + " and CreatedUserId=" + UserId + " and Type=" + (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Photo + "";
                    break;
                case EnumHelper.PostType.Product:
                    QueryCondition = " Status=" + (int)YSWL.MALL.Model.SNS.EnumHelper.PostStatus.AlreadyChecked + " and CreatedUserId=" + UserId + " and Type=" + (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Product + "";
                    break;
                case EnumHelper.PostType.Video:
                    QueryCondition = " Status=" + (int)YSWL.MALL.Model.SNS.EnumHelper.PostStatus.AlreadyChecked + " and CreatedUserId=" + UserId + " and VideoUrl is not null";
                    break;
                case EnumHelper.PostType.Blog:
                    QueryCondition = " Status=" + (int)YSWL.MALL.Model.SNS.EnumHelper.PostStatus.AlreadyChecked + " and CreatedUserId=" + UserId + " and Type=" + (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Blog;
                    break;
                default:
                    QueryCondition = "Status=" + (int)YSWL.MALL.Model.SNS.EnumHelper.PostStatus.AlreadyChecked + "";
                    break;
            ...
            if (!includeProduct)
            {
                QueryCondition += " and Type<>" + (int) YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Product;
            }
            ...
            //放到一个键值对当中（取值好取）
            foreach (YSWL.MALL.Model.SNS.Posts Post in OrigPosts)
            {
                OrigPostsDic.Add(Post.PostID, Post);
            }
            ///循环得到动态和被转发的动态
            foreach (YSWL.MALL.Model.SNS.Posts Post in Posts)
            {
                ViewModel.SNS.Posts Po = new ViewModel.SNS.Posts();
                Post.Description = ViewModel.ViewModelBase.RegexNickName(Post.Description, basePath);
                Po.Post = Post;
                if (OrigPostsDic.ContainsKey(Post.OriginalID))
                {
                    Po.OrigPost = OrigPostsDic[Post.OriginalID];
                    Po.OrigPost.Description = ViewModel.ViewModelBase.RegexNickName(Po.OrigPost.Description, basePath);
                }
                PostResult.Add(Po);
            }
            if (Type == EnumHelper.PostType.ReferMe)
            {
                YSWL.MALL.BLL.SNS.ReferUsers ReferBll = new ReferUsers();
                ReferBll.UpdateReferStateToRead(UserId, (int)YSWL.MALL.Model.SNS.EnumHelper.ReferType.Post);
            }
            return PostResult;
        }
        /// <summary>
        ...
        /// <returns></returns>
        public List<ViewModel.SNS.Posts> GetPostByType(int UserId, int StartIndex, int EndIndex, YSWL.MALL.Model.SNS.EnumHelper.PostType Type,string basePath, bool  includeProduct=true)
        {
            return GetPostByType(UserId, StartIndex, EndIndex, Type, 0,basePath ,includeProduct);
        }
        /// <summary> 
        /// 获取一条转发动态的的数据（重构）
        /// </summary>
        ...
        /// <returns></returns>
        public List<ViewModel.SNS.Posts> GetForPostByPostId(int PostId,bool includeProduct,string basePath)
        {
            return GetPostByType(0, 0, 1, YSWL.MALL.Model.SNS.EnumHelper.PostType.OnePost, PostId, basePath, includeProduct);
        }
        ...
        /// <returns></returns>
        public int GetCountByPostType(int UserId, YSWL.MALL.Model.SNS.EnumHelper.PostType PostType, bool includeProduct)
        {
            string QueryCondition = "";
            int Count;
            switch (PostType)
        ...
        public int PostForWard(string PostContent, int Origid, int ForWardid, int OrigUserId, string OrigNickName, int CurrentUserID, string CurrentNickName, string UserIp)
        {
            ...
            YSWL.MALL.Model.SNS.Posts PostModel = new Model.SNS.Posts();
            ...
            PostModel.Status = (int)YSWL.MALL.Model.SNS.EnumHelper.Status.Enabled;
            ...
            //    }
            //} 
            ReferBll.AddEx(PostContent, (int)YSWL.MALL.Model.SNS.EnumHelper.ReferType.Post, PostID);
            #endregion
            //下面是给@原动态作者的通知记录
            ReferModel.CreatedDate = DateTime.Now;
            ReferModel.IsRead = false;
        ...
        public int AddForwardPost(YSWL.MALL.Model.SNS.Posts model)
        {
            ...
            {
                model.Description = YSWL.MALL.BLL.Settings.FilterWords.ReplaceWords(model.Description);
            }
            PostID = dal.AddForwardPost(model);
            ///如果内容中有提到某人则进行记录
          
            referBll.AddEx(model.Description, (int)YSWL.MALL.Model.SNS.EnumHelper.ReferType.Post, PostID, model.CreatedNickName);
            return PostID;

        }

        public YSWL.MALL.Model.SNS.Posts AddPost(YSWL.MALL.Model.SNS.Posts Post, int AblumId, long Pid, int PhotoCateId, string PhotoAddress = "", string MapLng = "", string MapLat = "", bool CreatePost = true)
        {

            YSWL.MALL.Model.SNS.Products PModel = new Model.SNS.Products();
            YSWL.MALL.BLL.SNS.Tags tagBll = new Tags();
            if (Post.Type == (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Product)
            {
                //AblumDetailType = (int)YSWL.MALL.Model.SNS.EnumHelper.ImageType.Product;
                ITopClient client = BLL.SNS.TaoBaoConfig.GetTopClient();
                YSWL.MALL.BLL.SNS.CategorySource CateBll = new YSWL.MALL.BLL.SNS.CategorySource();
                YSWL.TaoBao.Domain.Item Item = new YSWL.TaoBao.Domain.Item();
                ...
                PModel.ProductSourceID = (int)YSWL.MALL.Model.SNS.EnumHelper.WebSiteType.TaoBao;
                ...
                //进行敏感字过滤
                if (YSWL.MALL.BLL.Settings.FilterWords.ContainsModWords(Post.Description))
                {
                ...
                {
                    Post.Description = YSWL.MALL.BLL.Settings.FilterWords.ReplaceWords(Post.Description);
                }
            ...
            else if (Post.Type == (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Normal)
            {
                string Status;
                ...
                {
                    Post.Type = (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Video;
            ...
            else if (Post.Type == (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Photo)
            {
                string Status;
                if (!string.IsNullOrEmpty(Post.ImageUrl))
                {
            ...
            YSWL.MALL.Model.SNS.Posts Result = dal.AddPost(Post, AblumId, Pid, PhotoCateId, PModel, RecommandStateInt, PhotoAddress, MapLng, MapLat, CreatePost);
            #region @某人的实现
            SNS.ReferUsers ReferBll = new ReferUsers();
            if (Result != null)
            {
                ReferBll.AddEx(Post.Description, YSWL.MALL.Model.SNS.EnumHelper.ReferType.Post, Result.PostID);
            }
            #endregion
            return Result;
        }
        ...
        //添加博客 动态
        public YSWL.MALL.Model.SNS.Posts AddBlogPost(YSWL.MALL.Model.SNS.Posts Post, YSWL.MALL.Model.SNS.UserBlog blogModel, bool CreatePost = true)
        {
            Post.Type = (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Blog;
            ...
            YSWL.MALL.Model.SNS.Posts Result = dal.AddBlogPost(Post, blogModel, CreatePost);
            #region @某人的实现
            SNS.ReferUsers ReferBll = new ReferUsers();
            if (Result != null)
            {
        ...
        //添加一般动态
        public YSWL.MALL.Model.SNS.Posts AddNormalPost(YSWL.MALL.Model.SNS.Posts Post)
        {
            Post.Type = (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Normal;
            string Status = BLL.SysManage.ConfigSystem.GetValueByCache("chk_check_word");
            if (!string.IsNullOrEmpty(Status))
            {
                Post.Status = Status == "0" ? (int)Model.SNS.EnumHelper.PostStatus.AlreadyChecked : (int)Model.SNS.EnumHelper.PostStatus.UnChecked;
            ...
            Post.PostID = postId;
            YSWL.MALL.Model.SNS.Posts Result = Post;
            #region @某人的实现
            SNS.ReferUsers ReferBll = new ReferUsers();
            if (Result != null)
            {
       ...
       //添加商品动态
        ...
        public YSWL.MALL.Model.SNS.Posts AddProductPost(YSWL.MALL.Model.SNS.Products PModel, int AblumId, bool CreatePost=true)
        {
           
                PModel.ProductSourceID = (int)YSWL.MALL.Model.SNS.EnumHelper.WebSiteType.None;
                PModel.CreatedDate = DateTime.Now;
                #region 进行默认的审核处理
                //管理员可以设置上传的商品默认为已审核状态，如果未定义，则为未审核
                string Status = BLL.SysManage.ConfigSystem.GetValueByCache("SNS_check_product");
                ...
                //进行敏感字过滤
                if (YSWL.MALL.BLL.Settings.FilterWords.ContainsModWords(PModel.ShareDescription))
                {
                    PModel.Status = (int)Model.SNS.EnumHelper.ProductStatus.UnChecked;
                }
                else
                {
                    PModel.ShareDescription = YSWL.MALL.BLL.Settings.FilterWords.ReplaceWords(PModel.ShareDescription);
                }
            ...
            YSWL.MALL.Model.SNS.Posts Result = dal.AddProductPost(PModel, AblumId,CreatePost);
            #region @某人的实现
            SNS.ReferUsers ReferBll = new ReferUsers();
            if (Result != null)
            {
                ReferBll.AddEx(Result.Description, YSWL.MALL.Model.SNS.EnumHelper.ReferType.Post, Result.PostID);
            }
            #endregion
            return Result;
        }
        ...
        /// <returns></returns>
        public List<YSWL.MALL.Model.SNS.Posts> GetTopPost(int top, int Type)
        {
        ...
        /// <param name="top"></param>
        /// <returns></returns>
        public List<YSWL.MALL.Model.SNS.Posts> GetScrollPost(int top, YSWL.MALL.Model.SNS.EnumHelper.PostContentType PostType)
        {
            DataSet ds=new DataSet();
            if (PostType == YSWL.MALL.Model.SNS.EnumHelper.PostContentType.None)
            {
        ...
        {
            YSWL.MALL.Model.SNS.Posts post = GetModelByCache(PostID);
            bool isSuccess= dal.DeleteEx(PostID);
            if (isSuccess && IsSendMess)
            {
                YSWL.MALL.BLL.Members.SiteMessage siteBll = new SiteMessage();
        ...
        public bool DeleteListEx(string PostIDs)
        {
            if (!string.IsNullOrEmpty(PostIDs))
            {
                ...
                {
                    if (!dal.DeleteEx(YSWL.Common.Globals.SafeInt(item, 0)))
                    {
                        return false;
        ...
        {
            //不查询状态为3的动态
            string strWhere = string.Format(" STATUS<>{0}", (int)YSWL.MALL.Model.SNS.EnumHelper.PostStatus.AlreadyDel);
            if (type > -1)
            {
            ...
            {
                strWhere += string.Format(" AND Description like '%{0}%'", Common.InjectionFilter.SqlFilter(keywords));
            }
        ...
        public DataSet GetVideoSearchList(string KeyWord)
        {
            ...
            {
                Sql += "and Description like '%" + Common.InjectionFilter.SqlFilter(KeyWord) + "%'";
            }
            return GetList(Sql);
    ...
    bool IsSuccess =dal.DeleteListByNormalPost(PostIDs);
        ...
        public List<YSWL.MALL.Model.SNS.Posts> GetVideoList(int uid, int top)
        {
        ...
        public List<YSWL.MALL.Model.SNS.Posts> GetVideoListByPageCache(int uid, int startIndex, int endIndex)
        {
            ...
            return (List<YSWL.MALL.Model.SNS.Posts>)objModel;
        }

        public List<YSWL.MALL.Model.SNS.Posts> GetVideoListByPage(int uid, int startIndex, int endIndex)
        {
            string strWhere = string.Format(" type=3 and Status=1");
            if (uid > 0)
            {
        ...
        /// <param name="endIndex"></param>
        /// <returns></returns>
        public List<YSWL.MALL.Model.SNS.Posts> GetAudioListByPage(int uid, int startIndex, int endIndex)
        {
