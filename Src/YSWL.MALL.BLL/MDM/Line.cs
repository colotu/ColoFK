...
namespace YSWL.MALL.BLL.MDM
{
	...
	{
		private readonly ILine dal=DAMDM.CreateLine();
		public Line()
		{}
		#region  BasicMethod
		...
		{
			return dal.DeleteList(YSWL.Common.Globals.SafeLongFilter(LineIdlist,0) );
		}
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public YSWL.MALL.Model.MDM.Line GetModelByCache(int LineId)
		{
			
			string CacheKey = "LineModel-" + LineId;
			...
			return (YSWL.MALL.Model.MDM.Line)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<YSWL.MALL.Model.MDM.Line> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<YSWL.MALL.Model.MDM.Line> DataTableToList(DataTable dt)
		{
			List<YSWL.MALL.Model.MDM.Line> modelList = new List<YSWL.MALL.Model.MDM.Line>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				YSWL.MALL.Model.MDM.Line model;
