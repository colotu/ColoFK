...
namespace YSWL.MALL.BLL.Shop.Supplier
{
    ...
    public partial class SupplierInfo
    {
       ...
       /// <returns></returns>
        ...
        {
            bool result = dal.DeleteEx(SupplierId);
            if (result)
            {
                ClearDataCache(SupplierId);
            }
            return result;
        }
       ...
       public bool Update(Model.Shop.Supplier.SupplierInfo supplierInfo, int SupplierId, List<int> idList,int suppAreaId=0)
       {
            YSWL.MALL.Model.Shop.Supplier.SuppAreas suppAreaModel=null;
            if (suppAreaId > 0) {
                Supplier.SuppAreas bll = new SuppAreas();
                suppAreaModel = bll.GetModelByCache(suppAreaId);
            }
            bool result = dal.Update(supplierInfo, SupplierId, idList, suppAreaModel);
            if (result)
            {
                ClearDataCache(supplierInfo.SupplierId);
            }
            return result;
       }
       ...
       public bool Add(int supplierId, List<int> idList)
       {
        ...
        public List<YSWL.MALL.Model.Shop.Supplier.SupplierInfo> GetListByPageEx(string key, int startIndex, int endIndex)
        {
            StringBuilder strWhere = new StringBuilder();
            strWhere.AppendFormat(" ShopName like '%{0}%'  and Status = 1 and StoreStatus = 1 ", InjectionFilter.SqlFilter(key));
        ...
        /// <returns></returns>
        public int GetSuppNumByCache(DateTime? date) {
            string CacheKey = "GetSuppliersNum";
            if (date.HasValue) {
                CacheKey = "GetSuppliersNum-" + date.Value.ToString("yyyy-MM-dd");
            }
            object num = YSWL.Common.DataCache.GetCache(CacheKey);
            if (num == null)
            {
                try
                {
                    num = dal.GetSuppNum(date);
                    if (num != null)
                    {
                        int ModelCache = YSWL.Common.ConfigHelper.GetConfigInt("CacheTime");
                        YSWL.Common.DataCache.SetCache(CacheKey, num, DateTime.Now.AddMinutes(ModelCache), TimeSpan.Zero);
                    }
                }
                catch { }
            }
        ...
        public int GetRecordCount(string key, int areaid)
        {
            return dal.GetRecordCount(key, areaid);
        }
        public List<YSWL.MALL.Model.Shop.Supplier.SupplierInfo> GetListByPage(string key, int areaid, string mod,int startIndex, int endIndex)
        {
            ...
            DataSet ds = dal.GetListByPage(key, areaid, orderby, startIndex, endIndex);
            return DataTableToList(ds.Tables[0]);
        }
        /// <summary>
        /// 增加一条数据
        /// </summary>
        public int Add(YSWL.MALL.Model.Shop.Supplier.SupplierInfo model,int suppAreaId) {
            YSWL.MALL.Model.Shop.Supplier.SuppAreas suppAreaModel = null;
            if (suppAreaId > 0) {
                Supplier.SuppAreas bll = new SuppAreas();
                suppAreaModel=bll.GetModelByCache(suppAreaId);
            }
            int result= dal.Add(model, suppAreaModel);
            if (result > 0) {
                ClearDataCache();
            }
            return result;
        ...
        public bool DeleteSupp(int SupplierId) {
            bool result = dal.DeleteSupp(SupplierId);
            if (result)
            {
                ClearDataCache();
            }
        ...
        public void ClearDataCache(int supplierId=0) {
            Common.DataCache.DeleteCache("GetSuppliersNum");
            Common.DataCache.DeleteCache("GetSuppliersNum-" + DateTime.Now.ToString("yyyy-MM-dd"));
            if (supplierId > 0)
            {
                Common.DataCache.DeleteCache("SuppliersModel-"+supplierId);
            }
        }
