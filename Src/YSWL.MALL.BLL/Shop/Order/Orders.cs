...
namespace YSWL.MALL.BLL.Shop.Order
{
    ...
    public partial class Orders
    {
        ...
        /// 获取实时交易
        /// </summary>
        public List<YSWL.MALL.Model.Shop.Order.OrderInfo> LiveTrading(int top, DateTime? startDate)
        {
            DataSet ds= dal.LiveTrading(top, startDate);
            ...
            if (rowsCount > 0)
            {
                ...
                {
                    row = dt.Rows[n];
                    model = new YSWL.MALL.Model.Shop.Order.OrderInfo();
                    if (row != null)
                    {
        ...
        /// <returns></returns>
        public List<YSWL.MALL.ViewModel.Order.ProductSaleInfo> GetProductSales(int top,string type)
        {
            DateTime startDate;
            //type  mouth:当月排行    day:当天排行
            if (type == "mouth")
            {
            ...
            DataSet ds = dal.GetProductSales(top, startDate);
            if (Common.DataSetTools.DataSetIsNull(ds))
            {
                return null;
            }
            List<YSWL.MALL.ViewModel.Order.ProductSaleInfo> list = new List<YSWL.MALL.ViewModel.Order.ProductSaleInfo>();
            DataTable dt = ds.Tables[0];
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                DataRow row;
                YSWL.MALL.ViewModel.Order.ProductSaleInfo model;
                for (int n = 0; n < rowsCount; n++)
                {
                    row = dt.Rows[n];
                    model = new YSWL.MALL.ViewModel.Order.ProductSaleInfo();
                    if (row != null)
                    {
                        if (row["TotalQuantity"] != null && row["TotalQuantity"].ToString() != "")
                        {
                            model.Count =Common.Globals.SafeInt(row["TotalQuantity"].ToString(),0);
                        }                     
                        if (row["ProductId"] != null)
                        {
                            model.ProductId = Common.Globals.SafeInt(row["ProductId"].ToString(), 0);
                        }
                        if (row["Name"] != null)
                        {
                            model.ProName = row["Name"].ToString();
                        }
                    }
                    list.Add(model);
                }
        ...
        /// <param name="type"> mouth:当月排行    day:当天排行</param>
        /// <returns></returns>
        public List<YSWL.MALL.ViewModel.Order.SuppSaleInfo> GetMerchantSales(int top, string type)
        {
            DateTime startDate;
            //type  mouth:当月排行    day:当天排行
            if (type == "mouth")
            ...
            DataSet ds = dal.GetMerchantSales(top, startDate);
            if (Common.DataSetTools.DataSetIsNull(ds))
            {
                return null;
            }
            List<YSWL.MALL.ViewModel.Order.SuppSaleInfo> list = new List<YSWL.MALL.ViewModel.Order.SuppSaleInfo>();
            DataTable dt = ds.Tables[0];
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                DataRow row;
                YSWL.MALL.ViewModel.Order.SuppSaleInfo model;
                for (int n = 0; n < rowsCount; n++)
                {
                    row = dt.Rows[n];
                    model = new YSWL.MALL.ViewModel.Order.SuppSaleInfo();
                    if (row != null)
                    {
                        if (row["TotalQuantity"] != null && row["TotalQuantity"].ToString() != "")
                        {
                        ...
                        if (row["SupplierName"] != null)
                        {
                            model.Name = row["SupplierName"].ToString();
                        }
