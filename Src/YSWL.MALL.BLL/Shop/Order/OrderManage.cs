...
namespace YSWL.MALL.BLL.Shop.Order
{
    public  class OrderManage
    {
        ...
        public static long CreateOrder(Model.Shop.Order.OrderInfo orderInfo,Accounts.Bus.User currentUser=null)
        {
            ...
            bool IsAutoCheck = YSWL.MALL.BLL.SysManage.ConfigSystem.GetBoolValueByCache("OMS_AutoCheck_IsOpen");
            if (IsAutoCheck)
            {
                bool IsBindLine = YSWL.MALL.BLL.SysManage.ConfigSystem.GetBoolValueByCache("CRM_BindLine_IsOpen");
                if (IsBindLine && orderInfo.LineId <= 0) //开启了绑定了线路，但订单中并没有线路，跳过审核订单
                {
                    return orderInfo.OrderId;
                }
                    YSWL.MALL.Model.MDM.Depot depotModel = YSWL.MALL.BLL.MDM.Depot.GetDepotInfo(orderInfo.DepotId);
                    CheckOrder(orderInfo, currentUser, depotModel);
            }
        ...
        /// <returns></returns>
        public static bool CheckOrder(Model.Shop.Order.OrderInfo orderInfo, Accounts.Bus.User currentUser, Model.MDM.Depot depotModel)
        {
          
            #region 检查订单状态，防止一个订单审核两次
            if (orderInfo.OrderStatus != (int)YSWL.MALL.Model.Shop.Order.EnumHelper.OrderStatus.UnHandle)//状态为已完成
            {
                return false;
            }
            #endregion
            ...
            {
                if (!YSWL.MALL.BLL.Shop.Order.StockService.CheckStock(item.SKU, item.Quantity, orderInfo.DepotId, (item.SupplierId.HasValue&& item.SupplierId.Value > 0 ? item.SupplierId.Value : 0)))
                {
                    return false;
                }
            }
            ...
            orderInfo.AssignUserId = -1;
            orderInfo.AssignName ="系统自动审核";
            orderInfo.AssignDate = DateTime.Now;
             
            //更新SKU库存
            YSWL.MALL.BLL.Shop.Order.StockService.UpdateCheckStock(orderInfo.OrderItems, depotModel.DepotId);

            bool isOpenWMS = CommonHelper.ConnectionWMS();
          
            return service.CheckOrder(orderInfo, depotModel, isOpenWMS);
        }
        #endregion
        ...
        public static bool CancelOrder(Model.Shop.Order.OrderInfo orderInfo, Accounts.Bus.User currentUser)
        {
            #region 检查订单状态
            //未发货时 可取消
            if (orderInfo.OrderStatus == (int) YSWL.MALL.Model.Shop.Order.EnumHelper.OrderStatus.Complete ||
                orderInfo.PaymentStatus != (int) Payment.Model.PaymentStatus.NotYet ||
                orderInfo.ShippingStatus >= (int) YSWL.MALL.Model.Shop.Order.EnumHelper.ShippingStatus.Shipped)
            {
                return false;
            }
            ...
            if (currentUser.UserType == "UU" &&
                (orderInfo.OrderStatus < 0 || orderInfo.OrderStatus  == 2 || currentUser.UserID != orderInfo.BuyerID))
            {
...
(orderInfo.OrderStatus != 1 || currentUser.UserID != orderInfo.BuyerID)) ||
        ...
        /// <param name="depotId"></param>
        /// <returns></returns>
        public static int GetUsedStock(string sku, int depotId, int supplierId)
        {
            return service.GetUsedStock(sku, depotId, supplierId);
        }
