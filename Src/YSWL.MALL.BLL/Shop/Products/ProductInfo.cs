...
namespace YSWL.MALL.BLL.Shop.Products
{
      ...
      public  int GetProductRecCount(ProductRecType type, int categoryId,int RegionId=0)
      {
        ...
        public List<YSWL.MALL.Model.Shop.Products.ProductInfo> ProductRecTableToList(DataTable dt)
        {
            ...
            if (rowsCount > 0)
            {
                ...
                bool HasSupplierId = dt.Columns.Contains("SupplierId");
                bool HasSaleCounts = dt.Columns.Contains("SaleCounts");
                YSWL.MALL.Model.Shop.Products.ProductInfo model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new YSWL.MALL.Model.Shop.Products.ProductInfo();
                    ...
                    if (HasSupplierId)
                    {
                        if (dt.Rows[n]["SupplierId"] != null && dt.Rows[n]["SupplierId"].ToString() != "")
                        {
                            model.SupplierId = Common.Globals.SafeInt(dt.Rows[n]["SupplierId"].ToString(), 0);
                        }
                    }
                    if (HasSaleCounts)
                    {
                        if (dt.Rows[n]["SaleCounts"] != null && dt.Rows[n]["SaleCounts"].ToString() != "")
                        {
                            model.SaleCounts = Common.Globals.SafeInt(dt.Rows[n]["SaleCounts"].ToString(), 0);
                        }
                    }
    ...
    string mod, int startIndex, int endIndex, int RegionId = 0)
        ...
        public List<Model.Shop.Products.ProductInfo> GetSearchListEx(int Cid, int BrandId, string keyWord, string priceRange, int saleType,
    string mod, int startIndex, int endIndex, int RegionId = 0,string sku=null)
        {
            ...
            DataSet ds = dal.GetSearchListEx(Cid, BrandId, keysList, priceRange, saleType,mod, startIndex, endIndex, SupplierId,sku);
            if (Common.DataSetTools.DataSetIsNull(ds)) return null;
            return dal.DataTableToList(ds.Tables[0]);
        }
        #endregion
    ...
    string mod, int startIndex, int endIndex, int RegionId = 0, int? SaleStatus = null,string sku = null)
        {
            ...
            DataSet ds = dal.GetSearchListExApp(Cid, BrandId, keysList, priceRange, mod, startIndex, endIndex, SupplierId,SaleStatus,sku);
            if (Common.DataSetTools.DataSetIsNull(ds)) return null;
