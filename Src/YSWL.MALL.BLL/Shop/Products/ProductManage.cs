...
namespace YSWL.MALL.BLL.Shop.Products
{
    public class ProductManage
    {
        ...
        public static bool AddProduct(Model.Shop.Products.ProductInfo productInfo, out long ProductId)
        {
            ...
            if (IsSuccess) {
                Common.DataCache.DeleteCache("GetInSaleSkuNum");
                Common.DataCache.DeleteCache("GetInSaleSkuNum-" + DateTime.Now.ToString("yyyy-MM-dd"));
            }
             return IsSuccess; 
        }
       ...
       {
            bool IsSuccess = service.AddSuppProduct(productInfo, out ProductId);
            if (IsSuccess)
            {
                Common.DataCache.DeleteCache("GetInSaleSkuNum");
                Common.DataCache.DeleteCache("GetInSaleSkuNum-" + DateTime.Now.ToString("yyyy-MM-dd"));
        ...
        {
            bool IsSuccess = service.ModifySuppProduct(productInfo, oldSaleStatus);
            if (IsSuccess)
            {
                Common.DataCache.DeleteCache("GetInSaleSkuNum");
                Common.DataCache.DeleteCache("GetInSaleSkuNum-" + DateTime.Now.ToString("yyyy-MM-dd"));
