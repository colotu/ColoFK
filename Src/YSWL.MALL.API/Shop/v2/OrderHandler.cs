    ...
    public partial class ShopHandler
    {
            ...
            foreach (OrderInfo item in orderList)
            {
                ...
                if (null != item.OrderItems && item.OrderItems.Count > 0)
                {
                    item.OrderItems.ForEach(info =>
                    {
                        ...
                        if (YSWL.Components.MvcApplication.IsAutoConn)
                        {
                            pList.Add(String.IsNullOrWhiteSpace(info.ThumbnailsUrl) ? info.ThumbnailsUrl : info.ThumbnailsUrl.Replace("/Upload/", mdmUrl + "/Upload/"));
                        }
                        else
                        {
        ...
        public JsonObject AddCartProductList(long orderId, int userId, int regionId = 0)
        {
            ...
            foreach (YSWL.MALL.Model.Shop.Order.OrderItems item in orderModel.OrderItems)
            {
                ...
                if (YSWL.Components.MvcApplication.IsAutoConn)
                {
                    model.ThumbnailUrl1 = String.IsNullOrWhiteSpace(model.ThumbnailUrl1) ? model.ThumbnailUrl1 : model.ThumbnailUrl1.Replace("/Upload/", mdmUrl + "/Upload/");
                }
                #endregion
                json = new JsonObject();       
                   ...
                   // skuValues = new string[listSkuItems.Count];
                    listSkuItems.ForEach(xx =>
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(xx.ImageUrl))
                        {
                            ...
                            if (YSWL.Components.MvcApplication.IsAutoConn)
                            {
                                skustrJson.Put("pic", String.IsNullOrWhiteSpace(xx.ImageUrl) ? xx.ImageUrl : xx.ImageUrl.Replace("/Upload/", mdmUrl + "/Upload/"));
                            }
                            else
                            {
