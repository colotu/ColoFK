...
namespace YSWL.MALL.API.Shop.v2
{
    public partial class ShopHandler
    {
        ...
        #region 商品列表
        [JsonRpcMethod("ProductListV2", Idempotent = false)]
        [JsonRpcHelp("商品列表")]
        ...
        #region 促销规则商品列表
        [JsonRpcMethod("SalesRuleProductListV2", Idempotent = false)]
        [JsonRpcHelp("促销商品列表")]
        public JsonObject SalesRuleProductList(int userId, int? page = 1, int pageNum = 30)
        {
            if (pageNum == 0)
            {
                pageNum = 30;
        ...
        #region 商品详情
        [JsonRpcMethod("ProductDetailV2", Idempotent = false)]
        [JsonRpcHelp("商品详情")]
        public JsonObject ProductDetailV2(int pId = -1, int userId = -1)
        {
            if (pId <= 0)
            {
                return new Result(ResultStatus.Failed, Result.FormatFailed(ERROR_CODE_ARGUMENT, ERROR_MSG_ARGUMENT));
...
attrSKUItem in productSKUModel.ListAttrSKUItems)
        ...
        #region 商品分类
        [JsonRpcMethod("CategoryListV2", Idempotent = false)]
        [JsonRpcHelp("类别列表")]
        public JsonObject CategoryListV2(int Cid, int Top = 7)
        {
            List<YSWL.MALL.Model.Shop.Products.CategoryInfo> cateList = YSWL.MALL.BLL.Shop.Products.CategoryInfo.GetAvailableCateList();
        ...
        #region 搜索商品列表
        [JsonRpcMethod("SearchProductListV2", Idempotent = false)]
        [JsonRpcHelp("搜索商品列表")]
        public JsonObject SearchProductListV2(int cid = 0, int brandid = 0, string keyword = "", string orderby = "hot", string price = "0-0",
