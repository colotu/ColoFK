...
namespace YSWL.MALL.API.Sales.v1
{
    public partial class SalesHandler
    {
        ...
        [JsonRpcHelp("客户订单")]
        public JsonObject GetCustOrderList(int userId, int custUserId,string startDate, string endDate, string keyWords, int type = 1 ,int? page = 1, int pageNum = 30)
        {
            ...
            strWhere.AppendFormat(" ReferID='{0}' ", userId);
            //主订单
            ...
            switch (type)
            {
                //待付款
                case 2:
                    strWhere.Append( " AND PaymentStatus = 0 and OrderStatus<> -1 AND PaymentGateway<> 'cod' AND PaymentGateway<> 'bank'");
                    break;
                //待发货
                case 3:
            ...
            foreach (OrderInfo item in orderList)
            {
               ...
               // pList = new JsonArray();
                ...
                //        {
                //            pList.Add(String.IsNullOrWhiteSpace(o.ThumbnailsUrl) ? o.ThumbnailsUrl : o.ThumbnailsUrl.Replace("/Upload/", mdmUrl + "/Upload/"));
                //        }
                //        else
                //        {
        ...
        [JsonRpcHelp("客户订单详情")]
        public JsonObject GetCustOrderDetail(int userId, int custUserId, long orderId = -1)
        {
            //Safe
            if (!userManage.Exists(userId, custUserId))//不是当前用户的客户
            {
        ...
        [JsonRpcHelp("提交订单")]
        public JsonObject SubmitOrder(int salesUserId, int userId, int shipId, int regionId, int shipTypeId, int paymentId, JsonArray productList, string remark, string invoiceHeader, string invoiceContent, string couponCode, int proSaleId = -1, int groupBuyId = -1)
        {
            JsonObject jsonObject = new JsonObject();
            ...
            else
            {
                ...
                if (null != userModel)
                {
                    ...
                    if (userModel.EmployeeID != salesUserId) {//信息不匹配
                        return new Result(ResultStatus.Failed, "InfoMismatch");
                    }
    ...
    ((int)YSWL.MALL.Model.Shop.Order.EnumHelper.ReferType.SalesMan).ToString(), 1440);
            ...
            YSWL.MALL.Model.Ms.Regions regionInfo = regionsBLL.GetModelByCache(regionId);
            if (regionInfo == null)
            {
            ...
            if (groupBuyId > 0)
            {
                YSWL.MALL.Model.Shop.PromoteSales.GroupBuy buyModel = groupBuyBll.GetModelByCache(groupBuyId);
                if (buyModel != null)
                {
