...
namespace YSWL.MALL.API.Pos.v1
{
    public partial class PosHandler
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(userName) || string.IsNullOrWhiteSpace(name) || string.IsNullOrWhiteSpace(celPhone) || string.IsNullOrWhiteSpace(pwd)) return new Result(ResultStatus.Failed, Result.FormatFailed(HandlerBase.ERROR_CODE_ARGUMENT, HandlerBase.ERROR_MSG_ARGUMENT));
        ...
        public JsonObject GetCustomInfo(int UserId, int CustUserId)
        {
            ...
            if (user == null)
            {
                return new Result(ResultStatus.Failed, Result.FormatFailed(HandlerBase.ERROR_CODE_NODATA, HandlerBase.ERROR_MSG_NODATA));
            }
