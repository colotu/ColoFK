...
namespace YSWL.MALL.WebApi.Controllers
{
    ...
    {
        private readonly YSWL.MALL.BLL.Shop.DisDepot.Depot _disDepotBll = new YSWL.MALL.BLL.Shop.DisDepot.Depot();
        ...
        private readonly YSWL.MALL.BLL.Shop.Order.Orders _orderManage = new YSWL.MALL.BLL.Shop.Order.Orders();
        private readonly BLL.Members.Users _userManage = new BLL.Members.Users();
        private readonly YSWL.MALL.BLL.Shop.Order.OrderAction _actionBll = new YSWL.MALL.BLL.Shop.Order.OrderAction();
        private readonly YSWL.MALL.BLL.Ms.Regions _regionsBll = new YSWL.MALL.BLL.Ms.Regions();
        private readonly YSWL.MALL.BLL.Shop.Products.ProductInfo _productManage = new YSWL.MALL.BLL.Shop.Products.ProductInfo();
        ...
        private readonly YSWL.MALL.BLL.Shop.Coupon.CouponInfo _couponBll = new BLL.Shop.Coupon.CouponInfo();
        private readonly YSWL.MALL.BLL.Shop.Activity.ActivityInfo _activInfoBll = new BLL.Shop.Activity.ActivityInfo();
        private readonly YSWL.MALL.BLL.Shop.Shipping.ShippingRegionGroups _shippingRegionManage = new YSWL.MALL.BLL.Shop.Shipping.ShippingRegionGroups();
        private readonly YSWL.MALL.BLL.Shop.PromoteSales.GroupBuy _groupBuyBll = new YSWL.MALL.BLL.Shop.PromoteSales.GroupBuy();
        ...
        [Route("depot/list")]
        public ResponseResult DepotList()
        {
            List<Model.Shop.DisDepot.Depot> depotList = _disDepotBll.GetModelList("Status=1");
            int defaultDepot=BLL.Shop.DisDepot.Depot.GetDefaultDepot();
            ...
            foreach (Model.Shop.DisDepot.Depot item in depotList)
            {
                ...
                obj.Put("isDefault", item.DepotId == defaultDepot);
                array.Put(obj);
            }
            return SuccessResult(array);
        }
        ...
        [HttpGet]
        [Route("region/list")]
        public ResponseResult GetRegionList(int regionId = 0)
        {
            List<YSWL.MALL.Model.Ms.Regions> allRegionList = _regionsBll.GetModelList("");
            ...
            JsonArray result = new JsonArray();
            JsonObject baseJson;
            foreach (Model.Ms.Regions item in provinceList)
            {
                baseJson = new JsonObject();
        ...
        private JsonArray GetDataByParentId(int parentId, List<YSWL.MALL.Model.Ms.Regions> allRegionList)
        {
            ...
            if (list.Count > 0)
            {
                foreach (Model.Ms.Regions item in list)
                {
        ...
        [AcceptVerbs("GET", "POST")]
        [Route("paytype/list")]
        public ResponseResult PayTypeList()
        {
        ...
        [HttpPost]
        [Route("freight/get")]
        public ResponseResult GetFreight([FromBody]YSWL.MALL.ViewModel.Order.FreightVm model)
        {
            ...
            try
            {
                ...
                shoppingCartInfo = GetShoppingCart(model.productList, model.userId, out shoppingCartHelper, -1, -1);
            }
            ...
            #region 获取收货地址及地区
            YSWL.MALL.Model.Shop.Shipping.ShippingAddress shippingAddress = _addressManage.GetModelByCache(model.shipId);
            if (shippingAddress == null)
            {
                jsonObject.Put("freight", Freight);
                return SuccessResult(jsonObject);
            }
            YSWL.MALL.Model.Ms.Regions regionInfo = _regionsBll.GetModelByCache(shippingAddress.RegionId);
            if (regionInfo == null)
            {
                jsonObject.Put("freight", Freight);
                return SuccessResult(jsonObject);
            ...
            #region 获取配送(物流)
            YSWL.MALL.Model.Shop.Shipping.ShippingType shippingType = _shippingTypeManage.GetModelByCache(model.shipTypeId);
            ...
            jsonObject.Put("freight", Freight);
            jsonObject.Put("productAmount", model.productList.Where(t=>t.Count>0).Sum(t=>t.AdjustPrice*t.Count));
            return SuccessResult(jsonObject);
        }
        ...
        public ResponseResult OrderList([FromUri]ViewModel.Order.OrderRequestVm orderVm, int? page = 1, int pageNum = 30)
        {
            ...
            JsonObject json;
            List<YSWL.MALL.Model.Shop.Order.OrderInfo> orderList = _orderManage.GetListByPageEX(strWhere.ToString(), "",
                startIndex, endIndex);
            ...
            foreach (YSWL.MALL.Model.Shop.Order.OrderInfo item in orderList)
            {
                json = new JsonObject();
        ...
        [HttpGet]
        [Route("order/detail")] 
        public ResponseResult OrderDetail(long orderId = -1)
        {
            //Safe
            //if (!_userManage.Exists(userId, custUserId))//不是当前用户的客户
            //{
            ...
            //        {
            //            jsonItem.Put("pic", String.IsNullOrWhiteSpace(item.ThumbnailsUrl) ? item.ThumbnailsUrl : item.ThumbnailsUrl.Replace("/Upload/", _mdmPath + "/Upload/"));
            //        }
            //        else
            //        {
            ...
            //        YSWL.MALL.Model.Shop.Products.ProductInfo pro = _productManage.GetModelByCache(item.ProductId);
            //        if (null != pro)
            //        {
        ...
        [HttpPost]
        [Route("order/submit")]
        public ResponseResult SubmitOrder([FromBody]YSWL.MALL.ViewModel.Order.OrderSubmitVm orderVm)
        {
            JsonObject jsonObject = new JsonObject();
            string orderRemark = orderVm.remark;
            if (!string.IsNullOrWhiteSpace(orderRemark))
            {
                orderRemark = YSWL.Common.InjectionFilter.Filter(orderRemark);
            }
            #region 发票信息 保存到备注中
            string invoice = "";
            if (!string.IsNullOrWhiteSpace(orderVm.invoiceHeader))
            {
                invoice += "发票抬头：" + YSWL.Common.InjectionFilter.Filter(orderVm.invoiceHeader);
            }
            if (!string.IsNullOrWhiteSpace(orderVm.invoiceContent))
            {
                invoice += "     发票内容：" + YSWL.Common.InjectionFilter.Filter(orderVm.invoiceContent);
            }
            ...
            {
                userModel = _userManage.GetModelByCache(orderVm.userId);
                if (null != userModel)
                {
    ...
    ((int)YSWL.MALL.Model.Shop.Order.EnumHelper.ReferType.SalesMan).ToString(), 1440);
            ...
            foreach (YSWL.MALL.Model.Shop.Products.ShoppingCartItem item in shoppingCartInfo.Items)
            {
                ...
                if (item.Quantity < 1 || item.Quantity > item.Stock)
                {
                    ...
                    notstockJson.Put("stockcount", item.Stock);
                    notstockArray.Add(notstockJson);
                }
            }
            if (notstockArray.Length > 0)
            {
                return Result(ResultStatus.Fail, new { Code = ResponseCode.NoHaveStock, Msg = notstockArray });
            ...
            #region 6.生成订单
            //DONE: 5.生成订单
            YSWL.MALL.Model.Shop.Order.OrderInfo mainOrder = new YSWL.MALL.Model.Shop.Order.OrderInfo();

            #region 填充订单数据

            #region 基础数据
            #region 下单类型
            mainOrder.ReferType = (int)YSWL.MALL.Model.Shop.Order.EnumHelper.ReferType.SalesMan;
            #endregion
            ...
            if (!string.IsNullOrWhiteSpace(orderVm.couponCode))
            {
                ...
                if (infoModel != null && infoModel.Status < 2 && infoModel.Status >= 0 &&
                    shoppingCartInfo.TotalAdjustedPrice > infoModel.LimitPrice) //优惠券未使用而且 商品总价大于优惠券限制金额（不含运费）
                {
            ...
            else if(orderVm.couponAmount!=0)  //代下单或者直接填写优惠价格使用
            {
                mainOrder.CouponAmount = orderVm.couponAmount;
            }
            ...
            #region 区域差异运费计算
            int topRegionId;
            topRegionId = regionInfo.Depth > 1 ? YSWL.Common.Globals.SafeInt(regionInfo.Path.Split(new[] { ',' })[1], -1) : regionInfo.RegionId;
        ...
        /// <returns></returns>
        [Route("order/cartlist")]
        [HttpPost]
        public ResponseResult GetCartList(ViewModel.Shop.ShoppingCartGetVm modelVm)
        {
            if (modelVm.ProductList == null)
            {
                return FailResult(ResponseCode.ParamError,"商品列表不能为空");
            }
            ...
            try
            {
                shoppingCartInfo = GetShoppingCartDepot(modelVm.ProductList, modelVm.UserId,depotId:modelVm.DepotId);
            }
            catch (ArgumentNullException)
            {
                return FailResult(ResponseCode.BadGateway, "PROSALEEXPIRED");
            }
            if (shoppingCartInfo == null ||
                shoppingCartInfo.Items == null ||
                shoppingCartInfo.Items.Count < 1)
            {
                return FailResult(ResponseCode.BadGateway, "NOSHOPPINGCARTINFO");
            }
            ...
            foreach (var item in shoppingCartInfo.Items)
            {
                ...
                if (WebApiApplication.IsAutoConn)
                {
                    json.Put("pic", String.IsNullOrWhiteSpace(item.ThumbnailsUrl) ? item.ThumbnailsUrl : item.ThumbnailsUrl.Replace("/Upload/", _mdmPath + "/Upload/"));
                }
                else
                {
        ...
        private YSWL.MALL.Model.Shop.Products.ShoppingCartInfo GetShoppingCartDepot(List<YSWL.MALL.ViewModel.Order.ProductVm> jsonSku, int userId, int prosaleId = -1, int groupbuyId = -1, int depotId = -1)
        {
            YSWL.MALL.Model.Shop.Products.ShoppingCartInfo shoppingCartInfo = new YSWL.MALL.Model.Shop.Products.ShoppingCartInfo();
            if (null != jsonSku && jsonSku.Count > 0)
            {
        ...
        #region 获取购物车数据
        private YSWL.MALL.Model.Shop.Products.ShoppingCartInfo GetShoppingCart(List<YSWL.MALL.ViewModel.Order.ProductVm> jsonSku, int userId,
           out YSWL.MALL.BLL.Shop.Products.ShoppingCartHelper shoppingCartHelper, int prosaleId = -1, int groupbuyId = -1)
        {
            ...
            if (null != jsonSku && jsonSku.Count > 0)
            {
                foreach (ProductVm product in jsonSku)
                {
                    ...
                    if (productInfo == null) return null;
                    YSWL.MALL.Model.Shop.Products.ShoppingCartItem itemInfo = new YSWL.MALL.Model.Shop.Products.ShoppingCartItem();
                    itemInfo.MarketPrice = productInfo.MarketPrice ?? 0;
                    itemInfo.Name = productInfo.ProductName;
                    itemInfo.Quantity = count;
                    itemInfo.SellPrice = skuInfo.SalePrice;
                    itemInfo.AdjustedPrice = skuInfo.SalePrice;
