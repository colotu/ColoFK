...
namespace YSWL.MALL.WebApi.Controllers
{
    ...
    /// </summary>
    [RoutePrefix("v1.0")]
    public class RankRuleController : ApiControllerBase
    {
        ...
        [Route("rankrule/list")]
        [HttpGet]
        public ResponseResult List(string ruleName = null, int? page = 1,
            int pageNum = 30)
        {
            if (!string.IsNullOrEmpty(ruleName))
            {
                ruleName = YSWL.Common.InjectionFilter.SqlFilter(ruleName);
            }
            List<Model.Shop.Sales.SalesRule> ruleList = _ruleBll.GetListByPageApp(ruleName, 1, page, pageNum);
        ...
        /// <returns></returns>
        [HttpGet]
        [Route("rankrule/detail")]
        public ResponseResult Detail(int ruleId)
        {
            if (ruleId < 1)
            {
            ...
            if (ruleModel != null)
            {
                ...
                //规则项信息
                List<Model.Shop.Sales.SalesItem> itemList = _ruleItemBll.GetModelList(" RuleId=" + ruleModel.RuleId);
                if (itemList != null && itemList.Count > 0)
                {
                    //只取一条数据
                    Model.Shop.Sales.SalesItem itemModel = itemList.OrderByDescending(c => c.ItemId).First();
                    model.RuleItem = new List<ViewModel.Shop.SaleRuleItemVm>
                    {
                        new ViewModel.Shop.SaleRuleItemVm
                        {
                            ItemType = itemModel.ItemType,
                            RateValue = itemModel.RateValue
                        }
                    };
                }
        ...
        [Route("rankrule/add")]
        [HttpPost]
        public ResponseResult Add(ViewModel.Shop.SalesRuleVm model)
        {
        ...
        [Route("rankrule/edit")]
        [HttpPost]
        public ResponseResult Update(ViewModel.Shop.SalesRuleVm model)
        {
            if (string.IsNullOrEmpty(model?.RuleName) || model.RuleId < 1)
            {
                return FailResult(ResponseCode.ParamError);
            }
            ...
            if (ruleModel == null)
            {
                return FailResult(ResponseCode.NotFound, "请求数据不存在");
            }
        ...
        /// <returns></returns>
        [Route("rankrule/delete")]
        [HttpGet]
        public ResponseResult DeleteBatch(string ids = null)
        {
