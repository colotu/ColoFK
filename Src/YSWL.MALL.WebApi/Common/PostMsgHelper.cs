...
namespace YSWL.MALL.WebApi.Common
{
    /// <summary>
    /// 处理微信用户消息回复
    /// </summary>
    public class PostMsgHelper
    {
        ...
        private static YSWL.WeChat.Model.Core.PostMsg GetProductMsg(YSWL.WeChat.Model.Core.RequestMsg msg, YSWL.WeChat.Model.Core.Command command, int step = 1)
        {
            ...
            #endregion
            int top = YSWL.Common.Globals.SafeInt(BLL.SysManage.ConfigSystem.GetValueByCache("MShop_ProductMsg_TopCount"), 5);
            ...
            if (productInfos != null && productInfos.Count > 0)
            {
                ...
                string HostUrl = "http://" + YSWL.Common.Globals.DomainFullName;
                int i = 0;
                                                                    ...
                                                                    YSWL.WeChat.Model.Core.Command command, int step = 1)
        {
            ...
            #endregion
            int articleCount = YSWL.Common.Globals.SafeInt(YSWL.MALL.BLL.SysManage.ConfigSystem.GetValueByCache("WeChat_Article_Count"), 10);
        ...
        /// <param name="command"></param>
        /// <returns></returns>
        private static YSWL.WeChat.Model.Core.PostMsg GetAttendance(YSWL.WeChat.Model.Core.RequestMsg msg)
        {
            YSWL.WeChat.Model.Core.PostMsg postMsg = new PostMsg();
            postMsg.CreateTime = DateTime.Now;
            postMsg.OpenId = msg.OpenId;
        ...
        /// <param name="command"></param>
        /// <returns></returns>
        private static YSWL.WeChat.Model.Core.PostMsg SumitTextReports(YSWL.WeChat.Model.Core.RequestMsg msg,
                                                                    YSWL.WeChat.Model.Core.Command command, int step = 1)
        {
            YSWL.WeChat.Model.Core.PostMsg postMsg = new PostMsg();

            YSWL.MALL.BLL.JLT.Reports reportBll = new Reports();
            YSWL.WeChat.BLL.Core.User userBll = new User();
            YSWL.WeChat.Model.Core.User userModel = userBll.GetUser(msg.OpenId, msg.UserName);
            //获取指令关键字
            string keyWords = step == 1 ? YSWL.WeChat.BLL.Core.Command.GetKeyWord(command, msg.Description) : msg.Description;
        ...
        /// <param name="command"></param>
        /// <returns></returns>
        private static YSWL.WeChat.Model.Core.PostMsg SumitImageReports(YSWL.WeChat.Model.Core.RequestMsg msg,
                                                                    YSWL.WeChat.Model.Core.Command command, int Step = 1)
        {
            YSWL.WeChat.Model.Core.PostMsg postMsg = new PostMsg();
            YSWL.WeChat.BLL.Core.User userBll = new User();
        ...
        /// <param name="command"></param>
        /// <returns></returns>
        private static YSWL.WeChat.Model.Core.PostMsg GetSNSPostMsg(YSWL.WeChat.Model.Core.RequestMsg msg,
                                                                    YSWL.WeChat.Model.Core.Command command, int Step = 1)
        {
            YSWL.WeChat.Model.Core.PostMsg postMsg = new PostMsg();
            YSWL.WeChat.BLL.Core.User userBll = new User();
            ...
            if (msg.MsgType == "text")
            {
                string keyWords = Step == 1
                                    ? YSWL.WeChat.BLL.Core.Command.GetKeyWord(command, msg.Description)
                                    : msg.Description;
                if (String.IsNullOrWhiteSpace(keyWords))
                {
                    SetActionCache(msg.UserName, command.ActionId);
        ...
        private static YSWL.WeChat.Model.Core.PostMsg GetCouponInfo(YSWL.WeChat.Model.Core.RequestMsg msg)
        {
            ...
            if (IsNeedBind)
            {
                ...
                if (wUserModel == null || wUserModel.UserId <= 0)
                {
                    ...
                    string returnUrl = YSWL.WeChat.BLL.Core.Utils.GetWCUrl(msg, "/COM/WeChat/UserBind", false);
            ...
            postMsg.MsgType = "news";
            YSWL.WeChat.Model.Core.MsgItem item = new MsgItem();
            item.PicUrl = "http://" + YSWL.Common.Globals.DomainFullName + "/Images/d-02.jpg";
            item.Url = "";
        ...
        private static YSWL.WeChat.Model.Core.PostMsg GetExpressInfo(YSWL.WeChat.Model.Core.RequestMsg msg, YSWL.WeChat.Model.Core.Command command, int step = 1)
        {
            ...
            #endregion
            List<YSWL.MALL.ViewModel.Shop.Express> expressList = ExpressHelper.GetExpress(keyWords);
            ...
            YSWL.WeChat.Model.Core.Command command, int type = 0, int step = 1)
        {
            ...
            postMsg.MsgType = "news";
            YSWL.WeChat.Model.Core.MsgItem item = new MsgItem();
            item.PicUrl = String.IsNullOrWhiteSpace(infoModel.ImageUrl) ? "" : "http://" + YSWL.Common.Globals.DomainFullName + String.Format(infoModel.ImageUrl, "N_");
            string basePath = YSWL.Components.MvcApplication.GetCurrentRoutePath(AreaRoute.COM);
            switch (type)
            {
                case 0:
        ...
        public static string GetCommandMsg(YSWL.WeChat.Model.Core.Command command, YSWL.WeChat.Model.Core.RequestMsg msg)
        {
            ...
            string xmlStr = "";
            //设置指令缓存 方便连续指令使用
            PostMsgHelper.SetCommandCache(command, msg.UserName);
            //匹配指令
        ...
        public static string GetEventMsg(YSWL.WeChat.Model.Core.RequestMsg msg)
        {
            ...
            if (actionId == 1)
            {
                command.TargetId = menuModel == null ? 0 : YSWL.Common.Globals.SafeInt(menuModel.Remark, 0);
                msg.Description = "";
            }
            //匹配指令
        ...
        public static string GetLocationMsg(YSWL.WeChat.Model.Core.RequestMsg msg)
        {
            string xmlStr = "";
            //获取开关设置
            int LocationType = YSWL.Common.Globals.SafeInt(YSWL.WeChat.BLL.Core.Config.GetValueByCache("WeChat_LocationMsg", msg.OpenId), 0);
            switch (LocationType)
            {
                case 0:
                    return xmlStr;
        ...
        public static string GetToken(string AppId, string AppSecret)
        {
            try
            {
                ...
                int code = YSWL.Common.Globals.SafeInt(result, 0);
                if (code > 0)
                {
                    YSWL.MALL.Model.SysManage.ErrorLog model = new YSWL.MALL.Model.SysManage.ErrorLog();
                    model.OPTime = DateTime.Now;
                                                              ...
                                                              YSWL.WeChat.Model.Core.Command command)
        {
            ...
            string token = PostMsgHelper.GetToken();
            YSWL.WeChat.Model.Core.PostMsg postMsg = new PostMsg();
            YSWL.WeChat.BLL.Core.User userBll = new User();
            YSWL.WeChat.Model.Core.User userModel = userBll.GetUser(msg.OpenId, msg.UserName);
            postMsg.CreateTime = DateTime.Now;
            ...
            IAsyncResult re = deleMethod.BeginInvoke(context, userModel, msg,null,null);
        ...
        public delegate void DeleMethod(HttpContext context,YSWL.WeChat.Model.Core.User userModel, YSWL.WeChat.Model.Core.RequestMsg msg);

        public static void CreateQrCode(HttpContext context,YSWL.WeChat.Model.Core.User userModel, YSWL.WeChat.Model.Core.RequestMsg msg)
        {
