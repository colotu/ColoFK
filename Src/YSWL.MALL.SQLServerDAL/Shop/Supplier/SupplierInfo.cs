       ...
       /// <returns></returns>
        public bool DeleteEx(int SupplierId)
        {
           ...
           List<CommandInfo> sqllist = new List<CommandInfo>();
            ...
            strSql14.Append(" delete from Shop_SuppAreaRelation  WHERE SupplierId=@SupplierId  ");
            SqlParameter[] parameters14 = {
                   new SqlParameter("@SupplierId", SqlDbType.Int,4)
            };
            parameters14[0].Value = SupplierId;
        ...
        public bool Update(Model.Shop.Supplier.SupplierInfo model, int SupplierId, List<int> idList,YSWL.MALL.Model.Shop.Supplier.SuppAreas suppAreaModel)
        {
            ...
            StringBuilder strSql3 = new StringBuilder();
            strSql3.Append("delete from  Shop_SuppAreaRelation ");
            strSql3.Append("where SupplierId=@SupplierId ");
            SqlParameter[] parameters3 = {
                    new SqlParameter("@SupplierId", SqlDbType.Int,4) };
            parameters3[0].Value = model.SupplierId;
            listCommand.Add(new CommandInfo(strSql3.ToString(), parameters3));

            if (suppAreaModel != null)
            {
                StringBuilder strSql4 = new StringBuilder();
                strSql4.Append("insert into Shop_SuppAreaRelation(");
                strSql4.Append("AreaId,SupplierId,AreaPath)");
                ...
                parameters4[0].Value = suppAreaModel.AreaId;
                parameters4[1].Value = model.SupplierId;
                parameters4[2].Value = suppAreaModel.Path;
                listCommand.Add(new CommandInfo(strSql4.ToString(), parameters4, EffentNextType.ExcuteEffectRows));
            }
        ...
        /// <returns></returns>
        public int GetSuppNum(DateTime? date)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" Select count(1) from Shop_Suppliers  where  Status=1 and StoreStatus=1 ");
            if (date.HasValue)
            {
                strSql.AppendFormat(" and CreatedDate>= '{0}' ", date.Value.ToString("yyyy-MM-dd"));
            }
            object obj = DBHelper.DefaultDBHelper.GetSingle(strSql.ToString());
            if (obj == null)
            {
        ...
        /// 获取记录总数
        /// </summary>
        public int GetRecordCount(string key, int areaid = 0)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) FROM Shop_Suppliers T ");
            strSql.Append(" where  Status = 1 and StoreStatus = 1  ");
            if (!String.IsNullOrWhiteSpace(key))
            {
                strSql.AppendFormat(" AND ShopName like '%{0}%' ", Common.InjectionFilter.SqlFilter(key));
            }
            if (areaid > 0)
            { 
                strSql.AppendFormat(
                    " AND EXISTS ( SELECT *  FROM  Shop_SuppAreaRelation WHERE  SupplierId =T.SupplierId  ");
                strSql.AppendFormat(
              "   AND ( AreaPath LIKE ( SELECT Path FROM Shop_SuppAreas WHERE AreaId = {0}  ) + '|%' ",
              areaid);
                strSql.AppendFormat(" OR Shop_SuppAreaRelation.AreaId = {0}))", areaid);
            }
            object obj = DBHelper.DefaultDBHelper.GetSingle(strSql.ToString());
            if (obj == null)
            {
        ...
        /// 分页获取数据列表
        /// </summary>
        public DataSet GetListByPage(string key, int areaid, string orderby, int startIndex, int endIndex)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT * FROM ( ");
            strSql.Append(" SELECT ROW_NUMBER() OVER (");
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(YSWL.MALL.Model.Shop.Supplier.SupplierInfo model, YSWL.MALL.Model.Shop.Supplier.SuppAreas suppAreaModel)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into Shop_Suppliers(");
            strSql.Append("Name,ShopName,StoreStatus,CategoryId,Rank,UserId,UserName,TelPhone,CellPhone,ContactMail,Introduction,RegisteredCapital,RegionId,Address,Contact,EstablishedDate,EstablishedCity,LOGO,Fax,PostCode,HomePage,ArtiPerson,CompanyType,BusinessLicense,TaxNumber,AccountBank,AccountInfo,ServicePhone,QQ,MSN,Status,CreatedDate,CreatedUserId,UpdatedDate,UpdatedUserId,ExpirationDate,Balance,IsUserApprove,IsSuppApprove,ScoreDesc,ScoreService,ScoreSpeed,Recomend,Sequence,FavoritesCount,SalesCount,ProductCount,PhotoCount,ThemeId,Remark,AgentId,IndexProdTop,IndexContent,Latitude,Longitude)");
            strSql.Append(" values (");
            strSql.Append("@Name,@ShopName,@StoreStatus,@CategoryId,@Rank,@UserId,@UserName,@TelPhone,@CellPhone,@ContactMail,@Introduction,@RegisteredCapital,@RegionId,@Address,@Contact,@EstablishedDate,@EstablishedCity,@LOGO,@Fax,@PostCode,@HomePage,@ArtiPerson,@CompanyType,@BusinessLicense,@TaxNumber,@AccountBank,@AccountInfo,@ServicePhone,@QQ,@MSN,@Status,@CreatedDate,@CreatedUserId,@UpdatedDate,@UpdatedUserId,@ExpirationDate,@Balance,@IsUserApprove,@IsSuppApprove,@ScoreDesc,@ScoreService,@ScoreSpeed,@Recomend,@Sequence,@FavoritesCount,@SalesCount,@ProductCount,@PhotoCount,@ThemeId,@Remark,@AgentId,@IndexProdTop,@IndexContent,@Latitude,@Longitude)");
            strSql.Append(";set @SupplierId=@@IDENTITY");
            SqlParameter[] parameters = {
                    new SqlParameter("@Name", SqlDbType.NVarChar,100),
                    new SqlParameter("@ShopName", SqlDbType.NVarChar,100),
                    new SqlParameter("@StoreStatus", SqlDbType.SmallInt,2),
            ...
            parameters[53].Value = model.Latitude;
            parameters[54].Value = model.Longitude;
            parameters[55].Direction = ParameterDirection.Output;
            ...
            if (suppAreaModel != null) {
                ...
                strSql.Append(" values (");
                strSql.Append("@AreaId,@@IDENTITY,@AreaPath)");
                SqlParameter[] parameters1 = {
                    new SqlParameter("@AreaId", SqlDbType.Int,4),
                    new SqlParameter("@AreaPath", SqlDbType.NVarChar,4000)};
                parameters1[0].Value = suppAreaModel.AreaId;
                parameters1[1].Value = suppAreaModel.Path;
            ...
            int SupplierId = (int)parameters[55].Value;
            if (rows > 0)
            {
                return SupplierId;
            }
        ...
        /// 删除一条数据
        /// </summary>
        public bool DeleteSupp(int SupplierId)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from Shop_Suppliers ");
            strSql.Append(" where SupplierId=@SupplierId");
            ...
            StringBuilder strSql1 = new StringBuilder();
            strSql1.Append("delete from Shop_SuppAreaRelation(");
            strSql1.Append(" where SupplierId=@SupplierId");
            SqlParameter[] parameters1 = {
                    new SqlParameter("@SupplierId", SqlDbType.Int,4)
            };
            parameters1[0].Value = SupplierId;
            sqllist.Add(new CommandInfo(strSql1.ToString(), parameters1));
