...
namespace YSWL.MALL.SQLServerDAL.Shop.Order
{
    ...
    public class OrderService : IOrderService
    {
        ...
        public CommandInfo GenerateOrderInfo(OrderInfo model)
        {
            ...
            parameters[100].Value = model.LineId;
            parameters[101].Value = model.LineName;
...
", subLength, startIndex, endIndex);
        ...
        /// <returns></returns>
        public int GetUsedStock(string sku, int depotId, int supplierId)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT  SUM(Quantity) FROM    OMS_OrderItems ");
            strSql.AppendFormat(" WHERE   sku = @SKU  AND EXISTS ( SELECT * FROM   OMS_Orders WHERE  OrderId = OMS_OrderItems.OrderId AND OrderStatus <=1 AND OrderStatus>-1 and  depotId =@DepotId and  ISNULL(WaveStatus,0) <301 )  ");
            if (supplierId > 0)
            {
                strSql.AppendFormat(" and  SupplierId={0} ", supplierId);
            }
        ...
        public bool CheckOrder(Model.Shop.Order.OrderInfo orderInfo,
            Model.MDM.Depot depotModel,bool isOpenWMS)
        {
            List<CommandInfo> sqllist = new List<CommandInfo>();
            ...
            //DONE: 更新订单状态
            StringBuilder strSql2 = new StringBuilder();
            strSql2.Append("UPDATE  OMS_Orders SET OrderStatus=1,WaveStatus=0, UpdatedDate =@UpdatedDate,DepotId=@DepotId,DepotName=@DepotName ");
            ...
            parameters2[0].Value = orderInfo.OrderId;
            parameters2[1].Value = DateTime.Now;
            parameters2[2].Value = depotModel.DepotId;
            parameters2[3].Value = depotModel.Name;
            CommandInfo cmd = new CommandInfo(strSql2.ToString(), parameters2, EffentNextType.ExcuteEffectRows);
            sqllist.Add(cmd);
            ...
            StringBuilder strSql3 = new StringBuilder();
            strSql3.Append("insert into OMS_OrderAction(");
            strSql3.Append("OrderId,OrderCode,SourceOrderId,SourceType,UserId,Username,ActionCode,ActionDate,Remark)");
            strSql3.Append(" values (");
            strSql3.Append("@OrderId,@OrderCode,@SourceOrderId,@SourceType,@UserId,@Username,@ActionCode,@ActionDate,@Remark)");
            SqlParameter[] parameters3 =
                {
            ...
            parameters3[0].Value = orderInfo.OrderId;
            parameters3[1].Value = orderInfo.OrderCode;
            parameters3[2].Value = orderInfo.SourceOrderId;
            parameters3[3].Value = orderInfo.SourceType;
            parameters3[4].Value = -1;
            parameters3[5].Value = "系统自动审核";
            parameters3[6].Value = (int)YSWL.MALL.Model.Shop.Order.EnumHelper.ActionCode.Audited; //orderInfo.ActionCode;
            parameters3[7].Value = DateTime.Now;
            parameters3[8].Value = "您的订单已经审核，被分配到【" + depotModel.Name + "】仓库";
            cmd = new CommandInfo(strSql3.ToString(), parameters3, EffentNextType.ExcuteEffectRows);
            sqllist.Add(cmd);
            ...
            if (isOpenWMS)
            {
                ...
                //{
                //    StringBuilder strSql4 = new StringBuilder();
                //    strSql4.AppendFormat("update WMS_Stock  set UsableStock=UsableStock-@Stock ,UsedStock=UsedStock+@Stock");
                //    strSql4.Append(" where SKU=@SKU and DepotId=@DepotId");
                //    SqlParameter[] parameters4 =
                //            {
                //            new SqlParameter("@SKU", SqlDbType.NVarChar, 50),
                //            new SqlParameter("@Stock", SqlDbType.Int, 4),
                //             new SqlParameter("@DepotId", SqlDbType.Int, 4)
                //        };
                //    parameters4[0].Value = item.SKU;
                //    parameters4[1].Value = item.Quantity;
                //    parameters4[2].Value = orderInfo.DepotId;
                //    sqllist.Add(new CommandInfo(strSql4.ToString(), parameters4));
                //}
                //#endregion
            }
            else
            {
                ...
                {
                    StringBuilder strSql5 = new StringBuilder();
                    strSql5.AppendFormat("update ERP_Stock  set UsableStock=UsableStock-@Stock ,UsedStock=UsedStock+@Stock");
                    strSql5.Append(" where SKU=@SKU and DepotId=@DepotId");
                    SqlParameter[] parameters5 =
                            {
                            new SqlParameter("@SKU", SqlDbType.NVarChar, 50),
                            new SqlParameter("@Stock", SqlDbType.Int, 4),
                    ...
                    parameters5[0].Value = item.SKU;
                    parameters5[1].Value = item.Quantity;
                    parameters5[2].Value = orderInfo.DepotId;
                    sqllist.Add(new CommandInfo(strSql5.ToString(), parameters5));
                }
