...
namespace YSWL.MALL.SQLServerDAL.Shop.Order
{
    ...
    public partial class Orders : IOrders
    {
        ...
        public bool Update(YSWL.MALL.Model.Shop.Order.OrderInfo model)
        {
            ...
            parameters[100].Value = model.LineId;
            parameters[101].Value = model.LineName;
            parameters[102].Value = model.OrderId;
        ...
        /// 获取实时交易
        /// </summary>
        public DataSet LiveTrading(int top,DateTime? startDate)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("  select ");
            if (top>0)
            ...
            strSql.Append(" OrderId,SupplierId,SupplierName,CreatedDate from oms_orders  ");
            strSql.Append(" where supplierId > 0 and OrderStatus<> -1 ");
            if (startDate.HasValue) {
                strSql.AppendFormat(" and CreatedDate >= '{0}'  ", startDate.Value.ToString("yyyy-MM-dd 00:00:00"));
            }
        ...
        /// 获取商品销量
        /// </summary>
        public DataSet GetProductSales(int top, DateTime? startDate)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("  select ");
            if (top > 0)
            ...
            strSql.Append(" sum(Quantity) as TotalQuantity ,ProductId,Name from oms_orderitems orderitems ");
            strSql.Append(" where  ");
            strSql.Append(" EXISTS(SELECT * FROM oms_orders orders  WHERE orderitems.orderid = orders.orderid and OrderStatus <> -1 and  supplierId > 0 ");
            if (startDate.HasValue)
            {
                strSql.AppendFormat(" and CreatedDate >= '{0}'  ", startDate.Value.ToString("yyyy-MM-dd 00:00:00"));
            }
            strSql.Append(" ) ");
            strSql.Append("  group by productid,name order by TotalQuantity desc ");
        ...
        /// 获取商户销量
        /// </summary>
        public DataSet GetMerchantSales(int top, DateTime? startDate)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("  select ");
            if (top > 0)
            ...
            strSql.Append(" sum(Quantity) as TotalQuantity,SupplierId,SupplierName from oms_orderitems  orderitems ");
            strSql.Append(" where  ");
            strSql.Append(" EXISTS(SELECT * FROM oms_orders orders  WHERE orderitems.orderid = orders.orderid and OrderStatus <> -1 and  supplierId > 0 ");
            if (startDate.HasValue)
            {
            ...
            strSql.Append(" ) ");
            strSql.Append("  group by SupplierId,SupplierName order by TotalQuantity desc ");
            return DBHelper.DefaultDBHelper.Query(strSql.ToString());
        }
        #endregion
    }
