...
namespace YSWL.MALL.SQLServerDAL.Shop.Products
{
    ...
    public partial class SKUInfo : ISKUInfo
    {
        ...
        /// <returns></returns>
        public int GetInSaleSkuNum(DateTime? date)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select count(1) from pms_skus  sku  where Upselling=1 ");
            strSql.AppendFormat(" and  EXISTS  ( SELECT *  FROM pms_products p  WHERE sku.ProductId=p.ProductId and SaleStatus=1 and SalesType=1 ");
            if (date.HasValue) {
                strSql.AppendFormat(" and AddedDate>= '{0}' ",date.Value.ToString("yyyy-MM-dd"));
            }
            strSql.Append(" ) ");
            object obj = DBHelper.DefaultDBHelper.GetSingle(strSql.ToString());
            if (obj == null)
