...
namespace YSWL.MALL.SQLServerDAL.Shop.Products
{
    ...
    public partial class ProductInfo : IProductInfo
    {
        ...
        public bool Update(YSWL.MALL.Model.Shop.Products.ProductInfo model)
        {
            ...
            parameters[44].Value = model.DeliveryTip;
            parameters[45].Value = model.Remark;
            parameters[46].Value = model.ProductId;
...
//TODO: 字段对应
        public DataSet GetProductRecList(ProductRecType type, int categoryId, int top,int SupplierId)
        {
            ...
            strSql.Append(" P.ProductId,P.MarketPrice,P.ShortDescription,P.ProductName,p.ThumbnailUrl1,p.ThumbnailUrl2,P.ProductCode ,P.LowestSalePrice, P.Points,P.SupplierId FROM    " + TableName + " P ");
            if (categoryId > 0)
            {
...
//TODO: 字段需要对应
        public DataSet GetProductRanListByRec(ProductRecType type, int categoryId, int top,int SupplierId)
        {
            ...
            strSql.Append(" P.ProductId,P.ShortDescription,P.ProductName,p.ThumbnailUrl1,p.ThumbnailUrl2,P.ProductCode ,P.LowestSalePrice , P.MarketPrice FROM    " + TableName + " P ");
            if (categoryId > 0)
            {
                strSql.Append(" INNER JOIN  ");
                strSql.Append("(SELECT DISTINCT ProductId FROM PMS_ProductCategories ");
                                          ...
                                          string mod, int startIndex, int endIndex, int SupplierId)
        {
            ...
            strSql.AppendFormat(" WHERE   SaleStatus<2 and  SalesType=1  ");
            //品牌查询
            if (BrandId > 0)
            {
                strSql.AppendFormat("   AND BrandId = {0}", BrandId);
        ...
        public DataSet GetSearchListEx(int Cid, int BrandId, List<String> keysList, string priceRange,int saleType,
                                          string mod, int startIndex, int endIndex, int SupplierId,string sku=null)
        {
            string TableName = "PMS_Products";
            if (SupplierId > 0)
            {
            ...
            {
                strSql.AppendFormat(" WHERE   SaleStatus={0} and  SalesType=1  ",saleType);
            }
            //品牌查询
            if (BrandId > 0)
            {
            ...
            {
                strSql.AppendFormat(" and (exists(select SkuId from PMS_SKUs where ProductId=T.ProductId and SKU='{0}') or ProductCode='{1}')", sku, sku);
            }
            #region 关键字列表搜索
            StringBuilder keySql = new StringBuilder();
            foreach (var item in keysList)
        ...
        public DataSet GetProdListByPage(string strWhere, string orderby, int startIndex, int endIndex,int SupplierId)
        {
            ...
            strSql.Append(" P.CategoryId, TypeId, P.ProductId, BrandId, ProductName, ProductCode, SaleStatus, AddedDate, VistiCounts, SaleCounts, MarketPrice, LowestSalePrice, PenetrationStatus, MainCategoryPath, ExtendCategoryPath,  ImageUrl, ThumbnailUrl1 ");
            strSql.Append("FROM " + TableName + " P ");
            if (!string.IsNullOrEmpty(strWhere.Trim()))
            {
                strSql.Append(" WHERE " + strWhere);
...
drop table #Temp_Shop_Categories;";
        ...
        public DataSet GetProductRecListByPage(ProductRecType type, int categoryId, string orderby, int startIndex, int endIndex)
        {
            ...
            strSql.Append("SELECT  ");
            strSql.Append("PSM.StationId, P.ProductId,P.MarketPrice,P.ShortDescription,P.ProductName,p.ThumbnailUrl1,p.ThumbnailUrl2,P.ProductCode ,P.LowestSalePrice, P.Points,P.SupplierId,P.SaleCounts FROM  PMS_Products P ");
            if (categoryId > 0)
            {
                strSql.Append(" INNER JOIN  ");
                strSql.Append("(SELECT DISTINCT ProductId FROM PMS_ProductCategories ");
        ...
        public DataSet GetSearchListExApp(int Cid, int BrandId, List<String> keysList, string priceRange,
                                          string mod, int startIndex, int endIndex, int SupplierId, int? SaleStatus = null,string sku=null)
        {
            string TableName = "PMS_Products";
            if (SupplierId > 0)
            {
