...
", subLength);
        ...
        public YSWL.MALL.ViewModel.SAAS.UserInfo GetSAUserInfo(string userName, string passWord,int userType=1,long enterprideId=0)
        {
            byte[] cryptPassword =YSWL.Accounts.Bus.AccountsPrincipal.EncryptPassword(passWord);
            DataSet ds= YSWL.SAAS.BLL.SAASInfo.GetSAASUserInfo(userName, cryptPassword, userType, enterprideId);
            if (ds!=null&&ds.Tables[0].Rows.Count > 0)
            {
                return YSWL.Common.ConvertUtity.ConvertToModel<YSWL.MALL.ViewModel.SAAS.UserInfo>(ds.Tables[0]);
            }
            else
            {
                return null;
