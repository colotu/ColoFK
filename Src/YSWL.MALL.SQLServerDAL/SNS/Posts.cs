            ...
            using (SqlConnection connection =  DBHelper.DefaultDBHelper.GetDBObject().GetConnection)
            {
                ...
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                         ...
                         if (model.Type == (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Photo || model.Type == (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Product)
                    {
                        ...
                        TargetID = DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateImageInfo(model, PModel, AlbumId, Pid, PhotoCateId, RecommandStateInt, photoAdress, mapLng, mapLat), transaction);
                        model.TargetId = Globals.SafeInt(TargetID.ToString(), -1);
                             ...
                             DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateAblumInfo(type,model.CreatedUserID, AlbumId, model.TargetId,model.Description), transaction);
                        DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateUpdateUserEx(model.CreatedUserID), transaction);
                        DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateUpdateAlbum(AlbumId), transaction);
                    }
                    //在向相应的动态表中加入相应的记录
                         if (CreatePost)
                         {
                             PostID = DBHelper.DefaultDBHelper.GetSingle4Trans(GeneratePostInfo(model), transaction);
            ...
            using (SqlConnection connection =  DBHelper.DefaultDBHelper.GetDBObject().GetConnection)
            {
                ...
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        object targetID = DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateProductInfo(PModel), transaction);
                         int TargetId = Globals.SafeInt(targetID.ToString(), -1);
                         DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateAblumInfo((int)YSWL.MALL.Model.SNS.EnumHelper.ImageType.Product, PModel.CreateUserID, AlbumId, TargetId, PModel.ShareDescription), transaction);
                            DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateUpdateUserEx(PModel.CreateUserID), transaction);
                            DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateUpdateAlbum(AlbumId), transaction);
            ...
            using (SqlConnection connection =  DBHelper.DefaultDBHelper.GetDBObject().GetConnection)
            {
                ...
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        TargetID = DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateBlog(blogModel), transaction);
                            model.TargetId = Globals.SafeInt(TargetID.ToString(), -1);
                        //在向相应的动态表中加入相应的记录
                        if (CreatePost)
                        {
...
;
        ...
        {
            List<CommandInfo> sqllist = new List<CommandInfo>();
            YSWL.MALL.SQLServerDAL.SNS.Comments dal = new YSWL.MALL.SQLServerDAL.SNS.Comments();
            YSWL.MALL.Model.SNS.Posts Model = new YSWL.MALL.Model.SNS.Posts();
