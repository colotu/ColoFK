...
namespace YSWL.MALL.SQLServerDAL.SNS
{
	...
	public partial class GroupTopicFav:IGroupTopicFav
	{
        ...
        public int GetRecordCount(int createdUserId)
        {
            ...
            string strWhere = "select count(*) from SNS_GroupTopicFav s ";
            strWhere += "where exists(";
            strWhere += "  select * from SNS_GroupTopicFav t where s.TopicID=t.TopicID and";
            strWhere += string.Format(" s.CreatedUserID={0})", createdUserId);
            object obj = DBHelper.DefaultDBHelper.GetSingle(strWhere);
            if (null != obj)
            {
                return Convert.ToInt32(obj);
            }
        ...
        {
            string strWhere = string.Format(" delete from SNS_GroupTopicFav where CreatedUserID={0} and TopicID={1}",
                                            userId, topicId);
            if (DBHelper.DefaultDBHelper.ExecuteSql(strWhere) > 0)
            {
                return true;
            }
            return false;
