...
namespace YSWL.MALL.SQLServerDAL.SNS
{
	...
	public partial class GroupTopicReply:IGroupTopicReply
	{
                ...
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        if ((PModel != null && PModel.ProductID > 0) || !string.IsNullOrEmpty(Tmodel.PhotoUrl))
                        {
                            ...
                            DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateUpdateUserEx(Tmodel.ReplyUserID, Tmodel.Type.Value), transaction);
                        }
                        ReplyId = DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateTopicReplyInfo(Tmodel), transaction);
                        DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateUpdateGroupEx(Tmodel.GroupID), transaction);
                        DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateUpdateTopicEx(Tmodel), transaction);
                        transaction.Commit();
            ...
            using (SqlConnection connection =  DBHelper.DefaultDBHelper.GetDBObject().GetConnection)
            {
                ...
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        //    DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateUpdateUserEx(Tmodel.ReplyUserID, Tmodel.Type.Value), transaction);
                        //}
                        ReplyId = DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateForwardReplyInfo(TModel), transaction);
                        DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateUpdateGroupInfo(TModel), transaction);
                        DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateUpdateUserExInfo(TModel), transaction);
                        DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateUpdateTopicInfo(TModel), transaction);
                        transaction.Commit();
                        return Common.Globals.SafeInt(ReplyId != null ? ReplyId.ToString() : "", 0);
                    }
                    catch (Exception)
        ...
        public DataSet GetReplyList(int userId)
        {
           ...
           SqlParameter sqlParameter=new SqlParameter("@userid",SqlDbType.Int);
            sqlParameter.Value = userId;
            return DBHelper.DefaultDBHelper.Query(strSql.ToString(),sqlParameter);
        }
