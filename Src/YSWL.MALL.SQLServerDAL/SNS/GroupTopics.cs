...
namespace YSWL.MALL.SQLServerDAL.SNS
{
	/// <summary>
	/// 数据访问类:GroupTopics
	/// </summary>
	public partial class GroupTopics:IGroupTopics
	{
		...
		public YSWL.MALL.Model.SNS.GroupTopics GetModel(int TopicID)
		{
			...
			YSWL.MALL.Model.SNS.GroupTopics model=new YSWL.MALL.Model.SNS.GroupTopics();
		...
		/// 得到一个对象实体
		/// </summary>
		public YSWL.MALL.Model.SNS.GroupTopics DataRowToModel(DataRow row)
		{
			YSWL.MALL.Model.SNS.GroupTopics model=new YSWL.MALL.Model.SNS.GroupTopics();
			if (row != null)
			{
        ...
        public bool DeleteEx(int TopicID)
        {
            ...
            YSWL.MALL.Model.SNS.GroupTopics model = GetModel(TopicID);
            if (null == model)
            {
                return true;
            }
        ...
        public bool DeleteData(YSWL.MALL.Model.SNS.GroupTopics model)
        {
        ...
        public int AddEx(YSWL.MALL.Model.SNS.GroupTopics Tmodel, YSWL.MALL.Model.SNS.Products PModel)
        {
            ...
            using (SqlConnection connection =  DBHelper.DefaultDBHelper.GetDBObject().GetConnection)
            {
                ...
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            TargetID = DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateImageInfo(Tmodel, PModel), transaction);
                        ...
                        TopicId = DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateTopicInfo(Tmodel), transaction);
                        DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateUpdateUserEx(Tmodel.CreatedUserID, Tmodel.Type), transaction);
                        DBHelper.DefaultDBHelper.GetSingle4Trans(GenerateUpdateGroupCount(Tmodel), transaction);
                        transaction.Commit();
        ...
        public CommandInfo GenerateUpdateGroupCount(YSWL.MALL.Model.SNS.GroupTopics Tmodel)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        #region 增加主题
        public CommandInfo GenerateTopicInfo(YSWL.MALL.Model.SNS.GroupTopics Tmodel)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into SNS_GroupTopics(");
            strSql.Append("CreatedUserID,CreatedNickName,GroupID,GroupName,Title,Description,IsRecomend,Sequence,ReplyCount,PvCount,DingCount,Status,IsTop,IsActive,IsAdminRecommend,ChannelSequence,HasReferUsers,PostExUrl,ImageUrl,VideoUrl,AudioUrl,ProductName,Price,ProductLinkUrl,Type,TargetID,FavCount,CreatedDate,LastReplyUserId,LastReplyNickName,LastPostTime,Tags)");
        ...
        public CommandInfo GenerateUpdateUserEx(int UserId, int type)
        {
            ...
            else if ((int)YSWL.MALL.Model.SNS.EnumHelper.ImageType.Photo == type)
            {
                StringBuilder strSql = new StringBuilder();
        ...
        private CommandInfo GenerateImageInfo(YSWL.MALL.Model.SNS.GroupTopics Tmodel, YSWL.MALL.Model.SNS.Products PModel)
        {
        ...
        public bool DeleteGroupTopic(int topicId)
        {
            List<CommandInfo> sqllist = new List<CommandInfo>();
            #region 删除评论关联的数据
            StringBuilder strSql1 = new StringBuilder();
            strSql1.Append("delete from SNS_GroupTopicReply ");
            strSql1.Append(" where TopicID=@TopicID");
            SqlParameter[] parameters0 = {
					new SqlParameter("@TopicID", SqlDbType.Int,4)
			};
            parameters0[0].Value = topicId;
            CommandInfo cmd1 = new CommandInfo(strSql1.ToString(), parameters0, EffentNextType.None);
            sqllist.Add(cmd1); 
            #endregion
            ...
            parameters[0].Value = topicId;
            CommandInfo cmd = new CommandInfo(strSql.ToString(), parameters, EffentNextType.None);
            sqllist.Add(cmd);
            #endregion

            StringBuilder strSql2=new StringBuilder();
            strSql2.Append("delete from SNS_GroupTopics ");
            strSql2.Append(" where TopicID=@TopicID");
            CommandInfo cmd2 = new CommandInfo(strSql2.ToString(), parameters0, EffentNextType.None);
            sqllist.Add(cmd2);
            return DBHelper.DefaultDBHelper.ExecuteSqlTran(sqllist) > 0 ? true : false;
        }
