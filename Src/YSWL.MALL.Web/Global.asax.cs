...
{ 
    ...
    {
        bool isAutoConn = Common.ConfigHelper.GetConfigBool("AutoConnection");
        ...
        public string GetAreaThemeName(AreaRoute areaRoute, ControllerContext context = null)
        {
            switch (areaRoute)
            {
                ...
                case AreaRoute.MBShop:
                    string mbshoptheme = "Y1";//YSWL.MALL.BLL.SysManage.ConfigSystem.GetValueByCache("MBShop_Theme");
                    return string.IsNullOrWhiteSpace(mbshoptheme) ? MvcApplication.ThemeName : mbshoptheme;
                case AreaRoute.MB:
                    string mbtheme = YSWL.MALL.BLL.SysManage.ConfigSystem.GetValueByCache("MB_Theme");
        ...
        public string AuthorizeCode
        {
            get
            {
                return isAutoConn ? "" : ConfigSystem.GetValueByCache("AuthorizeCode");
            }
        }
        ...
        public string ThemeName
        {
            get
            {
                return isAutoConn ? "Y1" : ConfigSystem.GetValueByCache("ThemeCurrent");
            }
        }
