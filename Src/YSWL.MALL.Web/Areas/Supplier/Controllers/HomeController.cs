...
namespace YSWL.MALL.Web.Areas.Supplier.Controllers
{
    public class HomeController : SupplierControllerBase
    {
        ...
        public PartialViewResult Top(string viewName = "_Top")
        {
            ...
            List<YSWL.MALL.Model.SysManage.SysNode> AllNodeList = sm.GetTreeListByTypeCache(4, true,MvcApplication.IsAutoConn);
        ...
        public string EditShop()
        {
            ...
            if (null != model)
            {
                #region 移动文件
                // 移动文件
                string logoImagepath = "/"+MvcApplication.UploadFolder+"/Supplier/Logo/";
        ...
        public ActionResult AddAds(YSWL.MALL.Model.Shop.Supplier.SupplierAD supplierAdModel)
        {
            ...
            //创建文件夹  移动文件
            string path = string.Format("/"+MvcApplication.UploadFolder+"/Supplier/{0}/AD/", SupplierId);
