    ...
    public class ProfileController : UsersProfileControllerBase
    {
        ...
        public ActionResult AjaxProductPost(FormCollection Fm)
        {
            ...
            productModel.ProductName = ProductName;
                //移动图片
                string savePath = "/"+MvcApplication.UploadFolder+"/SNS/Images/Product/" + DateTime.Now.ToString("yyyyMMdd") + "/";
                string saveThumbsPath = "/"+MvcApplication.UploadFolder+"/SNS/Images/ProductThumbs/" + DateTime.Now.ToString("yyyyMMdd") + "/";
        ...
        public ActionResult AjaxPostForward(FormCollection Fm)
        {
            ...
            list = PostsBll.GetForPostByPostId(ResultPostId, IncludeProduct, YSWL.Components.MvcApplication.GetCurrentRoutePath(YSWL.Web.AreaRoute.SNS));
        ...
        {
            string ImageUrl = Fm["ImageUrl"];
            if (!string.IsNullOrEmpty(ImageUrl) && ImageUrl.StartsWith("/"+MvcApplication.UploadFolder+"/")&&!ImageUrl.Contains(".."))
            {
                //假如是云存储
                ...
                if (ImageUrl.Contains("..") || !ImageUrl.StartsWith("/"+MvcApplication.UploadFolder+"/Temp"))
                {
                    return Content("No");
                }
                //首先删除临时文件夹原图
        ...
        public ActionResult AJaxCreateTopic(FormCollection fm)
        {
            ...
            //移动图片
            string savePath = "/"+MvcApplication.UploadFolder+"/SNS/Images/Group/" + DateTime.Now.ToString("yyyyMMdd") + "/";
            string saveThumbsPath = "/"+MvcApplication.UploadFolder+"/SNS/Images/GroupThumbs/" + DateTime.Now.ToString("yyyyMMdd") + "/";
