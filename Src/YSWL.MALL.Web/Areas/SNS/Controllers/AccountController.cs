...
namespace YSWL.MALL.Web.Areas.SNS.Controllers
{
    public class AccountController : SNSControllerBase
    {
        ...
        {
            //到主域的登陆
            if (MvcApplication.MainAreaRoute != AreaRoute.SNS && BLL.SysManage.ConfigSystem.GetBoolValueByCache("System_Unique_Login")) return Redirect("/Account/Login?returnUrl=" + returnUrl);
        ...
        {
            //到主域的注册
            if (MvcApplication.MainAreaRoute != AreaRoute.SNS) return Redirect("/Account/Register");
        ...
        public ActionResult Register(MALL.ViewModel.SNS.RegisterModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    return Redirect(MvcApplication.GetCurrentRoutePath(AreaRoute.Shop)+"UserCenter/Personal");
                }
            }
        ...
        public ActionResult FindPwd(FormCollection collection)
        {
            ...
            YSWL.Accounts.Bus.User userinfo = new User(email);
