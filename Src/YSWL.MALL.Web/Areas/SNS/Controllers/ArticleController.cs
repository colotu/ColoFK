...
namespace YSWL.MALL.Web.Areas.SNS.Controllers
{
    public class ArticleController : SNSControllerBase
    {
        ...
        YSWL.MALL.BLL.CMS.Comment comBll = new BLL.CMS.Comment();
        YSWL.MALL.BLL.CMS.Content contentBll = new Content();
        private YSWL.MALL.BLL.CMS.ContentClass contentClassBll = new ContentClass();
        ...
        public ActionResult Index(int classId, int? page)
        {
            ...
            YSWL.MALL.Model.CMS.ContentClass CurrentClass = AllClass.FirstOrDefault(c => c.ClassID == classId);
            ...
            #region SEO 优化设置
            IPageSetting pageSetting = YSWL.MALL.Web.Components.Setting.CMS.PageSetting.GetContentClassSetting(CurrentClass);
            ...
            if (totalcount == 0)
                return View();
            PagedList<YSWL.MALL.Model.CMS.Content> BlogList = new PagedList<YSWL.MALL.Model.CMS.Content>(
        ...
        public ActionResult ArticleDetail(int id)
        {
            ...
            YSWL.MALL.Model.CMS.Content content = contentBll.GetModelByCache(id);
            
            #region SEO 优化设置
            IPageSetting pageSetting = YSWL.MALL.Web.Components.Setting.CMS.PageSetting.GetArticleSetting(content);
            //pageSetting.Replace(
        ...
        {
            List<YSWL.MALL.Model.CMS.Content> contentList = contentBll.GetMoreList(ClassId, ContentId, top);
            return PartialView(viewName, contentList);
        }
        public PartialViewResult HotArticle(YSWL.MALL.Model.CMS.EnumHelper.ContentRec mode = YSWL.MALL.Model.CMS.EnumHelper.ContentRec.Hot, int ClassId = 0, string viewName = "_HotArticle", int top = -1)
        {
            List<YSWL.MALL.Model.CMS.Content> contentList = contentBll.GetRecList(ClassId, mode, top);
        ...
        {
            List<YSWL.MALL.Model.CMS.Content> contentList = contentBll.GetHotComList(ClassId, top);
            return PartialView(viewName, contentList);
        }
        public PartialViewResult TopArticle(YSWL.MALL.Model.CMS.EnumHelper.ContentRec mode = YSWL.MALL.Model.CMS.EnumHelper.ContentRec.Hot, int ClassId = 0, string viewName = "_TopArticle", int top = -1)
        {
            List<YSWL.MALL.Model.CMS.Content> contentList = contentBll.GetRecList(ClassId, mode, top);
            if (null != contentList)
            {
                contentList = contentList.OrderByDescending(c => c.PvCount).ToList();
            }
            string className = contentClassBll.GetClassnameById(ClassId);
            ViewBag.className = className ?? "暂无栏目名称";
            return PartialView(viewName, contentList);
        ...
        {
            YSWL.MALL.Model.CMS.Content contentModel = contentBll.GetModel(ContentId);
        ...
        public ActionResult AjaxAddComment(FormCollection Fm)
        {
            ...
            YSWL.MALL.Model.CMS.Comment ComModel = new Model.CMS.Comment();
            ...
            ComModel.TypeID = (int)YSWL.MALL.Model.CMS.EnumHelper.CommentType.Content;
            ComModel.State = true;
            if ((CommentId = comBll.AddEx(ComModel)) > 0)
            {
                ...
                List<YSWL.MALL.Model.CMS.Comment> list = new List<Model.CMS.Comment>();
                //是否含有审核词
        ...
        public ActionResult AjaxGetComments(int ContentId, int? PageIndex)
        {
            ...
            List<YSWL.MALL.Model.CMS.Comment> CommentList = comBll.GetComments(ContentId, StartIndex, EndIndex);
