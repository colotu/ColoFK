...
function submit() {
    ...
    if (!(errnum == 0 ? true : false)) {
        ...
        $.ajax({
            url: $YSWL.BasePath+"UserCenter/UserApprove",
            type: 'post',
            dataType: 'json',
            timeout: 10000,
            async: false,
            ...
            success: function (JsonData) {
                switch (JsonData.STATUS) {
                    case "SUCCESS":
                        window.location = $YSWL.BasePath + "UserCenter/SubmitApprove";
                        break;
                    case "FAILE":
...
function comsubmit() {
    $.ajax({
        url: $YSWL.BasePath+"UserCenter/SubmitApprove",
        type: 'post',
        dataType: 'json',
        timeout: 10000,
        async: false,
        ...
        success: function (JsonData) {
            switch (JsonData.STATUS) {
                case "SUCCESS":
                    window.location = $YSWL.BasePath + "UserCenter/SubmitSucc";
                    break;
                case "FAILE":
                    ShowFailTip("服务器忙，请稍后再试！");
                    window.location = $YSWL.BasePath + "UserCenter/UserApprove";
                    break;
                default:
                    ShowServerBusyTip("服务器没有返回数据，可能服务器忙，请稍候再试！");
                    break;
