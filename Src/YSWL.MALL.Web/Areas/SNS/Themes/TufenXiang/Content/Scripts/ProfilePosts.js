$(function () {
    ...
    var pointer = $("#pointer").length > 0 ? parseInt($("#pointer").val()) : 0;
    var rankscore = $("#rankscore").length > 0 ? parseInt($("#rankscore").val()) : 0;
    if (pointer > 0 && rankscore > 0) {
        $.MessageShow("&#12288;登录成功！积分<span style='color:yellow;font-weight: bolder;'>+" + pointer + "</span>，成长值<span style='color:yellow;font-weight: bolder;'>+" + rankscore + "</span>&#12288;");
    } else if (pointer > 0) {
        $.MessageShow("登录成功！积分，<span style='color:yellow;font-weight: bolder;'>+" + pointer + "</span>");
    } else if (rankscore > 0) {
        $.MessageShow("&#12288;登录成功！成长值，<span style='color:yellow;font-weight: bolder;'>+" + rankscore + "</span>&#12288;");
    }
    ...
    $("#postBlog").click(function () {
        ...
        $.ajax({
            url: $YSWL.BasePath+"profile/AjaxAddBlog",
            type: 'post',
            dataType: 'text',
            timeout: 10000,
    ...
    var uploader = new qq.FineUploader({
        ...
        callbacks: {
            onComplete: function (id, fileName, responseJSON) {
                ...
                if (type == "Photo") {
                    $(".btn-success").find("input").css("height", "28px").css("width", "200px").css("font-size", "12px");
                    $("#txtBtnPost").show();
                    $("#UploadPhotoWindow").hide();
                    if (responseJSON.success) {
                        $("#PhotoResultTemplate").load($YSWL.BasePath + "profile/AjaxGetUploadPhotoResult?image=" + responseJSON.data.format("T116x170_") + "&data=" + responseJSON.data, { limit: 25 },
                     function () {
...
function DelYulanTu() {
    $.ajax({
        url: $YSWL.BasePath+"profile/AjaxDelYulanTu",
...
function addProduct() {
    ...
    $.ajax({
        url: $YSWL.BasePath+"profile/AjaxGetProductInfo",
        type: 'post', dataType: 'text', timeout: 10000,
...
function addPhotoPost(imageurl, sharedes, albumid, maplng, maplat, address) {
    $("#txtBtnPost").hide();
    ...
    $.ajax({
        url: $YSWL.BasePath+"profile/AjaxPostAdd",
        type: 'post', dataType: 'text', timeout: 10000,
        data: { Type: "Photo", ImageUrl: imageurl, ShareDes: sharedes, AblumId: albumid, MapLng: maplng,MapLat:maplat,Address:address,ImageType: 0 },
        success: function (resultData) {
            if (resultData == "No") {
                ShowFailTip("操作失败，请您重试！");
            }
...
function AddPoint(type) {
    $.ajax({
        url: $YSWL.BasePath+"profile/AjaxAddPoint",
        type: 'post', dataType: 'text', timeout: 10000,
        data: { Type: type },
        success: function (resultData) {
            var arr = resultData.split("|");
            if (parseInt(arr[0]) > 0 && parseInt(arr[1]) > 0) {
                $.MessageShow("&#12288;恭喜！积分<span style='color:yellow;font-weight: bolder;'>+" + arr[0] + "</span>，成长值<span style='color:yellow;font-weight: bolder;'>+" + arr[1] + "</span>&#12288;");
            } else if (parseInt(arr[0]) > 0) {
                $.MessageShow("恭喜！积分，<span style='color:yellow;font-weight: bolder;'>+" + arr[0] + "</span>");
            } else if (parseInt(arr[1]) > 0) {
                $.MessageShow("恭喜！成长值，<span style='color:yellow;font-weight: bolder;'>+" + arr[1] + "</span>");
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            ShowFailTip("操作失败：" + errorThrown);
...
function LoadAblumFun() {
    $.ajax({
        url: $YSWL.BasePath+"profile/AjaxGetMyMyAblum",
        type: 'post', dataType: 'text', timeout: 10000,
        success: function (resultData) {
            if (resultData == "No") {
                ShowFailTip("操作失败，请您重试！");
...
$(function () {
    ...
    $("#addVideo").click(function (e) {
        ...
        var errnum = 0;
        $.ajax({
            url: $YSWL.BasePath+"Profile/CheckVideoUrl",
            type: 'post',
            dataType: 'json',
            timeout: 10000,
            async: false,
...
//        $(".lyz_tab_right").css("background", "url(../images/user_12.jpg)");
    });
    ...
    $("#postPhoto").die("click").live("click", function (e) {
        ...
        $("#con_one_2 .photolist").each(function () {
            ...
            var maplat = $(this).find(".fabiao_cs_2").children("[name='Address']").next().val();
            var maplng = $(this).find(".fabiao_cs_2").children("[name='Address']").next().next().val();
            ...
            if (!albumId) {
                ...
                addPhotoPost(imageurl, sharedes, albumId, maplng, maplat, address);
            }
    ...
    $("#LoadUrlClose").click(function (e) { e.preventDefault(); $("#LoadVideoWindow").hide(); e.stopPropagation(); $("#txtVideoUrl").val(""); });
    $("#LoadAudioClose").click(function (e) {
        e.preventDefault(); $("#LoadAudioWindow").hide(); e.stopPropagation(); $("#search_input").val("");
    });
