...
* Copyright © 2012 YSWL. All Rights Reserved.
*/
...
function CancelBind(Id) {
    $.ajax({
        url: $YSWL.BasePath + "UserCenter/CancelBind",
        type: 'post',
        data: { Action: "post", BindId: Id },
        dataType: 'text',
        success: function (resultData) {
            if (resultData == "success") {
                window.location = $YSWL.BasePath + "UserCenter/UserBind";
            }
        },
...
function btnBind(text) {
    $.jBox.info("还没有绑定" + text + "帐号，点击这里<a href='"+$YSWL.BasePath +"UserCenter/UserBind'  tyle='color: #fff;' target='_blank'>开始绑定</a>", "帐号绑定", { width: 350, top: 400 });
    $(".jbox-button").hide();
    //        $.jBox.confirm("还没有绑定" + text + "帐号，点击这里<a href='"+$YSWL.BasePath +"UserCenter/UserBind'  tyle='color: #fff;' target='_blank'>开始绑定</a>", "帐号绑定", submit, { width: 350, top: 400 });
}
(function () {
    var CurrentOption;
    function InfoSending(option) {
        ...
        if (CurrentOption.mediaIds != "") {
            $.ajax({
                url: $YSWL.BasePath + "profile/AjaxPostWeibo",
