...
var submitAddAlbum = function (v, h, f) {
    ...
    if (f.albumid == undefined || f.albumid <= 0) {
        ...
        $.ajax({
            url: $YSWL.BasePath + "Profile/AjaxAddToAlbum",
...
var AJaxGetUserId = function() {
    var userid = 0;
    $.ajax({
        url: $YSWL.BasePath + "Profile/AjaxGetUserId",
        type: 'post',
        dataType: 'text',
        timeout: 10000,
        success: function(resultData) {
...
$(".addalbum").die("click").live("click", function (e) {
    ...
    if (CheckUserState()) {
        ...
        $.ajax({
            type: "POST",
            dataType: "text",
            url: $YSWL.BasePath + "Album/GetUserAlbums",
            success: function (data) {
                ...
                if (data != "No") {
                    ...
                    ResultHtml += "</select> <a  href='" + $YSWL.BasePath + "Album/Create' target='_blank'>创建</a>";
