...
{
    public class UserCenterController : BShopControllerBaseUser
    {
            ...
            if (id > 0)
            {
                ...
                if (model != null && model.UserId != CurrentUser.UserID)
                {
                    LogHelp.AddInvadeLog(
                        string.Format(
                            "非法获取收货人数据|当前用户:{0}|获取收货地址:{1}|_YSWL.Web.Areas.BShop.Controllers.UserCenterController.ShippAddress",
        ...
        public ActionResult AjAxPReview(FormCollection fm)
        {
            ...
            foreach (JsonObject jsonObject in jsonArray)
            {
                ...
                if (pid > 0 && orderId > 0 && !String.IsNullOrWhiteSpace(contentval))
                {
                    ...
                    {
                        //创建文件夹  移动文件
                        string path = string.Format("/"+MvcApplication.UploadFolder+"/Shop/ProductReviews/{0}/", DateTime.Now.ToString("yyyyMM"));
