...
{
    public class PartialController : BShopControllerBase
    {
        ...
        public PartialViewResult Navigation(int top=0, string viewName = "_Navigation", string Theme = "M1")
        {
            ...
            List<YSWL.MALL.Model.Settings.MainMenus> NavList = meneBll.GetMenusByAreaByCacle(YSWL.MALL.Model.Ms.EnumHelper.AreaType.BShop, Theme,top);
        ...
        #region 文章
        public PartialViewResult RecContentList(YSWL.MALL.Model.CMS.EnumHelper.ContentRec recType = YSWL.MALL.Model.CMS.EnumHelper.ContentRec.Recomend, int ClassId = 0, int Top = -1, string viewName = "_HotArticle")
        {
            List<YSWL.MALL.Model.CMS.Content> contentList = conBll.GetRecList(ClassId, recType, Top);
            ViewBag.contentclassName = contentclassBll.GetClassnameById(ClassId);
        ...
        #region 商家浮动层
        public PartialViewResult SuppInfo(int suppId=0,string viewName = "_SuppInfo")
        {
            ...
            {
                model.RegionStr = model.RegionId.HasValue
                                    ? regsBll.GetAddress(model.RegionId)
                                    : "暂未设置";
        ...
        public ActionResult SuppLogo(int id=0,string size="")
        {
            ...
            string pathName = string.Format("/"+MvcApplication.UploadFolder+"/Supplier/Logo/{0}_{1}", id, size);
        ...
        #region 辅助方法  
        ////获取商家店铺名称     
        public static string GetSupplierShopName(int suppId=0)
        {
            if (suppId <= 0) {
                return "";
            }
            BLL.Shop.Supplier.SupplierInfo suppinfoBll = new BLL.Shop.Supplier.SupplierInfo();
            Model.Shop.Supplier.SupplierInfo model = suppinfoBll.GetModelByCache(suppId);
            return  model!=null ? model.ShopName:""; 
        }
        #endregion

        #region 店铺区域
        public PartialViewResult SuppAreasList(int AreaId = 0, int Top = 10, string viewName = "_SuppAreasList")
        {
            List<YSWL.MALL.Model.Shop.Supplier.SuppAreas> list = BLL.Shop.Supplier.SuppAreas.GetAvailableList();
            List<YSWL.MALL.Model.Shop.Supplier.SuppAreas> resultList=null;
            if (list == null) {
                return PartialView(viewName, resultList);
            }
            if (AreaId <= 0)
            {
                resultList = list.Where(c => c.ParentAreaId<=0).ToList();
            }else{
                resultList = list.Where(c => c.ParentAreaId == AreaId).ToList();
            }            
            if (Top > 0)
            {
                resultList = resultList.Take(Top).ToList();
            }
            return PartialView(viewName, resultList);
        }
        #endregion
