...
{
    public partial class OrderController : BShopControllerBaseUser
    {
            ...
            if (addressId > 0)
            {
                ...
                if (model != null && model.UserId != CurrentUser.UserID)
                {
                    LogHelp.AddInvadeLog(
                        string.Format("非法获取收货人数据|当前用户:{0}|获取收货地址:{1}|_YSWL.Web.Areas.BShop.Controllers.OrderController.ShowAddress",
        ...
        {
            return View(viewName, _shippingTypeManage.GetCacgeModelBySupplied(suppId));
        }
