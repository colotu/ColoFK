...
{
    public class ReturnOrderController : BShopControllerBaseUser
    {
        ...
        public ActionResult AjaxRetuApply(FormCollection fm)
        {
            ...
            #region 移动文件
              if (!String.IsNullOrWhiteSpace(imagesurlPath) && !String.IsNullOrWhiteSpace(imagesurlName))
                    {
                        //创建文件夹  移动文件
                        string path = string.Format("/"+MvcApplication.UploadFolder+"/Shop/ReturnOrder/{0}/", DateTime.Now.ToString("yyyyMM"));
