$(function() {
    ...
    $(document).on('click','#btnAddToCart',function () {
                ...
                var prodcutId = $(this).attr('productId');
                        ...
                        success: function (resultData) {
                            switch (resultData.STATUS) {
                                case "SUCCESS":
                                    ...
                                    //更新当前商品购物车数量
                                    var currNum =$('#prod_shopcart_num_' + prodcutId).text();
                                    if (currNum == "") {
                                        currNum = count;
                                    } else {
                                        currNum = parseInt(currNum) + count;
                                    }
                                    $('#prod_shopcart_num_' + prodcutId).text(currNum).show();
...
function addShoppingCart(productId, hasSku) {
    if (hasSku=='False') {//未开启sku直接在列表加入购物车，不再出现弹层
        //alert('未来其');
        //return;
    }
    dialogShow(productId);
}
...
//隐藏窗体
function dialoghide() {
    $("body,html").css({ "overflow": "auto" });//恢复页面滚动条
    $('#bg,#addCart-Lay').hide();
}
