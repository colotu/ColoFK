...
{
    public class PayResultController : MBControllerBase
    {
        ...
        public ActionResult Success(string viewName = "Success")
        {
           ...
           //判断是否需要支付
           ViewBag.HasPay = true;// (orderInfo.PaymentGateway != "cod" && orderInfo.PaymentGateway != "bank");

           #region SEO 优化设置
           IPageSetting pageSetting = PageSetting.GetPageSetting("Home", Model.SysManage.ApplicationKeyType.Shop);
           ViewBag.Title = (ViewBag.HasPay ? "支付成功" : "订单提交成功");
        ...
        public ActionResult RechargeFail(string viewName = "RechargeFail")
        {
            ...
            if (!string.IsNullOrWhiteSpace(rechargeIdStr))
            {
                ...
                if (rechargeId < 1)
                    return Content("ERROR_NOTSAFEORDERID");
                LogHelp.AddErrorLog("Mobile >> RechargeFail >> RechargeId[" + rechargeId + "] Status[" + statusStr + "]",
                    statusStr, "Mobile >> RechargeReturnHandler >> Redirect >> PayController");
