...
namespace YSWL.MALL.Web.Areas.MB.Controllers
{
    public partial class PartialController : MBControllerBase
    {
        ...
        /// <param name="viewName"></param>
        /// <returns></returns>
        public PartialViewResult SuppAreaPathList(int AreaId = -1, string viewName = "_SuppAreaPathList")
        {
            List<YSWL.MALL.Model.Shop.Supplier.SuppAreas> pathList = null;
            if (AreaId > 0)
            {
                ...
                if (areaModel != null)
                {
                    ...
                    pathList =areaList.Where(c => path_arr.Contains(c.AreaId.ToString())).OrderBy(c => c.Depth).ToList();
                }
            }
            return PartialView(viewName, pathList);
        }
        #endregion
    }
}