 ...
 SKUResult[key].rankprices.push(sku.rankprice);
    ...
    $(function () {
        ...
        $('#SKUOptions .SkuItems').click(function () {
            ...
            self.toggleClass('skuselect').siblings().removeClass('skuselect');
            ...
            if (self.hasClass('skuselect') && imgSel.length == 1) {
            ...
            var selectedObjs = $('#SKUOptions .skuselect');
            if (selectedObjs.length) {
                ...
                $('#SKUOptions .SkuItems').not(selectedObjs).not(self).each(function () {
                    var siblingsSelectedObj = $(this).siblings('#SKUOptions .skuselect');
 ...
 function InitializationSku() {
     ...
     $('#rankPrice').text(
        ...
        function Status_SKUItem(target, enabled) {
            if (enabled) {
                var skuItem = $(target).removeAttr('disabled').removeClass("unli").find('a:eq(0)').removeClass('unenablesku');
                skuItem.attr('title', skuItem.attr('alt'));
                skuItem.find('img:eq(0)').attr('title', skuItem.attr('alt'));
            } else {
                var skuItem = $(target).attr({ 'disabled':'disabled','cursor':'auto' }).removeClass('skuselect').addClass("unli").find('a:eq(0)').addClass('unenablesku');
