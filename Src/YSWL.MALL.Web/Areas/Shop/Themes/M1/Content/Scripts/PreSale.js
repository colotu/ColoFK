...
function showPreSale() {
    ...
    if (!UserInfo.IsGet) {
        $.ajax({
            type: "get",
            url: " /ArrivingNotify.aspx?Operation=GetUserInfo&callback=?",
            dataType: "jsonp",
...
function submitPreSale() {
    ...
    if (presaleDialog)
        presaleDialog.close();
    $.getJSON(" /ArrivingNotify.aspx?Operation=AddNotify&styleId=" + styleEntityListJson[0].StyleId + "&itemId=" + itemid + "&productId=" + productid + "&mobile=" + cellphone + "&email=" + email + "&flag1=1&callback=?", function (res) {
