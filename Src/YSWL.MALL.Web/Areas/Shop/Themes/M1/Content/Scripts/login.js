...
//LinkCss.href = " /web/c/comm/login.css ";
//$("head").append(LinkCss);
...
(function($) {
    ...
    var login = {
        ...
        CheckLogin: function(options, obj) {
            $.getJSON(" /AjaxLoginForm.aspx?jsoncallback=?",
            function(data) {
                if (!data.error) {
        ...
        ShowLoginForm: function(options, obj) {
            ...
            function(e) {
                e.preventDefault();
                location.href = " /UserRegister.aspx?ReturnUrl=" + escape(location.href);
                //				location.href = " /ContactSimpleRegister.aspx?ReturnUrl=" + escape(location.href);
            });
            //回车事件
        ...
        LoginNow: function(options, UserToken, PasswordWord) {
            $.getJSON(" /AjaxLoginForm.aspx?Uid=" + UserToken + "&Pwd=" + encodeURIComponent(PasswordWord) + "&jsoncallback=?",
            function(data) {
                if (!data.error) {
    ...
    $.fn.AjaxLogin.defaults = {
        ...
        //提示内容
        content: "",
        loginFormHTML: "<div class=\"mask\">" + "</div>" + "<div class=\"overlay\">" + "</div>" + "<div class=\"overlaymain\">" + "    <h2>" + "        会员登录</h2>" + "    <ul class=\"clew\" style=\"display: none;\" id=\"loginContent\">" + "    </ul>" + "    <form method=\"post\" class=\"clearfix\">" + "    <fieldset class=\"uname-length\">" + "        <legend>" + "            <p id=\"logTitle\">" + "            </p>" + "        </legend>" + "       <p class=\"hl mb5 zmt15\" id=\"CellphoneTip\" style=\"display:none;\">手机号如果已认证，可使用手机号登录</p>" + "        <div>" + "            <label for=\"username\">" + "                用户名/Email/手机：</label>" + "            <input type=\"text\" class=\"text\" id=\"UserName\" name=\"UserName\" />" + "        </div>" + "        <div>" + "            <label for=\"password\">" + "                密<span style=\"width:1em;display:inline-block;\"></span>码：</label>" + "            <input type=\"password\" class=\"text\" id=\"Password\" name=\"Password\" />" + "            <a href=\"/Contact/ContactGetPwdByMail.aspx\" target=\"_blank\">忘记密码？</a>" + "        </div>" + "    </fieldset>" + "    <div id=\"login_error\">" + "    </div>" + "    <div class=\"handle\">" + "        <input type=\"button\" id=\"btnLogin\" title=\"登录\" value=\"登录\" />" + "        <a href=\" /UserRegister.aspx\" target=\"_blank\">快速注册</a>" + "    </div>" + "    </form>" + "    <div class='otherLogin pt5 mt10 tc'><a class='m18login'  logintype='layer' href='javascript:void(0);'><img src=' /web/i/mobile/blogbtn.gif' complete='complete' /></a><a id='Kaixin_login' logintype='layer' href='javascript:void(0);' class='ml5 kaixinlogin'><img src=' /web/i/mobile/kaixinbtn.gif' complete='complete' /></a><a id='Alipay_login' logintype='layer' href='javascript:void(0);' class='ml5 alipaylogin'><img src=' /web/i/mobile/alipaybtn.gif' complete='complete' /></a></div>" + "    <a href=\"#\" id=\"CloseLogin\" class=\"close\">关闭</a></div>",
        success: function() {},
        error: function() {}
    };
})(jQuery);