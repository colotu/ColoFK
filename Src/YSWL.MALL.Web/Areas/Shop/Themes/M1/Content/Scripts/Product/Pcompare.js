...
$(function () {
    ...
    $('#btnpcompare').click(function () {
        ...
        currentItem.find('dt').append("<a target=\"_blank\" href=\"" + $YSWL.BasePath + "Product/Detail/" + productId + "\" ><img  width=\"50\" src=\"" + imageurl.format('T50X65_') + "\" height=\"50\"></a>");
        currentItem.find('dd').empty();
        currentItem.find('dd').append("<a target=\"_blank\" class=\"diff-item-name\" href=\"" + $YSWL.BasePath + "Product/Detail/" + productId + "\">" + pname + "</a><span class=\"p-price\"><strong class=\"J-p-1052308220\">￥" + price + "</strong><a class=\"del-comp-item\" prodid=\"" + productId + "\">删除</a></span> ");
        $('#pop-compare').show();
    });
    ...
    $('#goto-contrast').click(function () {
        ...
        if (json && json.length > 1) {
            ...
            window.open($YSWL.BasePath + "Product/Compare/" + type + "/" + prodids);
            return true;
        }
...
function loadPCompare() {
    ...
    if (jsonExists && jsonExists.length>0) {
        ...
        for (var i = 0; i < jsonExists.length; i++) {
            ...
            currentItem.removeClass('item-empty');
            currentItem.find('dt').empty();
            currentItem.find('dt').append("<a target=\"_blank\" href=\"" + $YSWL.BasePath + "Product/Detail/" + jsonExists[i].PId + "\" ><img  width=\"50\" src=\"" + jsonExists[i].ImageUrl.format('T50X65_') + "\" height=\"50\"></a>");
            currentItem.find('dd').empty();
            currentItem.find('dd').append("<a target=\"_blank\" class=\"diff-item-name\" href=\"" + $YSWL.BasePath + "Product/Detail/" + jsonExists[i].PId + "\">" + jsonExists[i].Name + "</a><span class=\"p-price\"><strong class=\"J-p-1052308220\">￥" + jsonExists[i].Price + "</strong><a class=\"del-comp-item\" prodid=\"" + jsonExists[i].PId + "\">删除</a></span> ");
            if (i == 3) {//最多4个
                break;
            }
        }
