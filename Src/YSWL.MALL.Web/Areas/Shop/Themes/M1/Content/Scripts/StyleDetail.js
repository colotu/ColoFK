...
function LoadSKU() {
    if (typeof(aloneMessage) != "undefined" && aloneMessage != null && aloneMessage.length > 0) {
        ...
        if (typeof(styleIdBak) != "undefined" && styleIdBak != null && styleIdBak.length > 0) {
            ...
            var link = " /p-" + styleIdBak + ".htm#rel=DPY_JGBG";
            ...
            var link = " /" + styleEntityListJson[0].ClassId1 + "-" + classId + "-12-60-1-00-1-1-N-N-0-0.htm";
                    ...
                    if (saleOutRelation.length >= num) {
                        for (var i = 0; i < num; i++) {
                            ...
                            if (saleOutRelation.length > 0) {
                                ...
                                likehtml += '<li class="item-m J_carousel_item">' + '<a name="__DPY_XSSP" href=" /p-' + res.StyleId + '.htm#rel=DPY_XSSP" title="' + res.ChineseName + '" target="_blank">' + '<img height="120" width="92" class="item-m-pic" src=" /GOODS/SMALL/' + res.ClassId3 + '/' + res.ImageFileName + '" alt="' + res.ChineseName + '">' + '<span class="item-m-title">' + res.ChineseName + '</span>' + '</a>' + '<p class="item-m-price"><span>' + changeTwoDecimal(res.WebSalePrice) + '</span></p>' + '</li>'
        ...
        if (typeof(scareBuyingItem) != "undefined" && scareBuyingItem != null) { //判断是否是天天抢
            ...
            if (scareBuyingItem.ErrorMsg != null && scareBuyingItem.ErrorMsg.length > 0 && scareBuyingItem.ErrorMsg != "0") {
                ...
                if (scareBuyingItem.ErrorMsg == "1") {
                    errorMsg = "对不起，云商团活动尚未开始或已结束。";
                } else if (scareBuyingItem.ErrorMsg == "2") {
                    errorMsg = "对不起，该商品已售完，请查看其它云商团商品。";
                } else if (scareBuyingItem.ErrorMsg == "3") {
                    errorMsg = "对不起，云商团活动尚未开始，请查看其它云商团商品。";
                } else if (scareBuyingItem.ErrorMsg == "4") {
                    errorMsg = "对不起，云商团活动已经结束，请查看其它云商团商品。";
                }
                $(".choicebox").html("");
                $(".choicebox").hide();
                $(".choicebox").after($("<div class=\"pInfoFrame pInfohover clearfix choicebox pInfoNogoods \"><div class=\"tc sallputTips\"><p class=\"mt20 mb15\">" + errorMsg + "</p><a class=\"f14 fb a2\" href=\"/Market/DailyScareBuying.aspx\">查看其它云商团商品</a></div></div>"));
                return;
            } else {
                //该天天抢可以购买，更改最大购买数量
...
$(document).ready(function() {
    ...
    var imgurl = " /StyleVisitLog.aspx?Refer=" + escape(refer) + "&Client=" + cid + "&ClientVisitTime=" + timeInfo + "&StyleId=" + styleEntityListJson[0].StyleId;
...
function GetLeaveword() {
    ...
    $.ajax({
        type: "GET",
        contentType: "application/json",
        url: " /Question/IStyleQuestion.aspx",
...
function FillBuyerLeaveword(questionList) {
    ...
    for (var i in questionList) {
        ...
        var strMoban = "<div class=\"cf\">" + "     <a class=\"discusslist-Item-avatar fl\" target=\"_blank\" href=\" /share/" + questionList[i].ContactId + "-" + questionList[i].SpaceKey + "-1.htm\">" + "        <img src=\"images/load.gif\" onmouseout='MemberCard.PopHide()' onmouseover=\"MemberCard.Show(this,'" + questionList[i].ContactId + "')\" ></a>" + "     <div class=\"discusslist-item-con\">" + "         <div class=\"tr c9\">" + "             <span class=\"fl\"><a href=\" /share/" + questionList[i].ContactId + "-" + questionList[i].SpaceKey + "-1.htm\" target=\"_blank\">" + "             " + questionList[i].Name + " (" + questionList[i].MembershipName + ")</a>提问：</span>" + "             " + questionList[i].QuestionTime + "         </div>" + "         <p class=\"mt5\">" + questionList[i].Content + "</p>" + "    </div>" + "</div>";
...
function GetComment(pageIndex) {
    ...
    if (pageIndex == 1) {
        $.ajax({
            type: "GET",
            contentType: "application/json",
            url: " /Comment/IStyleComment.aspx",
    ...
    //当加载第二页以上数据时，使用cdn缓存
        ...
        var url = "";
        if (origionalId) {
            var url = " /Comment/IStyleComment-commentcallback-" + pageIndex + "-" + pageSize + "-" + styleEntityListJson[0].StyleId + "-" + origionalId + ".htm";
        } else {
            var url = " /Comment/IStyleComment-commentcallback-" + pageIndex + "-" + pageSize + "-" + styleEntityListJson[0].StyleId + ".htm";
        }
        $.ajax({
            type: "GET",
            contentType: "application/json",
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result.Data != null) {
                var tempHtml = "";
                $(result.Data).each(function(i, n) {
                    $("#buyer-comment #comment-main-" + i).find("a").attr("href", " /share/" + n.ContactId + "-" + n.SpaceKey + "-1.htm");
...
function BindStyleDetailLeft() {
    ...
    //3.热门评论
    $.ajax({
        url: " /comment/hotcomment.htm?from=list&size=8&class=" + classId1,
        dataType: "jsonp",
        success: function(data) {
            ...
            if (data.length > 0) {
                ...
                $.each(data,
                function(i, n) {
                    var styleLink = " /p-" + n.styleid + ".htm#rel=DPY_RPSP";
                    itemsHtml += "<li class=\"item-s\">" + "<div class=\"item-s-hd\">" + "		<a target=\"_blank\" title=\"" + unescape(n.name) + "\" href=\"" + styleLink + "\" class=\"item-s-title\">" + unescape(n.name) + "</a>" + "		<p class=\"item-s-price\"><span>" + unescape(n.price).replace(/￥/, '') + "</span></p>" + "	</div>" + "	<a target=\"_blank\" title=\"" + unescape(n.name) + "\" href=\"" + styleLink + "\" class=\"item-s-pic\"><img height=\"65\" width=\"50\" alt=\"\" src=\"" + n.image + "\"></a>" + "	<div class=\"item-s-txt\">" + "		<p class=\"item-s-name\"><a target=\"_blank\" href=\" /share/" + n.contactId + "-" + n.hmac + "-1.htm\">" + unescape(n.member) + "</a></p>" + "		<p class=\"item-s-comm\"><a target=\"_blank\" title=\"" + unescape(n.content) + "\" href=\"" + styleLink + "\">" + unescape(n.content) + "</a></p>" + "	</div>" + "</li>";
            ...
            success: function(result) {
                ...
                if (result && result.IsSuccess && result.ReturnValue && result.ReturnValue.length > 0) {
                    ...
                    for (var i = 0; i < result.ReturnValue.length; i++) {
                        ...
                        var link = " /" + curStyle.StyleType + "-" + curStyle.StyleId + ".htm#rel=DPY_ZJLL";
...
function BindEvent() {
    ...
    if ($("#video").length > 0) {
        ...
        var s1 = new SWFObject(" /js/lib/jcplayer.swf", "play", "300", "390", "7", "#FFFFFF");
...
function sizeLink() {
    ...
    if ($('ul.sku-size-select a:first').eq(0).text() == '均码' || classid1 != 'N1') {} else {
        ...
        if ($('#iteminfo').find('.a2&.viewSize').size() == 0) {
            $('<a id="TLXYJSQ" class="a2 viewSize ml10" style="cursor:pointer;"><img alt="量体选衣" src=" /asset/m18/product/i/mb.gif" class="mr5">查看适合我的尺码</a>').appendTo('.sizeOptions').show();
...
function AutoClickSku1(autoSku1, styleEntity) {
    ...
    if (parentDiv.find(".sku-color-select li").size() == 0 && parentDiv.find(".sku-size-select li").size() == 0) {
        ...
        var link = " /" + styleEntity.ClassId1 + "-" + classId + "-12-60-1-00-1-1-N-N-0-0.htm";
