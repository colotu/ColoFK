...
var SinaLogin = {
    ...
    Login: function() {
        ...
        window.open(" /sina/logintransfer.aspx?from=m18&logintype=" + $(this).attr("logintype") + "&returnurl=" + strReturnUrl);
        $("body").CloseLogin();
    },
    AlipayLogin: function() {
        var strReturnUrl = "http://" + window.location.host + "/logintransfer.html?t=callback";
        window.open(" /Alipay/alipayTransferRequest.aspx?logintype=" + $(this).attr("logintype") + "&returnurl=" + strReturnUrl);
        $("body").CloseLogin();
    },
    KaixinLogin: function() {
        var strReturnUrl = "http://" + window.location.host + "/logintransfer.html?t=callback";
        window.open(" /Kaixin/KaixinTransferRequest.aspx?logintype=" + $(this).attr("logintype") + "&returnurl=" + strReturnUrl);
        $("body").CloseLogin();
    }
};
...
(function($) {
    var login = {
        ...
        CheckLogin: function(options, obj) {
            $.getJSON(" /Service/LoginV2.ashx?Method=CheckLogin&jsoncallback=?",
            function(data) {
                if (!data.error) {
        ...
        GetVerifyCode: function(type) {
            $.getJSON(" /Service/LoginV2.ashx?Method=FloatVerifyCode&jsoncallback=?",
            function(data) {
                if (type == "login") {
        ...
        RegNow: function(options, userName, pwd, vcode) {
            $.getJSON(" /Service/LoginV2.ashx?Method=floatRegister&userName=" + userName + "&pwd=" + encodeURIComponent(pwd) + "&verifyCode=" + vcode + "&oldGuid=" + login.verifyCodeReg + "&jsoncallback=?",
            function(data) {
                $("#vcodeReg").val("");
                $("#vcodeOkRegTip").html("");
                if (data.Msg == 6) {
        ...
        LoginNow: function(options, userName, pwd, vcode) {
            $.getJSON(" /Service/LoginV2.ashx?Method=floatLogin&userName=" + userName + "&pwd=" + encodeURIComponent(pwd) + "&vcode=" + vcode + "&oldGuid=" + login.verifyCodeLogin + "&jsoncallback=?",
            function(data) {
                $("#vcodeLogin").val("");
                $("#pwdOkLoginTip").html("");
                $("#vcodeOkLoginTip").html("");
        ...
        CheckAgreement: function() {
            if ($("#agreement").attr("checked") == true) {
                ...
                $("#vcodeRegTip").removeClass("msg msg-ok msg-naked").removeClass("msg msg-info").addClass("msg msg-err").html("<i class=\"msg-ico\"></i><p>请先阅读并同意《云商用户服务协议》</p>");
        ...
        CheckRegName: function() {
            ...
            if (emailval != "") {
                if (emailval.indexOf("@ys56.com") > 0) {
                    $("#UserNameRegTip").removeClass("msg msg-ok msg-naked").removeClass("msg msg-info").addClass("msg msg-err").html("<i class=\"msg-ico\"></i><p>请填写有效的Email地址,不能使用@ys56.com结尾的邮箱</p>");
                ...
                if (!regs.test(emailval)) {
                    ...
                    $.getJSON(" /Service/LoginV2.ashx?Method=floatCheckEmail&userName=" + emailval + "&jsoncallback=?",
                    function(data) {
                        if (data.code == "false") {
                            $("#UserNameRegTip").removeClass("msg msg-ok msg-naked").removeClass("msg msg-info").addClass("msg msg-err").html("<i class=\"msg-ico\"></i><p>该Email已被注册，请使用其他Email地址注册</a></p>");
                            login.userNameRegStatus = false;
