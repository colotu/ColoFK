...
$(function () {
    ...
    $("#btnSendSMS").click(function () {
 	...
 	if (phoneStatus == false) {
        ...
        if (phoneStatus) {
			...
			ShowErrorMsg("验证码已过期,请重新输入！");

                            //刷新图形验证码
                            ChangeImageCode();
                            break;
                        case "IMAGECODEISERROR": //验证码错误
			ShowErrorMsg("验证码有误,请重新输入！");
                            break;
    ...
    $("#checkCode").blur(function () {
        var code = $(this).val();
        if (code == "") {
            ShowErrorMsg("请输入手机效验码");
            codeStatus = false;
            return;
        }
        var phone = $("#phone").val();
        if (phone != $("#hfPhoneNumber").val()) {
            ShowErrorMsg("请输入一致的手机号码");
...
function CheckEmail(obj) {
    ...
    var emailval = escape(obj.val());
    if (emailval != "") {
        if (!regs.test(emailval)) {
            ShowErrorMsg("请填写有效的Email地址");
            mailStatus = false;
        } else {
//验证注册邮箱是否存在
        $.ajax({
            ...
            success: function (resultData) {
                if (resultData == "true") {
                    ...
                    ShowErrorMsg("该Email已被注册，请使用其他Email地址注册");
                    mailStatus = false;
                }
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
    ...
    ShowErrorMsg("请填写有效的Email地址作为登录用户名");
        mailStatus = false;
    }
    return;
}
            ...
            $.ajax({
                ...
                success: function (resultData) {
                    if (resultData == "true") {
                        ...
                        ShowErrorMsg("该手机号码已被注册，请使用其他手机号码注册");
                        phoneStatus = false;
                    }
                },
                error: function (XMLHttpRequest, textStatus, errorThrown) {
...
function CheckNickname(obj) {
    var i = 0;
    var niclnamevalue = escape(obj.val());
    if (niclnamevalue.indexOf(";") > -1) {
        ShowErrorMsg("用户名不能包含“；”");
        $(this).val("");
        i++;
        if (i >= 3) {
            ShowErrorMsg("别玩了，这样有意思吗？");
        }
        ...
        $.ajax({
            ...
            success: function (resultData) {
                if (resultData == "true") {
                    ...
                    ShowErrorMsg("该昵称已被其他用户抢先使用，换一个试试！");
                    nicknameStatus = false;
                }
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
