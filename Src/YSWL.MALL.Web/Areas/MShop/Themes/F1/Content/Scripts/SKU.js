        ...
        $('#SKUOptions .SkuItems').click(function () {
            ...
            if (selectedObjs.length) {
                ...
                if (selectedObjs.length == $('#SKUOptions .AttrItems').length &&
                    skus && skus.length == 1) {
                    $('#div_stock').css('display', 'inline-block').find('#stock_num').text(SKUResult[selectedIds.join(',')].count);
...
function InitializationSku() {
   ...
   if ($('#SKUOptions .SkuItems').length == $('#SKUOptions .SkuItems[disabled]').length) {
            $('#SKUOptions').parent().find('#btnAddToCart').removeClass('addCart').addClass('addCart-gray').text('该商品已售罄');
...
function Status_SKUItem(target, enabled) {
                        if (enabled) {
                            var skuItem = $(target).removeAttr('disabled').find('a:eq(0)').removeClass('none_');
                            skuItem.attr('title', skuItem.attr('alt'));
                            skuItem.find('img:eq(0)').attr('title', skuItem.attr('alt'));
                        } else {
                            var skuItem = $(target).attr('disabled', 'disabled').removeClass('cur').find('a:eq(0)').addClass('none_');
