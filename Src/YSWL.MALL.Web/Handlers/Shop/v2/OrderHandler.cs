    ...
    public class OrderHandler : YSWL.MALL.Web.Handlers.Shop.OrderHandler
    {
        ...
        private string SubmitOrder(HttpContext context)
        {
            ...
            if (dicSuppOrderItems.Count > 1)
            {
                ...
                var terraceShip = _shippingTypeManage.GetCacgeModelBySupplied(0);
                if (terraceShip == null)
                {
            ...
            { 
                ...
                foreach (KeyValuePair<int, List<OrderItems>> item in SuppOrderItems)
                {
                    ...
                    //DONE: 获取配送(物流)
                    Model.Shop.Shipping.ShippingType subshipType = _shippingTypeManage.GetCacgeModelBySupplied(item.Key);
                    ...
                    {
                        #region 商家商品计算运费 
                        decimal price= GetFreight(subOrder, regionInfo);
            ...
            else
            {
                ...
                decimal price = GetFreight(mainOrder, regionInfo);
            ...
            else
            {
                ...
                #region 配送信息(物流)
                Model.Shop.Shipping.ShippingType shippingType = _shippingTypeManage.GetCacgeModelBySupplied(mainOrder.SupplierId.Value);
        ...
        private  decimal GetFreight(OrderInfo orderInfo, Model.Ms.Regions regionInfo)
        {
            decimal price = 0;
            int weight = 0;
            ...
            Model.Shop.Shipping.ShippingType shipType = _shippingTypeManage.GetCacgeModelBySupplied(orderInfo.SupplierId??0);
