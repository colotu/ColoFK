...
namespace YSWL.MALL.Web.Handlers
{
    public class ImageReGenHandler : IHttpHandler
    {
        ...
        protected void HttpToGen(HttpContext context)
        {
            ...
            //社区图片任务
            if (taskType == (int)YSWL.MALL.Model.SysManage.EnumHelper.TaskQueueType.SNSImageReGen)
            {
        ...
        {
            YSWL.MALL.Model.SNS.Photos photoModel = photoBll.GetModelByCache(phottoId);
            string SNSThumbBath = "/"+MvcApplication.UploadFolder+"/SNS/Images/PhotosThumbs/";
            if (photoModel != null)
            {
                //是否是旧数据
                ...
                List<YSWL.MALL.Model.Ms.ThumbnailSize> thumSizeList =
                    YSWL.MALL.BLL.Ms.ThumbnailSize.GetThumSizeList(YSWL.MALL.Model.Ms.EnumHelper.AreaType.SNS,MvcApplication.ThemeName);
                //原始文件名
        ...
        protected void ReGenProductImage(int productId)
        {
            ...
            string ShopThumbBath = "/"+MvcApplication.UploadFolder+"/Shop/Images/ProductThumbs/";
            if (list.Count > 0)
            {
