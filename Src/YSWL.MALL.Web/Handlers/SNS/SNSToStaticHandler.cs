...
namespace YSWL.MALL.Web.Handlers.SNS
{
    public class SNSToStaticHandler : IHttpHandler
    {
        ...
        protected void HttpToStatic(HttpContext context)
        {
            ...
            if (type == (int)YSWL.MALL.Model.SysManage.EnumHelper.TaskQueueType.SNSProduct)//商品静态化
            {
                ProductToStatic(context);
            }
            if (type == (int)YSWL.MALL.Model.SysManage.EnumHelper.TaskQueueType.SNSPhoto)//图片静态化
            {
        ...
        protected void ProductToStatic(HttpContext context)
        {
            ...
            #region 循环静态化
            //静态化之前先清除表任务
            taskBll.DeleteTask((int)YSWL.MALL.Model.SysManage.EnumHelper.TaskQueueType.SNSProduct);
            ...
            if (list != null && list.Count > 0)
            {
                ...
                foreach (int productId in list)
                {
                    ...
                    if (!TaskList.Select(c => c.TaskId).Contains(productId))
                    {
                        ...
                        taskModel.Status = 0;
                        taskModel.Type = (int)YSWL.MALL.Model.SysManage.EnumHelper.TaskQueueType.SNSProduct;
        ...
        protected void PhotoToStatic(HttpContext context)
        {
            ...
            #region 循环静态化
            //静态化之前先清除表任务
            taskBll.DeleteTask((int)YSWL.MALL.Model.SysManage.EnumHelper.TaskQueueType.SNSPhoto);
            TaskList = new List<Model.SysManage.TaskQueue>();
            if (list != null && list.Count > 0)
            {
                YSWL.MALL.Model.SysManage.TaskQueue taskModel = null;
                ...
                foreach (int ID in list)
                {
                    ...
                    taskModel.TaskId = ID;
                    taskModel.Status = 0;
                    taskModel.Type = (int)YSWL.MALL.Model.SysManage.EnumHelper.TaskQueueType.SNSPhoto;
                    if (taskBll.Add(taskModel))
                    {
                        TaskList.Add(taskModel);
                        i++;
        ...
        protected void GenerateHtml(HttpContext context)
        {
            ...
            if (type == (int)YSWL.MALL.Model.SysManage.EnumHelper.TaskQueueType.SNSProduct && item != null)
            {
                string saveUrls = PageSetting.GetProductUrl(item.TaskId);
                //TODO: 前台商品页面暂时未出来 To:涂朝辉 BEN ADD 2012-12-22
                ...
                {
                    if (YSWL.MALL.BLL.CMS.GenerateHtml.HttpToStatic(requestUrl, saveUrls))
                    {
            ...
            //静态化图片页面
            if (type == (int)YSWL.MALL.Model.SysManage.EnumHelper.TaskQueueType.SNSPhoto && item != null)
            {
        ...
        protected void DeleteTask(HttpContext context)
        {
            ...
            YSWL.MALL.BLL.CMS.GenerateHtml.HttpToStatic(requestUrl, "/index.html");
            taskBll.DeleteTask((int)YSWL.MALL.Model.SysManage.EnumHelper.TaskQueueType.SNSProduct);//删除商品任务列表
            taskBll.DeleteTask((int)YSWL.MALL.Model.SysManage.EnumHelper.TaskQueueType.SNSPhoto);//删除图片任务列表
        }
