...
using YSWL.MALL.Web.Handlers.API;
using System.Text;
    ...
    public partial class SNSHandler
    {
            ...
            string TrueName, string Email, string Phone, string MobileDI, string MobileDMID)
        {
            ...
            try
            {
                ...
                YSWL.MALL.BLL.Members.UsersExp bllUsersExp = new BLL.Members.UsersExp();
        ...
        private JsonObject GetUserInfo4Json(User userInfo)
        {
            ...
            json.Put("headImage", "/"+MvcApplication.UploadFolder+"/User/Gravatar/" + (user.UserID) + ".jpg?id=" + DateTime.Now);
        ...
        public JsonObject FavorList(int pageIndex = 1, int pageNum = 10, int UserID = -1)
        {
            ...
            //计算分页起始索引
            int startIndex = pageIndex > 1 ? (pageIndex - 1) * pageNum + 1 : 1;
            ...
            PagedList<YSWL.MALL.ViewModel.Shop.FavoProdModel> lists = new PagedList<YSWL.MALL.ViewModel.Shop.FavoProdModel>(favoList, pageIndex, pageNum, toalCount);
            foreach (YSWL.MALL.ViewModel.Shop.FavoProdModel item in lists)
            {
                json = new JsonObject();
                json.Put("id", item.ProductId);
                json.Put("name", item.ProductName);
                json.Put("pic", YSWL.MALL.Web.Components.FileHelper.GeThumbImage(item.ThumbnailUrl1, "T175X228_"));
                array.Add(json);
            }
