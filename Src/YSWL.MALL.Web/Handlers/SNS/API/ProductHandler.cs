...
namespace YSWL.MALL.Web.Handlers.SNS.API
{
    public partial class  SNSHandler
    {
        ...
        {
            List<YSWL.MALL.Model.SNS.Categories> AllList = YSWL.MALL.BLL.SNS.Categories.GetAllList(Type);
            List<YSWL.MALL.Model.SNS.Categories> cateList = AllList.Where(c => c.ParentID == Pid).ToList();
            if (Top > 0)
            {
        ...
        public JsonObject GetAllList(int Type = 0)
        {
            ...
            List<YSWL.MALL.Model.SNS.Categories> firstList = AllList.Where(c => c.ParentID == 0).ToList();
            Json.JsonArray jsonArray = new JsonArray();
            JsonObject json;
        ...
        public JsonObject GetProductList(int Cid, int Top = 10, bool hasChild = true)
        {
            ...
            List<YSWL.MALL.Model.SNS.Products> productList = productBll.GetModelList("");
            result.Put("status", "success");
            return result;
        }
        #endregion
        ...
        public JsonObject GetProPageList(int Cid, int pageIndex = 1, int pageSize = 10, bool hasChild = true)
        {
            ...
            List<YSWL.MALL.Model.SNS.Products> productList = productBll.GetProductByPage(Query, startIndex, endIndex);
            if (productList == null)
            {
                result.Put("status", "fail");
                result.Put("result", "productModel");
        ...
        public JsonObject SearchProPageList(int Cid, string keyword, string order, int pageIndex = 1, int pageSize = 10, bool hasChild = true)
        {
            ...
            YSWL.MALL.ViewModel.SNS.ProductCategory Model = new ViewModel.SNS.ProductCategory();
            YSWL.MALL.BLL.SNS.Products ProductBll = new BLL.SNS.Products();
            YSWL.MALL.Model.SNS.ProductQuery Query = new Model.SNS.ProductQuery();
            Query.Order = order;
        ...
        public JsonObject ProductDetail(long productid)
        {
            ...
            YSWL.MALL.Model.SNS.Products productModel = productBll.GetModelByCache(productid);
