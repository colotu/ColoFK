...
{
    public partial class BShopList : PageBaseAdmin
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                {
                    this.labelB2BUrl.Text = "http://" + Common.CallContextHelper.GetEnterpriseId() + "." + Common.Globals.DomainFullName + MvcApplication.GetCurrentRoutePath(YSWL.Web.AreaRoute.BShop);
                }
                else
                {
                    this.labelB2BUrl.Text = "http://" + Common.Globals.DomainFullName + MvcApplication.GetCurrentRoutePath(YSWL.Web.AreaRoute.BShop);
                }
                BindData();
        ...
        {
            //获取该主区域 下的所有模板
            List<YSWL.MALL.Model.Ms.Theme> themeList = YSWL.MALL.Web.Components.FileHelper.GetThemes("BShop");
            //获取当前主模板
            string name = YSWL.MALL.BLL.SysManage.ConfigSystem.GetValueByCache("BShop_Theme");
            hidCurrentColor.Value = YSWL.MALL.BLL.SysManage.ConfigSystem.GetValueByCache("BShop_Theme_Color");
            ...
            foreach (var item in themeList)
            {
                ...
                if (tList.Contains(item.Name))
                {
                    ...
                    blueItem = new Model.Ms.Theme
                    {
                        ...
                        Author = item.Author,
                        Description = item.Description,
                        PreviewPhotoSrc = "/Areas/BShop/Themes/" + item.Name + "/Theme-blue.png",
                        Remark = "蓝色",
                        Color = "blue"
                    };
                    ...
                    orangeItem = new Model.Ms.Theme
                    {
                        ...
                        Author = item.Author,
                        Description = item.Description,
                        PreviewPhotoSrc = "/Areas/BShop/Themes/" + item.Name + "/Theme-orange.png",
                        Remark = "橙色",
                        Color = "orange"
                    };
                    ...
                    redItem = new Model.Ms.Theme
                    {
                        ...
                        Author = item.Author,
                        Description = item.Description,
                        PreviewPhotoSrc = "/Areas/BShop/Themes/" + item.Name + "/Theme-red.png",
                        Remark = "红色",
                        Color = "red"
                    };
        ...
        protected void DataListPhoto_ItemCommand(object source, DataListCommandEventArgs e)
        {
            if (e.CommandName == "start")
            {
                if (e.CommandArgument != null)
                {
                    ...
                    //写
                    YSWL.MALL.BLL.SysManage.ConfigSystem.Modify("BShop_Theme", cA[0], "商城模板的名称");
                    if (cA.Length > 1)
                    {
                        YSWL.MALL.BLL.SysManage.ConfigSystem.Modify("BShop_Theme_Color", cA[1], "商城模板颜色");
                    }
        ...
        private string UpdateColor()
        {
            ...
            {
                //写
                YSWL.MALL.BLL.SysManage.ConfigSystem.Modify("BShop_Theme_Color", color, "商城模板的颜色");
                Cache.Remove("ConfigSystemHashList");    //清除网站设置的缓存文件
