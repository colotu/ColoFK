    ...
    public partial class Add : PageBaseAdmin
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                #endregion
                model.SupplierId = SupplierId = bll.Add(model, suppAreaId);
                if (SupplierId > 0)
                {
                    if (null == list || list.Count >0) {
                        if (!bll.Add(SupplierId, list))
                        {
                            MessageBox.ShowServerBusyTip(this, "新增商家成功，关联品牌失败！", "List.aspx");
                            return;
