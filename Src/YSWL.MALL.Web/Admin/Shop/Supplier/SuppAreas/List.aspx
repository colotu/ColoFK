<%@ Page Title="ShopCategories" Language="C#" MasterPageFile="~/Admin/Basic.Master"
    AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="YSWL.MALL.Web.Admin.Shop.Supplier.SuppAreas.List" %>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $(".btnDelete").click(function () {
                ...
                if (confirm("删除区域会删除该区域下所有子区域\n，确定要删除选择的区域吗？")) {
                    $.ajax({
                        url: ("List.aspx?timestamp={0}").format(new Date().getTime()),
                        type: 'POST', dataType: 'json', timeout: 10000,
                        data: { Action: "Delete", Callback: "true", AreaId: AreaId },
                        success: function (resultData) {
                            ...
                            if (resultData.STATUS == "NO") {
                                ShowFailTip('该区域或者子区域已被其它店铺数据使用，不能删除！');
                                return;
                            }
            ...
            $(".btnUpdate").click(function () {
                ...
                $.ajax({
                    url: ("List.aspx?timestamp={0}").format(new Date().getTime()),
                    type: 'POST', dataType: 'json', timeout: 10000,
                    data: { Action: "UpdateSeqNum", Callback: "true", AreaId: AreaId, UpdateValue: value },
                    success: function (resultData) {
                        if (resultData.STATUS == "SUCCESS") {
                            ShowSuccessTip('保存成功');
                            self.prev().val(value);
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="borderkuang">
                <tr>
                    <td bgcolor="#FFFFFF" class="newstitle">
                        <asp:Literal ID="Literal1" runat="server" Text="店铺区域管理" />
                    </td>
                </tr>
            </table>
        </div>
            ...
            <div class="newsicon">
                <ul>
                    <li class="add-btn" id="liAdd"
                        runat="server"><a href="add.aspx" title="新增新的店铺区域">新增</a></li>
                    <li class="add-btn">
        ...
        <asp:UpdatePanel runat="server" ID="UpdatePanel1">
            <ContentTemplate>
                <asp:GridView ID="gridView" runat="server" AutoGenerateColumns="false" ShowHeader="true"
                    DataKeyNames="AreaId" CssClass="GridViewStyle" RowStyle-CssClass="grdrow"
                    ...
                    <Columns>
                        <asp:TemplateField HeaderText="<%$ Resources:CMSVideo, CategoryName %>" ItemStyle-HorizontalAlign="Left">
                            <ItemTemplate>
                                <span id="spShowImage" runat="server" parentid='<%# Eval("ParentAreaId") %>'>
                                    <img src="/admin/images/jian.gif" width="24" height="24" alt="" />
                                </span>
                                <asp:Label ID="lblName" runat="server" />
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="区域编号" ItemStyle-Width="80" ItemStyle-HorizontalAlign="Center">
                            <ItemTemplate>
                                <%#Eval("AreaId") %>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="显示顺序" ItemStyle-Width="120" ItemStyle-HorizontalAlign="Left">
                            <ItemTemplate>
                                ...
                                <a href="javascript:;" AreaId='<%#Eval("AreaId") %>' class="btnUpdate">保存</a>
                            </ItemTemplate>
                        </asp:TemplateField>
                        ...
                        <asp:TemplateField HeaderText="状态" ItemStyle-HorizontalAlign="Center" ItemStyle-Width="180">
                            <ItemTemplate>
                                <asp:LinkButton ID="lbtnStatus" runat="server" CausesValidation="False" CommandName="Status"
                                    CommandArgument='<%#Eval("AreaId")+","+Eval("Status")%>' Style="color: #0063dc;">
                        ...
                        <asp:TemplateField HeaderText="<%$ Resources:CMSVideo, Operation %>" ItemStyle-Width="240"
                            ...
                            <ItemTemplate>
                                <asp:HyperLink CssClass="Add" ID="HyperLink1" runat="server" Text="<%$ Resources:Site, ltlAdd %>"
                                    NavigateUrl='<%#Eval("AreaId", "Add.aspx?id={0}")%>' ForeColor="Black" Visible="false"></asp:HyperLink>
                                <asp:HyperLink CssClass="Show" ID="HyperLink3" runat="server" Text="<%$ Resources:Site, btnDetailText %>"
                                    NavigateUrl='<%#Eval("AreaId", "Show.aspx?id={0}")%>' ForeColor="Black" Visible="false"></asp:HyperLink>
                                <a class="iframe" href="swap.aspx?id=<%#Eval("AreaId") %>" style="display: none;">转移商品</a> &nbsp;&nbsp; <span id="lbtnModify" runat="server"><a href="Modify.aspx?id=<%#Eval("AreaId") %>"
                                >编辑</a> &nbsp;&nbsp;</span> <a class="btnDelete" areaId='<%#Eval("AreaId") %>'>
                                            删除</a>
                            </ItemTemplate>
                        </asp:TemplateField>
                    </Columns>
