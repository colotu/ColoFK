...
{
    public partial class ProductAdd : PageBaseAdmin
    {
        ...
        private void SaveProductInfo()
        {
            ...
            if (BLL.Shop.Products.ProductManage.AddProduct(productInfo, out ProductId))
            {
                ...
                string url = "http://" + Common.Globals.DomainFullName + MvcApplication.GetCurrentRoutePath(AreaRoute.Shop) + "Product/Detail/" + ProductId;
                ...
                if (productInfo.SalesType == 2)//跳转到预订商品页处理
                {
                    Response.Redirect(string.Format("/Admin/Shop/PrePro/AddPrePro.aspx?id={0}", ProductId));
                }
                else
                {
