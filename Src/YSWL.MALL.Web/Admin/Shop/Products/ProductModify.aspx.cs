...
namespace YSWL.MALL.Web.Admin.Shop.Products
{
    public partial class ProductModify : PageBaseAdmin
    {
        ...
        //待上传的SKU图片名称
        private string tempFile = string.Format("/"+MvcApplication.UploadFolder+"/Temp/{0}/", DateTime.Now.ToString("yyyyMMdd"));
        private string skuImageFile = string.Format("/"+MvcApplication.UploadFolder+"/Shop/Images/ProductsSkuImages/{0}/", DateTime.Now.ToString("yyyyMMdd"));
        ...
        private void SaveProductInfo()
        {
           ...
           // string productAccessorieJson = hfSelectedAccessories.Value;
            ...
            int oldSupplierId = productInfo.SupplierId;
            //产品分类
            ...
            //待上传的图片名称
            //string ImageFile = string.Format("/"+MvcApplication.UploadFolder+"/Shop/Images/Product/{0}", DateTime.Now.ToString("yyyyMMdd"));

            string savePath = string.Format("/"+MvcApplication.UploadFolder+"/Shop/Images/Product/{0}/", DateTime.Now.ToString("yyyyMMdd"));
            string saveThumbsPath = "/"+MvcApplication.UploadFolder+"/Shop/Images/ProductThumbs/" + DateTime.Now.ToString("yyyyMMdd") + "/";
            ...
            for (int i = 0; i < productImages.Length; i++)
            {
                ...
                else
                {
                    ...
                    {
                        string tempFileSwap="/"+MvcApplication.UploadFolder+"/Shop/Images/ProductThumbs/";
                        string savePathSwap= "/"+MvcApplication.UploadFolder+"/Shop/Images/Product/";
                        string AttachImageUrl = string.Format(productImages[i], "");
                        string AttachThumbnailUrl1 = productImages[i];
                        productInfo.ProductImages.Add(
                          new ProductImage
            ...
            if (ProductManage.ModifyProduct(productInfo, oldSaleStatus))
            {
                ...
                {
                    supplierIds = productInfo.SupplierId+",";
                    DataCache.DeleteCache("SuppliersModel-" + productInfo.SupplierId);
                }

                if (oldSupplierId > 0)
                {
                    supplierIds += oldSupplierId+",";
                    DataCache.DeleteCache("SuppliersModel-" + oldSupplierId);
                }
                if (!String.IsNullOrWhiteSpace(supplierIds)) {
                    supplierIds = supplierIds.TrimEnd(',');
                    supplierBll.UpdateProductCountList(supplierIds);
                }
