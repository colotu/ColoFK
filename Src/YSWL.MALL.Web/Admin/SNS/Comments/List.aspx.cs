...
namespace YSWL.MALL.Web.Admin.SNS.Comments
{
    public partial class List : PageBaseAdmin
    {
        //int Act_ShowInvalid = -1; //查看失效数据行为

        private YSWL.MALL.BLL.SNS.Comments bll = new YSWL.MALL.BLL.SNS.Comments();
        ...
        public int TargetID
        {
            get
            {
                if (!string.IsNullOrWhiteSpace(Request.Params["targetid"]))
                {
                    ...
                    {
                        YSWL.MALL.BLL.SNS.Posts bllPost = new BLL.SNS.Posts();
                        YSWL.MALL.Model.SNS.Posts model = bllPost.GetModel(targetid);
