<%@ Page Title="小组话题管理" Language="C#" MasterPageFile="~/Admin/Basic.Master" AutoEventWireup="true"
    CodeBehind="GroupTopics.aspx.cs" Inherits="YSWL.MALL.Web.Admin.SNS.Groups.SNS_Groups.GroupTopics" %>
            ...
            <Columns>
                <asp:TemplateField HeaderText="标题" ItemStyle-HorizontalAlign="Left">
                    <ItemTemplate>
                            ...
                            href="/MSNS/Home/TopicReply?viewName=AdminTopicDetail&id=<%#Eval("TopicID")%>"><%#Eval("Title")%></a>

                        <%--<a target="_blank" href="<%=YSWL.Components.MvcApplication.GetCurrentRoutePath(YSWL.Web.AreaRoute.SNS)%>Group/TopicReply?id=<%#Eval("TopicID")%>">
                            <%#Eval("Title")%></a>--%>
                    </ItemTemplate>
                </asp:TemplateField>
                ...
                <asp:TemplateField ItemStyle-Width="100" HeaderText="置顶" ItemStyle-HorizontalAlign="Center"   >
                    <ItemTemplate>
                        <asp:LinkButton ID="lbtnAdminToTop" runat="server" CommandArgument='<%#Eval("TopicID")+","+Eval("IsTop")%>'
                            Style="color: #0063dc;" CommandName="RecommendToTop">
                         <span><%#(int)Eval("IsTop") == 1 ? "取消置顶" : "置顶"%></span></asp:LinkButton>
                    </ItemTemplate>
                </asp:TemplateField>
