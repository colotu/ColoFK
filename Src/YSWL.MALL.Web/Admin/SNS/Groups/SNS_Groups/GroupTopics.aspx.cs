...
{
    public partial class GroupTopics : PageBaseAdmin
    {
        //int Act_ShowInvalid = -1; //查看失效数据行为

        private YSWL.MALL.BLL.SNS.GroupTopics bllTopic = new YSWL.MALL.BLL.SNS.GroupTopics();
        private YSWL.MALL.BLL.SNS.Groups bllGroup = new BLL.SNS.Groups();
        ...
        public string GetGroupName(int GroupId)
        {
            if (GroupId > 0)
            {
                YSWL.MALL.Model.SNS.Groups model = bllGroup.GetModel(GroupId);
                if (model != null)
                {
        ...
        public void BindData()
        {
            ...
            if (!string.IsNullOrWhiteSpace(strWhere.ToString()))
            {
                ...
                {
                    strWhere.AppendFormat(" AND CreatedNickName  like '%{0}%'", Common.InjectionFilter.SqlFilter(this.txtName.Text));
                }
            }
            else
            {
                if (!string.IsNullOrWhiteSpace(this.txtName.Text))
                {
                    strWhere.AppendFormat(" CreatedNickName  like '%{0}%'", Common.InjectionFilter.SqlFilter(this.txtName.Text));
                }
            }
            if (txtKeyword.Text.Trim() != "")
            {
                ...
                strWhere.AppendFormat("GroupName like '%{0}%'", Common.InjectionFilter.SqlFilter(txtKeyword.Text.Trim()));
            }
        ...
        protected void gridView_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            //TODO 后台推荐
            if (e.CommandName == "RecommendToTop")
            {
                if (e.CommandArgument != null)
                {
                    int TopicID = 0;
                    ...
                    string[] Args = e.CommandArgument.ToString().Split(new char[] { ',' });
                    TopicID = Common.Globals.SafeInt(Args[0], 0);
                    IsTop = Common.Globals.SafeInt(Args[1], 0);
                    YSWL.MALL.BLL.SNS.GroupTopics groupTopicBll = new YSWL.MALL.BLL.SNS.GroupTopics();
                    YSWL.MALL.Model.SNS.GroupTopics groupTopicModel = groupTopicBll.GetModel(TopicID);
                    if (null != groupTopicModel)
                    {
                        groupTopicModel.IsTop = IsTop==1?0:1;//置顶为1 取消置顶为0
                        if (groupTopicBll.Update(groupTopicModel))
                        {
                            LogHelp.AddUserLog(CurrentUser.UserName, CurrentUser.UserType, "置顶群组主题（id=" + TopicID + "）成功!", this);
                            gridView.OnBind();
                        }
                    }
                }
        ...
        protected void ddlBatch_Changed(object sender, EventArgs e)
        {
            ...
            switch (type)
            {
                //批量审核
                case  1:
                    if (bllTopic.UpdateStatusList(idlist, YSWL.MALL.Model.SNS.EnumHelper.TopicStatus.Checked))
                    {
                ...
                //批量拒绝
                case 2:
                    if (bllTopic.UpdateStatusList(idlist, YSWL.MALL.Model.SNS.EnumHelper.TopicStatus.CheckedUnPass))
                    {
                   ...
                   // 批量禁言
                case 3:
                    YSWL.MALL.BLL.SNS.GroupUsers groupuserBll = new BLL.SNS.GroupUsers();
                    if (groupuserBll.UpdateStatusByTopicIds(idlist, (int)YSWL.MALL.Model.SNS.EnumHelper.GroupUserStatus.ForbidSpeak))
                    {
