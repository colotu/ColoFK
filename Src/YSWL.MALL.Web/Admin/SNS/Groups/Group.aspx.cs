...
namespace YSWL.MALL.Web.Admin.SNS.Groups
{
    public partial class List : PageBaseAdmin
    {
        //int Act_ShowInvalid = -1; //查看失效数据行为

        private YSWL.MALL.BLL.SNS.Groups bll = new YSWL.MALL.BLL.SNS.Groups();
        ...
        protected void btnCheck_Click(object sender, EventArgs e)
        {
            string idlist = GetSelIDlist();
            if (idlist.Trim().Length == 0) return;
            if (bll.UpdateStatusList(idlist, YSWL.MALL.Model.SNS.EnumHelper.GroupStatus.Checked))
            {
        ...
        protected void btnCheckedUnPass_Click(object sender, EventArgs e)
        {
            string idlist = GetSelIDlist();
            if (idlist.Trim().Length == 0) return;
            if (bll.UpdateStatusList(idlist, YSWL.MALL.Model.SNS.EnumHelper.GroupStatus.CheckedUnPass))
            {
                MessageBox.ShowSuccessTip(this, "操作成功！");
