...
namespace YSWL.MALL.Web.Admin.SNS.SearchWordTop
{
    public partial class List : PageBaseAdmin
    {
        //int Act_ShowInvalid = -1; //查看失效数据行为

        private YSWL.MALL.BLL.SNS.SearchWordTop bll = new YSWL.MALL.BLL.SNS.SearchWordTop();
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                LogHelp.AddUserLog(CurrentUser.UserName, CurrentUser.UserType, "批量删除查询排行(id=" + idlist + ")成功", this);

            }
            else
            {
                LogHelp.AddUserLog(CurrentUser.UserName, CurrentUser.UserType, "批量删除查询排行(id=" + idlist + ")失败", this);
            }
        ...
        public void BindData()
        {
            ...
            {
                strWhere.AppendFormat("HotWord like '%{0}%'", Common.InjectionFilter.SqlFilter(txtKeyword.Text.Trim()));
            }
        ...
        protected void gridView_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            if (bll.Delete(ID))
            { LogHelp.AddUserLog(CurrentUser.UserName, CurrentUser.UserType, "删除查询排行(id=" + ID + ")成功", this); }
            else
            { LogHelp.AddUserLog(CurrentUser.UserName, CurrentUser.UserType, "删除查询排行(id=" + ID + ")失败", this); }
            { }
            gridView.OnBind();
        }
