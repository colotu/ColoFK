...
namespace YSWL.MALL.Web.Admin.SNS.Posts
{
    public partial class List : PageBaseAdmin
    {
        private YSWL.MALL.BLL.SNS.Posts bll = new YSWL.MALL.BLL.SNS.Posts();
        private YSWL.MALL.BLL.SNS.UserBlog blogBll = new BLL.SNS.UserBlog();
        ...
        public void BindData()
        {
            ...
            if (!string.IsNullOrEmpty(txtPoster.Text.Trim()))
            {
                ...
                strWhere.AppendFormat("CreatedNickName like '%{0}%' ", Common.InjectionFilter.SqlFilter(txtPoster.Text.Trim()));
            ...
            if (txtKeyword.Text.Length > 0)
            {
                ...
                strWhere.Append(" Description like '%" + Common.InjectionFilter.SqlFilter(txtKeyword.Text) + "%' ");
            }
            if (strWhere.Length > 0)
            {
                strWhere.Append(" and ");
        ...
        protected void gridView_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandArgument.Equals("Delete"))
            {
                ...
                YSWL.MALL.Model.SNS.Posts PostModel = bll.GetModelByCache(postId);
        ...
        public string GetContent(object Type, object TargetId, object Description, object ImageUrl)
        {
            ...
            if (!StringPlus.IsNullOrEmpty(Type))
            {
                ...
                switch (Globals.SafeInt(Type.ToString(), -1))
                {
                    ...
                    case (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Normal:
                        str = Description != null ? Description.ToString() : "";
                        break;

                    case (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Photo:
                        {
                            if (!StringPlus.IsNullOrEmpty(ImageUrl))
                            {

                                str = " <a  target='_blank' href='" + BasePath + "Photo/Detail/" + TargetId.ToString() + "'><img src=\"" + YSWL.MALL.Web.Components.FileHelper.GeThumbImage(ImageUrl.ToString(), "T80x80_") + "\" style=\"height:64px;width:64px;border-width:0px;\"><a>";
                            }
                            else
                            {
                                str = Description != null ? Description.ToString() : "";
                        ...
                        break;
                    case (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Product:
                        {
                            if (!StringPlus.IsNullOrEmpty(ImageUrl))
                            {
                                str = " <a  target='_blank' href='" + BasePath + "Product/Detail/" + TargetId.ToString() + "'><img src=\"" + YSWL.MALL.Web.Components.FileHelper.GeThumbImage(ImageUrl.ToString(), "T80x80_") + "\" style=\"height:64px;width:64px;border-width:0px;\"><a>";
                            }
                            else
                            {
                                str = Description != null ? Description.ToString() : "";
                        ...
                        break;
                    case (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Video:
                        str = Description != null ? Description.ToString() : "";
                        break;
                    case (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Blog:
        ...
        public string GetTypeName(object Type, object TargetId)
        {
            ...
            if (!StringPlus.IsNullOrEmpty(Type))
            {
                switch (Globals.SafeInt(Type.ToString(), -1))
                {
                    ...
                    case (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Photo:
                        str = " <a  target='_blank' href='" + YSWL.Components.MvcApplication.GetCurrentRoutePath(AreaRoute.SNS) + "Photo/Detail/" + TargetId.ToString() + "'>图片<a>";
                        break;

                    case (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Product:
                        str = " <a  target='_blank' href='" + YSWL.Components.MvcApplication.GetCurrentRoutePath(AreaRoute.SNS) + "Product/Detail/" + TargetId.ToString() + "'>商品 <a>";
                        break;
                    case (int)YSWL.MALL.Model.SNS.EnumHelper.PostContentType.Video:
                        str = "视频 ";
                        break;
        ...
        {
            YSWL.MALL.Web.Components.FileHelper.DeleteFile(YSWL.MALL.Model.Ms.EnumHelper.AreaType.SNS, path);
        ...
        protected void btnCheck_Click(object sender, EventArgs e)
        {
            string idlist = GetSelIDlist();
            if (idlist.Trim().Length == 0) return;
            if (bll.UpdateStatusList(idlist, (int)YSWL.MALL.Model.SNS.EnumHelper.PostStatus.AlreadyChecked))
            {
        ...
        protected void btnCheckedUnPass_Click(object sender, EventArgs e)
        {
            string idlist = GetSelIDlist();
            if (idlist.Trim().Length == 0) return;
            if (bll.UpdateStatusList(idlist, (int)YSWL.MALL.Model.SNS.EnumHelper.PostStatus.CheckedUnPass))
            {
                MessageBox.ShowSuccessTip(this, "操作成功！");
