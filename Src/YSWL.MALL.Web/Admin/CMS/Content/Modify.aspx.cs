    ...
    public partial class Modify : PageBaseAdmin
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (ContentID > 0)
            {
                ...
                //上传图片正式地址
                string ImageFile = string.Format("/"+MvcApplication.UploadFolder+"/CMS/Article/{0}/", DateTime.Now.ToString("yyyyMM"));

                //上传附件正式地址
                string attachmentFile = string.Format("/"+MvcApplication.UploadFolder+"/CMS/Files/{0}/", DateTime.Now.ToString("yyyyMM"));

                string ThumbImageFile = string.Format("/"+MvcApplication.UploadFolder+"/CMS/ArticleThumbs/{0}/", DateTime.Now.ToString("yyyyMM"));
