...
Social_TencentSercet
            ...
            string nickNameOAuth, string emailOAuth)
        {
            ...
            if (userId > 0)
            {
                ...
                YSWL.MALL.BLL.SNS.UserAlbums albumsBll = new BLL.SNS.UserAlbums();
                YSWL.MALL.BLL.SNS.UserShip shipBll = new BLL.SNS.UserShip();
                albumsModel.AlbumName = "默认专辑";
                albumsModel.CreatedDate = DateTime.Now;
                albumsModel.CreatedNickName = currentUser.NickName;
                albumsModel.CreatedUserID = currentUser.UserID;
                albumsBll.AddEx(albumsModel, 1);

                //自动给是粉丝
                shipBll.GiveUserFellow(currentUser.UserID);
                //登录之后跳转到首页
                return RedirectToHome();
            }
            return Redirect("/");
